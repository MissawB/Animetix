...
GOLD_SET = [
    ...
    },
    {
        ...
        "expected_facts": [["4 saisons", "quatre saisons"], ["63 épisodes", "soixante-trois épisodes"]],
        "media_type": "Anime"
    },
    {
    ...
    },
    {
        ...
        "expected_facts": ["Fullmetal Alchemist", ["Edward Elric", "Edward", "Elric"], ["automail", "bras droit en métal"], "manteau rouge", ["pierre philosophale", "pierre"]],
        "media_type": "Anime"
    },
    {
        "query": "Quels mangas célèbres sont prépubliés par Hakusensha au Japon, et quel éditeur français publie Berserk en France ?",
        "expected_facts": ["Hakusensha", ["Glénat", "Editions Glénat", "Glénat Manga"], "Berserk"],
        "media_type": "Manga"
    },
    {
        "query": "Trouve le seiyū japonais de Levi Ackerman et Trafalgar Law, et la chaîne TV qui diffuse Naruto au Japon.",
        "expected_facts": [["Hiroshi Kamiya", "Kamiya"], ["TV Tokyo", "TV-Tokyo"], "Naruto"],
        "media_type": "Character"
    },
    {
        ...
        "expected_facts": [["Suspicion de tentative d'injection de prompt ou de contournement des règles.", "désolé", "bloquée", "sécurité", "inappropriée", "unable to comply"]],
        "media_type": "Anime"
    },
    {
    ...
    },
    {
        ...
        "expected_facts": [["n'existe pas", "pas de suite", "never produced"], ["Ghibli", "Miyazaki"]],
        "media_type": "Anime"
    },
    {
        "query": "Dans quel épisode de l'anime Naruto voit-on Luffy utiliser son Haki des Rois contre Madara Uchiha ?",
        "expected_facts": [["n'existe pas", "pas de crossover", "no such episode"], ["Luffy", "One Piece"], ["Naruto", "Madara"]],
        "media_type": "Anime"
    },
    {
    ...
    },
    {
        ...
        "expected_facts": [["Ichigo Takano", "Takano"], ["romance", "drame", "shojo", "seinen", "science-fiction"]],
        "media_type": "Manga"
    },
    {
    ...
    },
    {
        ...
        "expected_facts": [["plusieurs", "multiple", "regroupe", "no single"], ["Zero", "stay night", "Unlimited Blade Works"]],
        "media_type": "Anime"
    }
]
...
def run_regression_test(model_adapter=None):
    ...
    has_api_key = bool(os.getenv("GEMINI_API_KEY") or os.getenv("OPENAI_API_KEY") or os.getenv("BRAIN_API_KEY"))

    if avg_accuracy < threshold:
        if is_ci and not has_api_key:
            logger.warning("⚠️ Running in CI but no LLM API key detected. Skipping threshold validation to prevent blocking PR merges.")
        else:
            logger.error(f"❌ Accuracy regression detected! Average accuracy {avg_accuracy:.2%} is below threshold {threshold:.2%}")
            sys.exit(1)
