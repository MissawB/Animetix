...
class RAGEvaluator:
    ...
    def get_contexts_with_ids(self, query: str, mode: str = "vector") -> Tuple[List[str], List[str]]:
        ...
        try:
            ...
            vector_contexts = [doc for doc in results['documents'][0] if doc is not None]
        ...
        try:
            ...
            if graph_context:
                return vector_contexts + [f"Informations Graph complémentaires:\n{graph_context}"], media_ids
        except Exception as e:
            logger.warning(f"Neo4j graph enrichment failed: {e}")
        
        return vector_contexts, media_ids

    def calculate_deterministic_metrics(self, retrieved_contexts: List[str], retrieved_ids: List[str], expected_contexts: List[str], expected_chunks: List[str]) -> Dict[str, float]:
        ...
        if expected_contexts:
            ...
            for expected in expected_contexts:
                if any(expected.lower().strip() in retrieved.lower() for retrieved in retrieved_contexts):
                    hits += 1
        ...
        if expected_contexts:
            ...
            for k, retrieved in enumerate(retrieved_contexts):
                is_relevant = any(expected.lower().strip() in retrieved.lower() for expected in expected_contexts)
                if is_relevant:
                    matched_ranks += 1
    ...
    def generate_answer(self, query: str, contexts: List[str], history: List[Dict[str, str]] = None) -> str:
    ...
    async def run_evaluation(self, test_questions: List[Dict[str, str]], mode: str = "vector"):
        ...
        result = {
            ...
            "deterministic_context_recall": sum(det_recalls) / len(det_recalls) if det_recalls else 0.0,
            "deterministic_context_precision": sum(det_precisions) / len(det_precisions) if det_precisions else 0.0
        }
