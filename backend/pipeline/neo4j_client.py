...
class Neo4jManager(GraphDatabasePort):
    ...
    def _sync_character_tx(tx, item):
        ...
        for anime_id in clean_item.get('anime_appearances', []):
             tx.run("MATCH (c:Character {id: $cid}) MATCH (m:Media {id: $mid}) MERGE (c)-[:APPEARS_IN]->(m)", cid=str(clean_item['id']), mid=str(anime_id))
    ...
    def sync_ai_extracted_graph(self, media_id: str, extracted_data: dict):
        ...
        with self.driver.session() as session:
            ...
            session.execute_write(self._link_to_parent_tx, media_id, clean_data.get('entities', []))
    ...
    def _create_entity_tx(tx, entity):
        ...
        tx.run(query, name=clean_ent['name'], type=clean_ent['type'], description=clean_ent.get('description', ''))
    ...
    def _create_relation_tx(tx, rel):
        ...
        try:
            ...
            tx.run(query, source=clean_rel['source'], target=clean_rel['target'], rel_type=rel_type_raw)
    ...
    def _sync_tx(tx, item, media_type):
        ...
        tx.run(query, id=str(clean_item['id']), type=media_type, title=clean_item['title'], year=clean_item.get('year'))
        for studio_name in clean_item.get('studios', []):
            tx.run("MATCH (m:Media {id: $id}) MERGE (s:Studio {name: $studio_name}) MERGE (m)-[:PRODUCED_BY]->(s)", id=str(clean_item['id']), studio_name=studio_name)
        for tag in clean_item.get('micro_tags', []):
            tx.run("MATCH (m:Media {id: $id}) MERGE (t:MicroTag {name: $tag_name}) MERGE (m)-[:HAS_THEME]->(t)", id=str(clean_item['id']), tag_name=tag)
        author = clean_item.get('graph_nodes', {}).get('author') or clean_item.get('graph_nodes', {}).get('director')
        if author:
            tx.run("MATCH (m:Media {id: $id}) MERGE (p:Person {name: $author_name}) MERGE (m)-[:CREATED_BY]->(p)", id=str(clean_item['id']), author_name=author)
    ...
    def sync_saga(self, saga_name: str, executive_summary: str, media_ids: List[str]):
        ...
        with self.driver.session() as session:
            try:
                session.run(query, saga_name=clean_saga, summary=clean_summary, ids=media_ids)
            except Exception as e:
    ...
    def is_safe_read_query(query: str) -> bool:
        ...
        for kw in dangerous_keywords:
            if re.search(rf"\b{kw}\b", query_upper):
                logger.error(f"❌ Security Alert: Dangerous Cypher keyword '{kw}' detected.")
                return False
        return True

    def execute_read(self, query: str, parameters: Optional[Dict] = None) -> List[Dict]:
        if not self.is_safe_read_query(query): raise ValueError("Dangerous Cypher query rejected.")
        if not self.driver: return []
        with self.driver.session() as session:
            try:
