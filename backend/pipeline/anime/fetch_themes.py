...
def fetch_themes_for_anime(mal_id: int) -> dict | None:
    ...
    try:
        response = safe_http_request(
            "GET", ANIMETHEMES_API_URL, params=params, timeout=15
        )
...
def run_fetching(limit: int = 200) -> str:
    ...
    for anime in animes:
        ...
        logger.info(
            f"   - [{processed + 1}/{limit}] Fetching for: {anime.get('title')}"
        )
    ...
    status = (
        f"✅ Finished! Added themes for {count} animes. Total in DB: {len(themes_data)}"
    )
    logger.info(status)
    return status
