...
def run_expert_enrichment():
    ...
    FACTS_FILE = os.path.join(
        BASE_DIR, "src", "core", "domain", "services", "prompts", "expert_facts.yaml"
    )
    ...
    for file_path, label in files_to_process:
        ...
        logger.info(
            f"🔄 Enrichissement des {label}s dans {os.path.basename(file_path)}..."
        )
        ...
        logger.info(
            f"✅ Enrichissement terminé pour {label} : {enriched_items_count} items mis à jour ({total_facts_added} faits injectés)."
        )
