...
def run_vectorization():
    ...
    try:
        ...
        new_items = [
            ...
            or str(item["id"]) not in existing_plot_ids
        ]

        if not new_items:
            logger.info("ℹ️ Manga database up to date.")
            return

        logger.info(
            f"🚀 Multimodal Vectorization of {len(new_items)} mangas (Batching mode: {BATCH_SIZE})..."
        )
        ...
        for i in range(0, len(new_items), BATCH_SIZE):
            ...
            for item in batch:
                ...
                if m_id not in existing_vision_ids and item["image"]:
                    ...
                    try:
                        neo4j_manager.sync_media_to_graph(item, "Manga")
                    except Exception as e:
            ...
            if plot_texts:
                p_embeddings = text_model.encode(
                    plot_texts, convert_to_numpy=True
                ).tolist()
            ...
            if vision_images:
                ...
                repo.upsert_items(
                    "manga_visual_vibe", vision_ids, v_embeddings, vision_metas
                )
        ...
        logger.info("✅ Manga Multimodal Sync Complete.")

    except Exception as e:
        import traceback  # noqa: E402

        error_msg = (
            f"CRITICAL ERROR in run_vectorization():\n{e}\n{traceback.format_exc()}"
        )
        logger.error(error_msg)
        with open(
            os.path.join(BASE_DIR, "manga_vectorize_error.log"), "a", encoding="utf-8"
        ) as f:
            f.write(error_msg + "\n")
