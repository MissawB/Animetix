        ...
        if response.status_code == 200:
            ...
            for item in data:
                ...
                if locale in covers and file_name:
                    url = (
                        f"https://uploads.mangadex.org/covers/{mangadex_id}/{file_name}"
                    )
                    covers[locale].append({"url": url, "volume": volume})
        return covers
    except Exception as e:
...
def run_fetching(limit: int = 100) -> str:
    ...
    status = (
        f"✅ Finished! Added covers for {count} mangas. Total in DB: {len(covers_data)}"
    )
    logger.info(status)
    return status
