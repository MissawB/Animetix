...
def ingest_movies():
    ...
    for page in range(1, 51):
        ...
        if s:
            for item in s.get('results', []):
                ...
                item['media_type_custom'] = 'Cartoon' if (16 in genres and 'JP' not in origin) else 'Series'
                all_raw.append(item)
    ...
    for i, item in enumerate(candidates_to_enrich):
        ...
        details = fetch_tmdb_page(f"{m_type}/{item['id']}", params={"append_to_response": "credits,keywords,recommendations"})
        ...
        if details:
            formatted = {
                ...
                "title_english": details.get('original_title') or details.get('original_name'),
                "title_native": details.get('original_title') or details.get('original_name'),
                ...
                "tags": [k['name'] for k in details.get('keywords', {}).get('keywords' if m_type=="movie" else 'results', [])],
                "recommendations": {r['title'] if m_type=="movie" else r['name']: 50 for r in details.get('recommendations', {}).get('results', [])[:10]},
