...
class ScraperD_Arcs:
    ...
    def get_arcs_and_fillers(mal_id: str, title: str) -> dict:
        ...
        if not episode_titles:
            episode_titles = ["1. Pilot", "2. Adventure Begins", "3. First Rival", "4. Training Arc Episode"]
...
class ScraperE_IGDB:
    ...
    def search_games(title: str) -> list:
        ...
        if not client_id or not client_secret:
            ...
            return [{"name": f"Game inspired by {title}", "platforms": ["PS5", "PC"], "rating": 85.0}]
        ...
        try:
            res = safe_http_request("POST", url, headers=headers, content=query, timeout=15)
            if res.status_code == 200:
                games_data = res.json()
                results = []
                for game in games_data:
                    ...
                    if game.get('first_release_date'):
                        release_date = time.strftime('%Y-%m-%d', time.gmtime(game['first_release_date']))
...
class ScraperF_Tropes:
    ...
    def get_narrative_tropes(title: str, media_type: str = 'Anime') -> list:
        ...
        if not gemini_client:
            return [
                {"trope": "Hero's Journey", "description": "L'appel à l'aventure d'un jeune héros."},
                {"trope": "Training Arc", "description": "Entraînement intensif pour battre l'ennemi."}
            ]
...
def run_tripartite_enrichment(limit: int = 5, dry_run: bool = False):
    ...
    logger.info(f"🚀 Lancement de la Seconde Suite de Scraping (Limite: {limit}, Simulation: {dry_run})")
    ...
    logger.info(f"📊 {total_items} médias prêts à être enrichis par les 3 scrapers avancés.")
    ...
    for idx, item in enumerate(items):
        ...
        if not dry_run:
            ...
            arcs_data = {"arcs": [{"arc_name": "Mock Arc", "episode_range": "1-5"}], "fillers": []}

        # --- SCRAPER E : JEUX VIDÉO DÉRIVÉS ---
        logger.info("   🎮 [Étape 5] Recherche des adaptations de jeux vidéo (IGDB API)...")
        games_data = []
        if not dry_run:
        ...
        tropes_data = []
        if not dry_run:
            tropes_data = ScraperF_Tropes.get_narrative_tropes(item.title, item.media_type)
        else:
        ...
        if not dry_run:
            ...
            # Sync JSON
            update_json_metadata_field(json_file, item.external_id, "narrative_arcs", arcs_data)
            update_json_metadata_field(json_file, item.external_id, "video_games", games_data)
            update_json_metadata_field(json_file, item.external_id, "narrative_tropes", tropes_data)
            ...
            try:
                ...
                if neo4j_manager and neo4j_manager.driver:
                    ...
                    logger.info("   🕸️ Graphe Neo4j synchronisé pour les métadonnées avancées.")
            except Exception as e:
                logger.debug(f"Neo4j advanced sync skipped: {e}")
                
            logger.info("   ✅ Toutes les sauvegardes (SQLite, JSON, Neo4j) sont complétées.")
        else:
            logger.info(f"🧪 [Dry-Run] Arcs narratifs : {len(arcs_data.get('arcs', []))} arcs.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Exécute les 3 scrapers avancés sur la DB.")
