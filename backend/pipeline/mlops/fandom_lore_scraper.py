...
class FandomLoreScraper:
    ...
    def get_popular_franchises(self) -> List[str]:
        ...
        if MediaItem:
            try:
                ...
                items = MediaItem.objects.filter(popularity__gt=80).values_list('title', flat=True)[:15]
    ...
    def execute_pipeline(self):
        ...
        for franchise in franchises:
            ...
            if not matched_key:
                logger.info(f"⏩ Franchise '{franchise}' has no predefined Fandom wiki map. Skipping.")
                continue
            ...
            for source_url, raw_text in texts_to_process:
                ...
                clean_text = raw_text[:8000] # focus sur les 8000 premiers caractères descriptifs
                ...
                logger.info(f"⚙️ Distilled into {len(chunks)} contextual chunks for {source_url}.")
                
                if self.dry_run:
                    logger.info(f"🧪 [Dry-Run] Sample Chunk: {context_header}{chunks[0][:150]}...")
                ...
                if self.container:
                    try:
                        ...
                        logger.info(f"✅ Upserted {len(chunks)} lore chunks for {matched_key} in ChromaDB.")
                        indexed_count += len(chunks)
                    except Exception as e:
                        logger.error(f"❌ Failed to index chunks in ChromaDB: {e}")
                        
        logger.info(f"✨ Fandom Lore Pipeline finished. Processed {indexed_count} total ssemantic chunks.")

if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Acquiert le lore Fandom à grande échelle.")
