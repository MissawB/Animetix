...
TOPIC_KEYWORDS = {
    "shonen": ["combat", "bataille", "tournoi", "puissance", "pouvoir", "entraînement", "fight", "battle", "power", "training", "rival"],
    "seinen": ["mature", "psychologique", "politique", "survie", "violence", "adulte", "complex", "survival", "political"],
    "shojo": ["romance", "amour", "sentiments", "relation", "cœur", "love", "romantic", "feelings", "heart"],
    "isekai": ["isekai", "autre monde", "réincarné", "invoqué", "transported", "reincarnated", "another world"],
    "mecha": ["robot", "mecha", "pilote", "cockpit", "eva", "gundam", "armure", "armor"],
    "slice_of_life": ["quotidien", "école", "amitié", "slice of life", "daily", "school", "friendship", "lycée"],
    "horror": ["horreur", "sang", "mort", "terreur", "horror", "blood", "death", "gore", "dark"],
    "sports": ["sport", "match", "équipe", "compétition", "team", "competition", "basketball", "football", "volleyball"],
    "fantasy": ["magie", "dragon", "sort", "sorcier", "elfe", "magic", "spell", "wizard", "fantasy"],
    "sci_fi": ["espace", "futur", "technologie", "cyborg", "space", "future", "technology", "cyber", "galactique"],
    "culture_otaku": ["otaku", "cosplay", "convention", "manga", "anime", "japon", "japan", "doublage", "seiyuu", "studio"],
    "french_market": ["france", "français", "éditeur", "doubleur", "crunchyroll", "kana", "glénat", "adn", "wakanim"],
}
...
def compute_fingerprint(
    ...
    if not samples:
        return DatasetFingerprint(
            total_samples=0, avg_output_length=0, median_output_length=0,
            std_output_length=0, min_output_length=0, max_output_length=0,
            language_distribution={}, topic_distribution={},
            filtered_ratio=1.0, snapshot_timestamp=datetime.now().isoformat()
        )
    ...
    median_len = sorted_lengths[n // 2] if n % 2 == 1 else (sorted_lengths[n // 2 - 1] + sorted_lengths[n // 2]) / 2
    ...
    if dataset_type == "dpo":
        ...
        for s in samples:
            ...
            chosen = s.get("chosen", "")
            if any(kw in rejected.lower() for kw in ["désolé", "aucune idée", "sorry", "don't know", "cherche sur google"]):
                strategy_counts["refusal"] += 1
            elif any(kw in rejected.lower() for kw in ["basically", "literally", "fr fr", "actually", "évident", "triviale", "répète", "honestly", "condescend"]):
...
def analyze_drift(
    ...
    topic_distance = _cosine_distance(current.topic_distribution, baseline.topic_distribution)
    metrics["topic_cosine_distance"] = round(topic_distance, 4)
    t = DRIFT_THRESHOLDS["topic_cosine_distance"]
    if topic_distance >= t["block"]:
        blockers.append(f"Dérive thématique critique : cosine distance = {topic_distance:.3f} (seuil bloquant = {t['block']})")
    elif topic_distance >= t["warning"]:
        warnings.append(f"Dérive thématique détectée : cosine distance = {topic_distance:.3f} (seuil d'alerte = {t['warning']})")

    # 2. Length shift
    if baseline.avg_output_length > 0:
        length_shift = abs(current.avg_output_length - baseline.avg_output_length) / baseline.avg_output_length
    else:
        length_shift = 0.0
    metrics["length_shift_ratio"] = round(length_shift, 4)
    t = DRIFT_THRESHOLDS["length_shift_ratio"]
    if length_shift >= t["block"]:
        blockers.append(f"Dérive de longueur critique : shift = {length_shift:.1%} (seuil bloquant = {t['block']:.0%})")
    elif length_shift >= t["warning"]:
        warnings.append(f"Dérive de longueur détectée : shift = {length_shift:.1%} (seuil d'alerte = {t['warning']:.0%})")

    # 3. Language distribution drift (Jensen-Shannon divergence)
    lang_div = _kl_divergence(current.language_distribution, baseline.language_distribution)
    metrics["language_js_divergence"] = round(lang_div, 4)
    t = DRIFT_THRESHOLDS["language_kl_divergence"]
    if lang_div >= t["block"]:
        blockers.append(f"Dérive linguistique critique : JS divergence = {lang_div:.3f} (seuil bloquant = {t['block']})")
    elif lang_div >= t["warning"]:
        warnings.append(f"Dérive linguistique détectée : JS divergence = {lang_div:.3f} (seuil d'alerte = {t['warning']})")

    # 4. Sample count drop
    if baseline.total_samples > 0:
        count_drop = (baseline.total_samples - current.total_samples) / baseline.total_samples
    else:
        count_drop = 0.0
    ...
    if count_drop >= t["block"]:
        blockers.append(f"Perte d'échantillons critique : -{count_drop:.1%} (seuil bloquant = {t['block']:.0%})")
    elif count_drop >= t["warning"]:
        warnings.append(f"Perte d'échantillons détectée : -{count_drop:.1%} (seuil d'alerte = {t['warning']:.0%})")
    ...
    if filtered_increase >= t["block"]:
        blockers.append(f"Augmentation critique du taux de filtrage : +{filtered_increase:.1%} (seuil bloquant = {t['block']:.0%})")
    elif filtered_increase >= t["warning"]:
        warnings.append(f"Augmentation du taux de filtrage : +{filtered_increase:.1%} (seuil d'alerte = {t['warning']:.0%})")

    # Overall drift score (weighted average of normalized metrics)
    weights = {"topic": 0.35, "length": 0.20, "language": 0.20, "count": 0.15, "filtered": 0.10}
    normalized = {
        "topic": min(1.0, topic_distance / DRIFT_THRESHOLDS["topic_cosine_distance"]["block"]),
        "length": min(1.0, length_shift / DRIFT_THRESHOLDS["length_shift_ratio"]["block"]),
        "language": min(1.0, lang_div / DRIFT_THRESHOLDS["language_kl_divergence"]["block"]),
        "count": min(1.0, count_drop / DRIFT_THRESHOLDS["sample_count_drop_ratio"]["block"]),
        "filtered": min(1.0, filtered_increase / DRIFT_THRESHOLDS["filtered_ratio_increase"]["block"]),
    }
...
def run_pre_training_drift_check(
    ...
    if report.go_for_training:
        logger.info(f"✅ Drift check PASSED (score={report.overall_drift_score:.3f}). Safe to proceed with training.")
    else:
        logger.warning(f"❌ Drift check FAILED (score={report.overall_drift_score:.3f}). Training should NOT proceed.")
