...
class GraphCommunityPartitioner:
    ...
    def run_partitioning(self) -> List[Dict[str, Any]]:
        ...
        """
        logger.info("🕸️ GraphRAG: Running Louvain community detection and partition extraction...")
        
        if not self.neo4j_manager:
            logger.warning("⚠️ Neo4j Manager non disponible. Utilisation d'un partitionnement factice (mock) de haute fidélité.")
        ...
        try:
            ...
            for item in nodes:
                ...
                if lbl not in raw_groups:
                    raw_groups[lbl] = []
                raw_groups[lbl].append(f"{item.get('entity_name')} -[{item.get('relationship')}]-> {item.get('connected_entity')}")
            ...
            logger.info(f"✅ GraphRAG: Partitioned graph into {len(self.communities)} communities.")
            return self.communities
            
        except Exception as e:
            logger.error(f"❌ GraphRAG community extraction failed: {e}. Falling back to mock communities.")
            return self._generate_mock_communities()
        ...
        self.communities = [
            {
                ...
                "entities": ["Berserk", "Guts", "Glénat", "Tokyo Ghoul", "Kentaro Miura"]
            },
            {
                ...
                "entities": ["One Piece", "Luffy", "Weekly Shōnen Jump", "Hunter x Hunter", "Naruto"]
            },
            {
                ...
                "entities": ["Stéphane Excoffier", "Hiroshi Kamiya", "Christophe Lemoine", "Luffy", "Levi Ackerman", "Baggy"]
            }
        ]
        return self.communities
