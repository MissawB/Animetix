...
class OtakuKnowledgeIndexer:
    ...
    def compile_all_facts(self) -> List[Dict[str, str]]:
        ...
        for name, details in ANIME_SONGS_AND_SINGERS.items():
            ...
            facts.append(
                {"category": "Anisong Artists", "title": name, "content": text}
            )
        ...
        for name, details in SEIYUU_PROFILES.items():
            ...
            facts.append(
                {"category": "Seiyuu Profiles", "title": name, "content": text}
            )
        ...
        for name, details in CREATORS_AND_STUDIOS.items():
            ...
            facts.append(
                {"category": "Creators & Directors", "title": name, "content": text}
            )
        ...
        for name, details in FRENCH_VOICE_ACTORS.items():
            ...
            facts.append(
                {"category": "French Voice Actors (VF)", "title": name, "content": text}
            )
        ...
        for name, details in FRENCH_MANGA_PUBLISHERS.items():
            ...
            facts.append(
                {"category": "French Manga Publishers", "title": name, "content": text}
            )
        ...
        for name, details in FRENCH_ANIME_DISTRIBUTORS.items():
            ...
            facts.append(
                {
                    "category": "French Anime Distributors",
                    "title": name,
                    "content": text,
                }
            )
        ...
        for name, details in JAPANESE_MANGA_PUBLISHERS.items():
            ...
            facts.append(
                {
                    "category": "Japanese Manga Publishers",
                    "title": name,
                    "content": text,
                }
            )
        ...
        for name, details in JAPANESE_ANIME_DISTRIBUTORS.items():
            ...
            facts.append(
                {
                    "category": "Japanese Anime Distributors (JP)",
                    "title": name,
                    "content": text,
                }
            )
        ...
        for name, details in POP_CULTURE_AWARDS.items():
            ...
            facts.append(
                {"category": "Pop-Culture Awards", "title": name, "content": text}
            )
        ...
        for name, details in SERIALIZATION_MAGAZINES.items():
            ...
            facts.append(
                {"category": "Serialization Magazines", "title": name, "content": text}
            )
        ...
        for key, details in VOLUMES_AND_EPISODES_DATA.items():
            ...
            facts.append(
                {"category": "Volumes & Episodes Stats", "title": key, "content": text}
            )
        ...
        for rel in all_relations:
            if "question" in rel and "answer" in rel:
                ...
                content = (
                    f"Fait relationnel Otaku expert : {rel.get('relation_text', '')}"
                )
            facts.append(
                {
    ...
    def execute_indexation(self):
        ...
        if self.dry_run:
            ...
            logger.info(
                f"✨ Simulating finished for {len(facts)} compiled factual entries."
            )
            return

        if not self.container:
            logger.error(
                "❌ Django Container is not initialized. Cannot run indexation in production."
            )
            return
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Indexation de masse de la base Otaku unifiée."
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
