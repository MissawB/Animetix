...
try:
    ...
    logger.warning(f"Failed to load Google GenAI SDK: {e}. Translations will be simulated.")
    gemini_client = None
...
def fetch_jikan_details(mal_id: str, media_type: str = 'anime') -> dict:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.warning(f"⚠️ Rate limit Jikan pour {media_type} MAL ID {mal_id}. Pause de 10s...")
            time.sleep(10)
            return fetch_jikan_details(mal_id, media_type)
        else:
            logger.debug(f"ℹ️ Aucun détail trouvé pour {media_type} {mal_id} (Code: {response.status_code})")
            return {}
    except Exception as e:
...
def update_json_file(file_path: str, external_id: str, updates: dict):
    ...
    try:
        ...
        if updated:
            ...
            logger.debug(f"📝 JSON mis à jour pour {external_id} dans {os.path.basename(file_path)}")
    except Exception as e:
...
def run_enrichment(limit: int = 20, dry_run: bool = False):
    ...
    logger.info(f"🚀 Lancement de l'enrichisseur sémantique (Limite: {limit}, Simulation: {dry_run})")
    
    if not MediaItem:
        logger.error("❌ Django MediaItem n'est pas disponible. Impossible de procéder.")
        return
    ...
    items_to_enrich = items_to_enrich.exclude(description='').order_by('-popularity')[:limit]
    
    total_count = len(items_to_enrich)
    logger.info(f"📊 Trouvé {total_count} items éligibles pour l'enrichissement sémantique.")
    
    if total_count == 0:
        logger.info("ℹ️ Aucun élément à enrichir. La base de données est déjà complète !")
        return
    ...
    for item in items_to_enrich:
        logger.info(f"🔮 [{processed_count + 1}/{total_count}] Enrichissement de '{item.title}' (ID: {item.external_id})")
        ...
        if item.media_type in ['Anime', 'Manga'] and item.external_id.isdigit():
            ...
            if not item.metadata or 'themes' not in item.metadata or 'background' not in item.metadata:
                logger.info(f"   🕸️ Récupération des infos Jikan pour MAL ID {item.external_id}...")
        ...
        if dry_run:
            ...
            if jikan_data:
                logger.info(f"🧪 [Dry-Run] Thèmes trouvés : {jikan_data.get('themes', [])}")
            processed_count += 1
            continue
        ...
        if jikan_data:
            ...
            # Thèmes
            themes = [t.get('name') for t in jikan_data.get('themes', []) if t.get('name')]
            if themes:
            ...
            # Titres alternatifs
            titles = [t.get('title') for t in jikan_data.get('titles', []) if t.get('title')]
            if titles:
                metadata['alternative_titles'] = list(set(metadata.get('alternative_titles', []) + titles))
        ...
        item.save()
        logger.info(f"   ✅ Base SQLite mise à jour avec succès.")
        ...
        try:
            ...
            if neo4j_manager and neo4j_manager.driver:
                ...
                logger.info(f"   🕸️ Graphe Neo4j mis à jour.")
        except Exception as e:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Scrape et enrichit le catalogue avec des synopsis français et infos complexes.")
    parser.add_argument("--limit", type=int, default=10, help="Nombre max d'œuvres à enrichir.")
    parser.add_argument("--dry-run", action="store_true", help="Simule l'indexation sans écrire en base.")
