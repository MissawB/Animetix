...
def fetch_jikan_metadata(mal_id, media_type):
    ...
    metadata = {"entities": [], "relations": [], "exact_date": None}
                ...
                if any("Director" in p for p in positions):
                    ...
                    if name:
                        metadata["entities"].append(
                            {"name": name, "type": "Person", "description": "Director"}
                        )
                        metadata["relations"].append(
                            {"source": "", "target": name, "type": "DIRECTED_BY"}
                        )
        time.sleep(0.5)
    except Exception as e:
                        ...
                        metadata["entities"].append(
                            {
                                "name": name,
                                "type": "Person",
                                "description": "Seiyuu (Voice Actor)",
                            }
                        )
                        metadata["relations"].append(
                            {"source": "", "target": name, "type": "VOICED_BY"}
                        )
        time.sleep(0.5)
    except Exception as e:
...
def enrich_media_type(media_type: str, limit: int = 50):
    ...
    items_to_process = [
        it for it in data if it.get("description") and len(it["description"]) > 100
    ]
    items_to_process = items_to_process[:limit]

    logger.info(
        f"🕸️ Starting AI & API enrichment for {len(items_to_process)} {media_type} items..."
    )
    ...
    for item in tqdm(items_to_process, desc=f"Enriching {media_type}"):
        try:
            ...
            if extracted.get("entities") or extracted.get("relations"):
                ...
                if (
                    api_metadata
                    and api_metadata.get("exact_date")
                    and neo4j_manager.driver
                ):
                    with neo4j_manager.driver.session() as session:
                        session.run(
                            "MATCH (m:Media {id: $id}) SET m.exact_date = $date",
                            id=media_id,
                            date=api_metadata["exact_date"],
                        )

                logger.debug(
                    f"✅ Enriched {title}: {len(extracted.get('entities', []))} entities, {len(extracted.get('relations', []))} relations found."
                )
            else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Enrich Neo4j graph with AI and external APIs."
    )
    parser.add_argument(
        "--type", type=str, default="Anime", help="Media type (Anime/Manga)"
    )
    parser.add_argument(
        "--limit", type=int, default=10, help="Number of items to process"
    )
