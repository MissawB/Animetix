...
class ScraperI_Pilgrimage:
    ...
    def get_pilgrimage_locations(title: str, media_type: str = "Anime") -> list:
        ...
        if not gemini_client:
            return [
                {
                    "location_name": "Sanctuaire Suga",
                    "city": "Tokyo",
                    "scene_description": "Les escaliers de la rencontre finale.",
                },
                {
                    "location_name": "Gare de Shinjuku",
                    "city": "Tokyo",
                    "scene_description": "Lieu de transit récurrent.",
                },
            ]
...
def run_tripartite_enrichment(limit: int = 5, dry_run: bool = False):
    ...
    logger.info(
        f"🚀 Lancement de la Troisième Suite de Scraping (Limite: {limit}, Simulation: {dry_run})"
    )
    ...
    logger.info(
        f"📊 {total_items} médias prêts à être enrichis par les 3 scrapers experts."
    )
    ...
    for idx, item in enumerate(items):
        ...
        logger.info(
            "   📺 [Étape 7] Recherche des plateformes et diffuseurs français..."
        )
        streaming_data = []
        if not dry_run:
            streaming_data = ScraperG_Streaming.get_french_licence_and_streaming(
        ...
        logger.info(
            "   🧠 [Étape 8] Récupération des recommandations textuelles humaines (Jikan)..."
        )
        recs_data = []
        if not dry_run and item.external_id.isdigit():
            ...
            recs_data = [
                {
                    "recommended_title": "Mock Rec",
                    "mal_id": 9999,
                    "reason": "Because it is awesome.",
                }
            ]
        ...
        pilgrimage_data = []
        if not dry_run:
            pilgrimage_data = ScraperI_Pilgrimage.get_pilgrimage_locations(
                item.title, item.media_type
            )
        else:
            pilgrimage_data = [
                {
                    "location_name": "Sanctuaire Suga",
                    "city": "Tokyo",
                    "scene_description": "Scène finale.",
                }
            ]
        ...
        if not dry_run:
            ...
            update_json_metadata_field(
                json_file, item.external_id, "streaming_platforms", streaming_data
            )
            update_json_metadata_field(
                json_file, item.external_id, "human_recommendations", recs_data
            )
            update_json_metadata_field(
                json_file,
                item.external_id,
                "real_locations_pilgrimage",
                pilgrimage_data,
            )
            ...
            logger.info(
                f"🧪 [Dry-Run] Plateformes : {len(streaming_data)} identifiées."
            )
            logger.info(f"🧪 [Dry-Run] Recommandations : {len(recs_data)} argumentées.")
            logger.info(
                f"🧪 [Dry-Run] Pèlerinage : {len(pilgrimage_data)} lieux réels."
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Exécute les 3 scrapers experts sur la DB."
    )
    parser.add_argument(
