    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if match:
                ...
                logger.warning(
                    f"⚠️ No JSON found in response for {name}: {text[:100]}..."
                )
        else:
            logger.error(
                f"❌ API Error {response.status_code} for {name}: {response.text[:100]}..."
            )
...
def simulate_binary_attributes(name, description):
    ...
    if "épée" in desc or "sword" in desc:
        attrs["porte_une_epee"] = True
    if "cheveux blonds" in desc or "blonde hair" in desc:
        attrs["a_des_cheveux_blonds"] = True
    if "magie" in desc or "magic" in desc:
        attrs["utilise_la_magie"] = True
    if "lycée" in desc or "school" in desc:
        attrs["est_un_lyceen"] = True
    if "pouvoir" in desc or "power" in desc:
        attrs["possede_des_super_pouvoirs"] = True

    # Ajout d'un attribut spécifique au nom pour le test
    if "Levi" in name:
        attrs["est_le_plus_fort_de_l_humanite"] = True
...
def run_extraction():
    ...
    logger.info(
        f"🧠 Simulation/Extraction des attributs pour {len(chars_to_process)} personnages..."
    )
    ...
    for c in tqdm(chars_to_process):
        ...
        attrs = None
        if BRAIN_URL:
            attrs = extract_binary_attributes(
                c["name"], description, c.get("metadata", {})
            )
    ...
    logger.info(
        f"✅ Terminé ! {success_count} personnages mappés. Total dans la DB: {len(attr_db)}."
    )
