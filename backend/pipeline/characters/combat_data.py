...
def run_combat_data_ingestion(limit: int = 100):
    ...
    if os.path.exists(COMBAT_DATA_PATH):
        try:
            ...
            logger.error(
                "Error loading combat data from %s: %s",
                COMBAT_DATA_PATH,
                e,
                exc_info=True,
            )
    ...
    if os.path.exists(FAILED_ATTEMPTS_PATH):
        try:
            ...
            logger.error(
                "Error loading failed attempts from %s: %s",
    ...
    for char in characters:
        ...
        try:
            ...
            franchise = (
                char.get("origin")
                or char.get("franchise")
                or char.get("media", {}).get("title")
            )

            # Fetch ALL versions of the character
            combat_versions = vs_service.fetch_character_versions(
                name, franchise=franchise
            )

            if not combat_versions:
                logger.warning(
                    f"⚠️ No valid profiles found for {name}, marking as failed."
                )
            ...
            for combat_char in combat_versions:
                ...
                if (
                    not combat_char.summary
                    or combat_char.summary == "No summary available."
                ):
                    continue
    ...
    logger.info(
        f"🎯 Combat data ingestion complete. Total: {len(results)}, Failed: {len(failed_attempts)}"
    )
    return True
