...
class GoogleGenAIAdapter(InferencePort):
    ...
    def __init__(
        ...
        self.embedding_model = os.getenv("GEMINI_EMBEDDING_MODEL") or "text-embedding-004"
    ...
    def get_image_embedding(self, image_data: bytes, model_id: Optional[str] = None) -> List[float]:
        ...
        try:
            ...
            self._log_usage(engine=f"google_genai:{model_id or self.embedding_model}:vision", units=1)
            return res.embeddings[0].values
        except Exception as e:
            logger.warning(f"Gemini multimodal embedding not supported or failed: {e}. Falling back to text description embedding.")
    ...
    def generate(
        ...
        if cache_name: config_args["cached_content"] = cache_name
        ...
        try:
            ...
            return InferenceResponse(
                text=text,
                metadata=InferenceMetadata(logprobs=parsed_logprobs, usage=usage, thinking=thoughts)
            )
        except Exception as e:
    ...
    def stream_generate(
        ...
        try:
            response_stream = self.client.models.generate_content_stream(model=self.model_name, contents=prompt, config=config)
            for chunk in response_stream:
                ...
                yield InferenceResponse(text=chunk_text, metadata=InferenceMetadata(logprobs=parsed_logprobs, usage=usage, thinking=thoughts))
        except Exception as e:
    ...
    def generate_structured(self, prompt: str, response_model: type, system_prompt: str = "Tu es un expert en extraction de données structurées.", max_retries: int = 3) -> Any:
        if not self.client: raise InferenceNotImplementedError("Google GenAI client is not initialized.")
        config = types.GenerateContentConfig(system_instruction=system_prompt, response_mime_type="application/json", response_schema=response_model, temperature=0.1)
        for attempt in range(max_retries):
            try:
                response = self.client.models.generate_content(model=self.model_name, contents=prompt, config=config)
                if hasattr(response, "parsed") and response.parsed is not None: return response.parsed
                text = response.text
                if text:
                    match = re.search(r'\{.*\}', text, re.DOTALL)
                    if match:
                        data = json.loads(match.group(0))
                        if hasattr(response_model, "model_validate"): return response_model.model_validate(data)
                        return data
            except Exception as e:
                logger.error(f"Structured Generation failed (Attempt {attempt+1}/{max_retries}): {e}")
                if attempt == max_retries - 1: raise InferenceError(f"GoogleGenAI generate_structured failed: {e}")
                time.sleep(1)
    ...
    def generate_image_description(self, image_data: bytes, prompt: str = "Décris cette image d'anime de manière très détaillée.") -> str:
        ...
        image_part = types.Part.from_bytes(data=image_data, mime_type=mime)
        try:
            response = self.client.models.generate_content(model=self.model_name, contents=[image_part, prompt])
            text = response.text or ""
            usage = self._get_usage_dict(response, len(prompt), len(text))
            self._log_usage(engine=f"google_genai:{self.model_name}:vision", input_tokens=usage.get("prompt_tokens", 0) + 258, output_tokens=usage.get("completion_tokens", 0))
            return text
        except Exception as e:
    ...
    def detect_objects(self, image_data: bytes, candidate_queries: List[str], model_id: Optional[str] = None) -> List[Dict]:
        ...
        try:
            ...
            if match:
                ...
                for obj in objects:
                    if 'box_2d' in obj: obj['box'] = obj.pop('box_2d')
                return objects
        except Exception as e:
    ...
    def visual_rerank(self, query: str, image_urls: List[str], system_prompt: str = "Tu es un expert en analyse visuelle d'anime.") -> List[Dict[str, Any]]:
        if not self.client or not image_urls: return []
        parts = []
        downloaded_indices = []
        ...
        if not downloaded_indices: return [{"index": i, "score": 0.0} for i in range(len(image_urls))]
        ...
        try:
            response = self.client.models.generate_content(model=self.model_name, contents=parts, config=types.GenerateContentConfig(system_instruction=system_prompt, response_mime_type="application/json", temperature=0.1))
            ...
            if match:
                ...
                for item in scores:
                    idx = item.get("index")
                    if idx is not None: final_results.append({"index": int(idx), "score": float(item.get("score", 0.0))})
                if final_results: return final_results
        except Exception as e:
    ...
    def rerank_documents(self, query: str, documents: List[str]) -> List[float]:
        ...
        for i, doc in enumerate(documents): prompt += f"ID {i} : {doc[:1000]}\n"
        prompt += "\nPour chaque document, donne un score de pertinence entre 0.0 et 1.0. Réponds avec une liste JSON : [score0, score1, ...]"
        try:
            response = self.generate(prompt, system_prompt="Tu es un système de réordonnancement (reranker) expert.")
            match = re.search(r'\[.*\]', response.text)
            if match:
                scores = json.loads(match.group(0))
                if len(scores) == len(documents): return [float(s) for s in scores]
        except Exception as e:
    ...
    def generate_image(self, prompt: str, style: str = "") -> str:
        ...
        if not self.use_vertexai:
            logger.warning("generate_image via GoogleGenAI requires Vertex AI backend. Falling back to InferenceNotImplementedError.")
        ...
        try:
            ...
            res = self.client.models.generate_image(
                ...
                config=types.GenerateImageConfig(number_of_images=1, include_rai_reason=True)
            )
    ...
    def _get_or_create_cache(self, system_prompt: str) -> Optional[str]:
        if not self.client or not system_prompt or len(system_prompt) < self.cache_threshold_chars: return None
        if "gemini" not in self.model_name.lower(): return None
        import hashlib
        ...
        try:
            ...
            cache = self.client.caches.create(model=self.model_name, config=types.CreateCachedContentConfig(display_name=f"animetix_ctx_{context_hash[:8]}", system_instruction=system_prompt, ttl=ttl_str))
            self._context_caches[context_hash] = (cache.name, current_time + self.cache_ttl_seconds)
            return cache.name
        except Exception as e:
    ...
    def _parse_logprobs(self, candidate: Any) -> Optional[List[TokenLogProb]]:
        if not hasattr(candidate, "logprobs_result") or not candidate.logprobs_result: return None
        parsed_logprobs = []
            ...
            if idx < len(top_candidates):
                ...
                if isinstance(candidates_list, list):
                    for alt in candidates_list:
                        top_list.append({getattr(alt, "token", ""): getattr(alt, "log_probability", 0.0)})
            parsed_logprobs.append(TokenLogProb(token=getattr(chosen, "token", ""), logprob=getattr(chosen, "log_probability", 0.0), top_logprobs=top_list if top_list else None))
        return parsed_logprobs

    def _extract_thoughts(self, response: Any) -> Optional[str]:
        if not response.candidates or not response.candidates[0].content: return None
        thoughts = []
        for part in response.candidates[0].content.parts:
            if getattr(part, "thought", False):
                text_val = getattr(part, "text", "")
                if text_val: thoughts.append(text_val)
    ...
    def _get_usage_dict(self, response: Any, default_prompt_len: int = 0, default_text_len: int = 0) -> Dict[str, int]:
        ...
        return {"prompt_tokens": default_prompt_len // 4, "completion_tokens": default_text_len // 4, "total_tokens": (default_prompt_len + default_text_len) // 4}
