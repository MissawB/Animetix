...
class ImageGenMixin:
    ...
    def transform_image_to_anime(self, image_data: bytes, studio_style: str = "", prompt: str = "") -> str:
        ...
        try:
            ...
            logger.error(f"❌ Img2Anime failed: {e}"); return ""
    ...
    def transform_video_to_anime(self, video_data: bytes, studio_style: str = "", prompt: str = "") -> str:
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(delete=False, suffix=".mp4") as tmp_out:
                ...
                for img in styled_images: writer.append_data(np.array(img))
                writer.close()
                with open(tmp_out.name, "rb") as f: res_base64 = base64.b64encode(f.read()).decode("utf-8")
            os.unlink(tmp_out.name)
            self._log_usage(engine=f"diffusers:{model_id}:fatezero_lite", units=len(input_frames))
            return f"data:video/mp4;base64,{res_base64}"
        except Exception as e:
            logger.error(f"❌ Video2Anime failed: {e}"); return ""
    ...
    def inpaint_text_bubbles(self, image_data: bytes, bubbles: List[Dict]) -> str:
        ...
        try:
            ...
            if hasattr(self, '_inpaint_pipe') and self._inpaint_pipe:
                ...
                inpainted = init_image.copy(); d = PILImageDraw.Draw(inpainted)
                for b in bubbles:
                    bbox = b.get('bbox')
                    if bbox: d.rectangle(bbox, fill="white")
            d_final = PILImageDraw.Draw(inpainted)
            try: font = PILImageFont.truetype("arial.ttf", 20)
            except: font = PILImageFont.load_default()
            for b in bubbles:
                ...
                if bbox and text:
                    ...
                    d_final.text(((x1 + x2) / 2 - t_w / 2, (y1 + y2) / 2 - t_h / 2), text, fill="black", font=font)
            buf = BytesIO(); inpainted.save(buf, format="JPEG", quality=85)
            self._log_usage(engine="vision:inpaint", units=1)
            return f"data:image/jpeg;base64,{base64.b64encode(buf.getvalue()).decode('utf-8')}"
        except Exception as e:
            logger.error(f"❌ Inpaint failed: {e}"); raise InferenceError(f"Inpainting failed: {e}")
