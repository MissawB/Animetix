...
class BrainAPIAdapter(InferencePort):
    ...
    """
    def __init__(self, api_url: str = None, api_key: str = None, usage_port: Optional[UsagePort] = None):
    ...
    def generate_sprite(self, prompt: str, style: str = "") -> str:
        ...
        try:
            ...
            raise InferenceNotImplementedError(f"BrainAPI Sprite Generation failed: {e}")
    ...
    def classify_image(self, image_data: bytes, candidate_labels: List[str], model_id: Optional[str] = None) -> Dict[str, float]:
        try:
            ...
            response = safe_http_request(
                ...
                json={"image": img_b64, "candidate_labels": candidate_labels, "model_id": model_id},
                headers=self._get_headers()
            )
    ...
    def detect_objects(self, image_data: bytes, candidate_queries: List[str], model_id: Optional[str] = None) -> List[Dict]:
        try:
            ...
            response = safe_http_request(
                ...
                json={"image": img_b64, "candidate_labels": candidate_queries, "model_id": model_id},
                headers=self._get_headers()
            )
    ...
    def visual_rerank(self, query: str, image_urls: List[str], system_prompt: str = "") -> List[Dict[str, Any]]:
        try:
            response = safe_http_request(
                ...
                json={"query": query, "image_urls": image_urls, "system_prompt": system_prompt},
                headers=self._get_headers()
            )
    ...
    def generate_structured(self, prompt: str, response_model: type, system_prompt: str = "Tu es un expert en extraction de données structurées.", max_retries: int = 3) -> Any:
        ...
        try:
            ...
            return super().generate_structured(prompt, response_model, system_prompt, max_retries)
        except Exception as e:
