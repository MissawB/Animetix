...
class FandomAdapter(FandomPort):
    ...
    def fetch_character_data(
        ...
        clean_name = (
            character_name.replace(" profile VS Battles Wiki", "")
            .replace(" VS Battles Wiki", "")
            .strip()
        )

        query = (
            f"{clean_name} {franchise} VS Battles Wiki"
            if franchise
            else f"{clean_name} profile VS Battles Wiki"
        )
        ...
        try:
            res = safe_http_request(
                "GET", self.api_url, params=search_params, timeout=10
            )
            ...
            for result in search_results:
                ...
                res = safe_http_request(
                    "GET", self.api_url, params=fetch_params, timeout=10
                )
                res.raise_for_status()
                data = res.json()
                ...
                if pages:
                    page = pages[0]
                    categories = [
                        cl.get("title", "") for cl in page.get("categories", [])
                    ]
