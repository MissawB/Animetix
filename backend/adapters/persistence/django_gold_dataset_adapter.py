...
class DjangoGoldDatasetAdapter(GoldDatasetPort):
    ...
    def sync_positive_feedback(self) -> int:
        ...
        positive_feedbacks = AIFeedback.objects.filter(is_positive=True).exclude(
            golddatasetentry__isnull=False
        )
        count = 0
    ...
    def get_unprocessed_validated_entries(self) -> List[Dict[str, Any]]:
        ...
        entries = GoldDatasetEntry.objects.filter(is_validated=True).order_by(
            "created_at"
        )
        return [self._to_dict(e) for e in entries]
    ...
    def get_unvalidated_entries_by_type(self, entry_type: str) -> List[Dict[str, Any]]:
        ...
        entries = GoldDatasetEntry.objects.filter(
            entry_type=entry_type, is_validated=False
        ).order_by("created_at")
        return [self._to_dict(e) for e in entries]
