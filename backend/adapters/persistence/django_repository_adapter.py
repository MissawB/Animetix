...
def is_alloydb_nl_query_supported() -> bool:
    ...
    try:
        ...
        logger.warning(
            f"[AlloyDB AI] Failed to probe alloydb_ai_nl.get_sql function: {e}"
        )
        _alloydb_nl_supported = False
    return _alloydb_nl_supported
    ...
    def query_data_natural_language(
        ...
        if nl_query_active and is_alloydb_nl_query_supported():
            try:
                ...
                logger.error(
                    f"[AlloyDB AI] Text-to-SQL error using native function: {e}"
                )
        ...
        if not generated_sql:
            if not llm_service:
                logger.error(
                    "Text-to-SQL Fallback: No LLM service provided to generate query."
                )
                return []
...
"""
            try:
                generated_sql = llm_service.generate(
                    prompt,
                    system_prompt="You are a SQL generator. Output only raw SQL.",
                )
                if generated_sql:
        ...
        try:
            ...
            logger.error(
                f"Text-to-SQL: Database execution failed for query '{generated_sql}': {e}"
            )
            return []
