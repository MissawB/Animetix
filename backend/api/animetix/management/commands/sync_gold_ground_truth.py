...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('🚀 Starting Gold Set Ground Truth Synchronization...'))
        ...
        if res.get("status") == "error":
            self.stdout.write(self.style.ERROR(f"❌ Synchronization failed: {res.get('reason')}"))
            return
            
        updated_count = res.get("updated_count", 0)
        self.stdout.write(self.style.SUCCESS(f"🏁 Synchronization finished. Updated {updated_count} entries."))
        
        for detail in res.get("details", []):
            self.stdout.write(f"  📝 Query: {detail['query']}")
            self.stdout.write(self.style.WARNING(f"    - Drift reason: {detail['reasoning']}"))
