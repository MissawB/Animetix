...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        dbt_project_dir = os.path.join(os.path.dirname(base_dir), 'pipeline', 'dbt_project')
        ...
        if res_rel.returncode != 0:
            ...
            raise CommandError("[FAIL] Relational SQL database data quality tests failed.")
        else:
            self.stdout.write(self.style.SUCCESS("[SUCCESS] Relational database data quality checks passed."))
        ...
        if not options['exclude_bigquery']:
            ...
            cmd_bq = [
                ...
                '--profiles-dir', dbt_project_dir,
                '--target', 'bigquery',
                '--select', 'source:telemetry_source archetype_drift_values_check'
            ]
            ...
            if res_bq.returncode != 0:
                ...
                raise CommandError("[FAIL] BigQuery Telemetry database data quality tests failed.")
            else:
                self.stdout.write(self.style.SUCCESS("[SUCCESS] BigQuery Telemetry database data quality checks passed."))
        else:
