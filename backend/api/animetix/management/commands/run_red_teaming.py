...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--limit", type=int, default=5, help="Nombre d'œuvres à tester"
        )
    ...
    def handle(self, *args, **options):
        ...
        for item in sample_items:
            ...
            for q in queries:
                ...
                eval_res = red_teamer.evaluate_vulnerability(
                    query=q, response=response, ground_truth=item["description"]
                )
                ...
                if eval_res["is_vulnerable"]:
                    self.stdout.write(
                        self.style.WARNING(
                            f"  ⚠️ Vulnerability Found: {eval_res['analysis']}"
                        )
                    )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"🏁 Red-Teaming finished. Found {total_vulns} vulnerabilities."
            )
        )
