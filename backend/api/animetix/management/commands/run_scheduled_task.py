...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('task_key', type=str, help='The identifier of the scheduled task to run.')
    ...
    def handle(self, *args, **options):
        ...
        from animetix.tasks.pipeline_tasks import (
        ...
        if task_key not in task_mapping:
            ...
            raise CommandError(f"Unknown task key '{task_key}'. Valid options are: {valid_keys}")

        self.stdout.write(f"Executing task '{task_key}'...")
        task_func = task_mapping[task_key]
        try:
            result = task_func()
            self.stdout.write(self.style.SUCCESS(f"Task '{task_key}' completed successfully. Result: {result}"))
        except Exception as e:
            self.stderr.write(self.style.ERROR(f"Task '{task_key}' failed with error: {e}"))
