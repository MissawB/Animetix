    ...
    def post(self, request):
        ...
        try:
            ...
            guard_output = guardrail_service.validate_output(
                response_text, context=context_url, query=user_message
            )
            ...
            if not guard_output.get("is_safe", True):
                if guard_output.get("action") == "mask":
                    ...
                    return Response(
                        {
                            "error": guard_output.get(
                                "reason", "Output blocked by security filters."
                            )
                        },
                        status=status.HTTP_400_BAD_REQUEST,
                    )
            ...
            return Response({"response": response_text, "history": history})
