...
class ForgeVNView(APIView):
    ...
    def get(self, request, fusion_id):
        ...
        try:
            ...
            if (
                not fusion.is_public
                and fusion.creator != request.user
                and not getattr(request.user, "is_staff", False)
            ):
                return Response(
                    {"error": "Unauthorized access to private fusion"},
    ...
    def post(self, request, fusion_id):
        ...
        try:
            ...
            if action == "generate":
                ...
                check_text = (
                    f"{fusion.title_a} x {fusion.title_b}\n{fusion.scenario_text}"
                )
                ...
                if not guard_input.get("is_safe", True):
                    return Response(
                        {"error": "Provided script rejected by security filters."},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
