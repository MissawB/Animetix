...
class AdminFinancialsAPIView(APIView):
    ...
    def get(self, request):
        ...
        gold_sponsors = sum(1 for p in Profile.objects.all() if isinstance(p.unlocked_badges, list) and "Sponsor Or" in p.unlocked_badges)
...
class AdminEconomicAuditAPIView(APIView):
    ...
    def get(self, request):
        ...
        # 1. Masse Monétaire
        total_circulation = Profile.objects.aggregate(Sum('wallet_balance'))['wallet_balance__sum'] or 0
        avg_balance = Profile.objects.aggregate(Avg('wallet_balance'))['wallet_balance__avg'] or 0
        max_balance = Profile.objects.aggregate(Max('wallet_balance'))['wallet_balance__max'] or 0
        
        # 2. Flux (24h)
        today = timezone.now() - timedelta(hours=24)
        minted_24h = WalletTransaction.objects.filter(created_at__gte=today, amount__gt=0).aggregate(Sum('amount'))['amount__sum'] or 0
        burned_24h = abs(WalletTransaction.objects.filter(created_at__gte=today, amount__lt=0).aggregate(Sum('amount'))['amount__sum'] or 0)
        
        # 3. Répartition par source (Total historique)
        stats_by_type = WalletTransaction.objects.values('transaction_type').annotate(total=Sum('amount'))
        repartition = {item['transaction_type']: item['total'] for item in stats_by_type}
        
        # 4. Inflation Index (Ratio Mint/Burn sur 7 jours)
        week_ago = timezone.now() - timedelta(days=7)
        minted_7d = WalletTransaction.objects.filter(created_at__gte=week_ago, amount__gt=0).aggregate(Sum('amount'))['amount__sum'] or 0
        burned_7d = abs(WalletTransaction.objects.filter(created_at__gte=week_ago, amount__lt=0).aggregate(Sum('amount'))['amount__sum'] or 0)
