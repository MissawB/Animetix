...
class VisionGameStateView(CpuGameAPIView):
    @inject
...
class VisionGameStartView(CpuGameAPIView):
    @inject
    def post(
        self,
        request,
    ...
    def post(
        ...
        try:
            ...
            score = vision_quest_service.calculate_score(
                query, state.secret_id, state.secret_title, state.media_type
            )
