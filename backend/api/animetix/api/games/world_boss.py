...
from ...serializers import GlobalBossSerializer, BossParticipationSerializer
...
class ActiveWorldBossView(views.APIView):
    ...
    def get(self, request):
        boss = GlobalBoss.objects.filter(is_active=True, end_date__gt=timezone.now()).first()
        if not boss:
            return response.Response({"detail": "No active World Boss at the moment."}, status=status.HTTP_404_NOT_FOUND)
...
class WorldBossAttackView(views.APIView):
    ...
    def post(self, request):
        ...
        if not boss_id or not guess:
            return response.Response({"detail": "boss_id and guess are required."}, status=status.HTTP_400_BAD_REQUEST)
            
        try:
            boss = GlobalBoss.objects.get(id=boss_id, is_active=True)
        except GlobalBoss.DoesNotExist:
            return response.Response({"detail": "Boss not found or inactive."}, status=status.HTTP_404_NOT_FOUND)
        ...
        if is_correct:
            ...
            participation, created = BossParticipation.objects.get_or_create(user=request.user, boss=boss)
