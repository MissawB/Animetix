...
class CreateDuelRoomView(views.APIView):
    ...
    def post(self, request, catalog_service = Provide[Container.core.catalog_service]):
        ...
        if not data:
            return response.Response({"detail": "Invalid media type."}, status=status.HTTP_400_BAD_REQUEST)
...
class JoinDuelRoomView(views.APIView):
    ...
    def post(self, request):
        ...
        if not room_code:
            return response.Response({"detail": "room_code is required."}, status=status.HTTP_400_BAD_REQUEST)
            
        try:
            room = DuelRoom.objects.get(room_code=room_code, is_finished=False)
        except DuelRoom.DoesNotExist:
            return response.Response({"detail": "Room not found or already finished."}, status=status.HTTP_404_NOT_FOUND)
            
        if room.player1 == request.user:
            return response.Response({"detail": "You are already in this room."}, status=status.HTTP_200_OK)
            
        if room.player2 and room.player2 != request.user:
            return response.Response({"detail": "Room is full."}, status=status.HTTP_400_BAD_REQUEST)
...
class MatchmakingView(views.APIView):
    ...
    def post(self, request):
        ...
        if room:
            room.player2 = request.user
            room.save()
            return response.Response({"room_code": room.room_code, "media_type": room.media_type})
