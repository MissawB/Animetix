...
from animetix.models import Profile, User
...
class DeveloperRAGView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            events = list(agent.plan_and_solve_stream(query, media_type, user_id=user_id))
        ...
        if event_type == "checkout.session.completed":
            ...
            if status in ("canceled", "unpaid") or event_type == "customer.subscription.deleted":
                try:
                    ...
                    profile.save()
                    logger.info(f"User {profile.user.username} subscription canceled/unpaid. Tier downgraded to free.")
                except Profile.DoesNotExist:
                    logger.error(f"Profile not found for customer_id={customer_id} on subscription cancel.")
