...
class DuelConsumer(BaseConsumer):
    ...
    async def handle_guess(self, guess):
        ...
        data = await sync_to_async(container.core.catalog_service.load_data)(duel.media_type)
        secret = duel.secret_title
        
        # Vérification match direct
        secret_item = data['title_to_full_data'].get(secret)
        is_correct = await sync_to_async(container.core.game_service.check_title_match)(guess, secret_item)
        ...
        if is_correct:
            ...
            raw_sim = await sync_to_async(container.core.game_service.calculate_raw_similarity)(
