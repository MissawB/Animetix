...
def process_client_audio(audio_bytes: bytes) -> bytes:
    ...
    try:
        ...
        with wave.open(io.BytesIO(audio_bytes), 'rb') as wav_file:
            ...
            if params.framerate == 16000 and params.nchannels == 1 and params.sampwidth == 2:
                return raw_frames
    ...
    try:
        ...
        raise ValueError(f"Audio conversion failed: ffmpeg might be required for compressed formats. Details: {e}")
...
class SpeechToSpeechLiveConsumer(AsyncWebsocketConsumer):
    ...
    async def run_gemini_session(self):
        ...
        try:
            if not api_key:
                logger.info("Initializing Google GenAI client with Vertex AI backend for Live API.")
                ...
                logger.info("Initializing Google GenAI client with API Key for Live API.")
            ...
            async with self.gemini_client.aio.live.connect(model=model, config=config) as session:
                ...
                async for response in session.receive():
                    ...
                    if server_content:
                        ...
                        if server_content.output_transcription:
                            await self.send(json.dumps({
                                "type": "text_chunk",
                                "text": str(server_content.output_transcription)
                            }))
