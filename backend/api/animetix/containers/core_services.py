...
class CoreServicesContainer(containers.DeclarativeContainer):
    ...
    reasoning_agent = providers.Singleton(
        LazyClass("core.domain.services.reasoning_agent_service", "ReasoningAgentService"),
    ...
    sync_service = providers.Singleton(
        ...
        sync_adapter=providers.Factory(
            LazyClass("adapters.persistence.pipeline_sync_adapter", "PipelineSyncAdapter"),
    ...
    vision_service = providers.Singleton(
        LazyClass("core.domain.services.advanced_vision_service", "AdvancedVisionService"),
        inference_engine=inference.inference_engine
    )

    graph_builder = providers.Singleton(
        LazyClass("core.domain.services.graph_construction_service", "KnowledgeGraphConstructionService"),
        inference_engine=inference.inference_engine,
        prompt_manager=infrastructure.prompt_manager
    )
    ...
    vision_quest_service = providers.Singleton(
        LazyClass("core.domain.services.vision_quest_service", "VisionQuestDomainService"),
    ...
    neuro_symbolic_service = providers.Singleton(
        LazyClass("core.domain.services.neuro_symbolic_service", "NeuroSymbolicService"),
        inference_engine=inference.inference_engine,
        prompt_manager=infrastructure.prompt_manager
    )
    ...
    achievement_service = providers.Singleton(
        LazyClass("core.domain.services.achievement_service", "AchievementDomainService"),
        port=providers.Factory(LazyClass("adapters.persistence.django_achievement_adapter", "DjangoAchievementAdapter")),
    ...
    game_service = providers.Singleton(
        ...
        repository=persistence.repository,
        catalog_service=catalog_service,
        similarity_service=providers.Factory(LazyClass("core.domain.services.similarity_service", "SimilarityService"), repository=persistence.repository),
    ...
    visual_novel_service = providers.Singleton(
        LazyClass("core.domain.services.creative.visual_novel_service", "VisualNovelService"),
        llm_service=agentic.llm_service,
        repository=persistence.repository
    )

    studio_transform_service = providers.Singleton(
        LazyClass("core.domain.services.creative.studio_transform", "StudioTransformService"),
    ...
    soundscape_service = providers.Singleton(
        LazyClass("core.domain.services.creative.soundscape", "SoundscapeGenerationService"),
        inference_engine=inference.inference_engine,
    ...
    orchestrator_agent_service = providers.Singleton(
        LazyClass("core.domain.services.orchestrator_agent_service", "OrchestratorAgentService"),
        llm_service=agentic.llm_service,
        prompt_manager=infrastructure.prompt_manager
    )

    cross_modal_search_service = providers.Singleton(
        LazyClass("core.domain.services.cross_modal_service", "CrossModalSearchService"),
        repository=persistence.repository
    )
    ...
    spatial_computing_service = providers.Singleton(
        LazyClass("core.domain.services.spatial_computing_service", "SpatialComputingService"),
        vision_service=vision_service
    )

    static_diorama_3d_service = providers.Singleton(
        LazyClass("core.domain.services.static_diorama_3d_service", "StaticDiorama3DService"),
        vision_service=vision_service
    )

    cinematic_volumetric_reconstruction_service = providers.Singleton(
        LazyClass("core.domain.services.cinematic_volumetric_reconstruction_service", "CinematicVolumetricReconstructionService"),
        vision_service=vision_service
    )


    liquid_neural_network = providers.Singleton(
        LazyClass("core.domain.services.neuromorphic_lnn_service", "LiquidNeuralNetworkService"),
        state_dimension=4,
        input_dimension=2
    )
    ...
    native_speech_llm_service = providers.Singleton(
        LazyClass("core.domain.services.spatial_audio_service", "NativeSpeechLLMService"),
        inference_engine=inference.inference_engine
    )
    ...
    tree_of_thoughts_service = providers.Singleton(
        LazyClass("core.domain.services.tree_of_thoughts_service", "TreeOfThoughtsSearchService"),
        inference_engine=inference.inference_engine,
        prompt_manager=infrastructure.prompt_manager
    )

    episodic_memory_compressor = providers.Singleton(
        LazyClass("core.domain.services.episodic_memory_compressor", "EpisodicMemoryCompressor"),
    ...
    neuro_symbolic_user_profiler = providers.Singleton(
        LazyClass("core.domain.services.neuro_symbolic_user_profiler", "NeuroSymbolicUserProfiler"),
    ...
    counterfactual_simulator = providers.Singleton(
        LazyClass("core.domain.services.counterfactual_simulator", "CounterfactualConversationSimulator"),
        inference_engine=inference.inference_engine
    )
    ...
    autonomous_domain_synthesizer = providers.Singleton(
        LazyClass("core.domain.services.domain_synthesizer", "AutonomousDomainSynthesizer"),
        inference_engine=inference.inference_engine,
        neo4j_manager=persistence.graph_persistence_port,
        gold_dataset_port=persistence.gold_dataset_adapter
    )
    ...
    model_distillation_pipeline = providers.Singleton(
        LazyClass("core.domain.services.distillation_pipeline", "ModelDistillationPipeline"),
    ...
    synthetic_promotion_service = providers.Singleton(
        LazyClass("core.domain.services.synthetic_promotion_service", "SyntheticDataPromotionService"),
        gold_dataset_port=persistence.gold_dataset_adapter,
    ...
    archetype_drift_service = providers.Singleton(
        LazyClass("core.domain.services.archetype_drift_service", "ArchetypeDriftService"),
    ...
    fusion_service = providers.Singleton(
        LazyClass("core.domain.services.creative.fusion_service", "FusionDomainService"),
        llm_service=agentic.llm_service
    )
    ...
    health_dashboard_service = providers.Singleton(
        LazyClass("core.domain.services.health_dashboard_service", "HealthDashboardService"),
        usage_port=infrastructure.usage_port,
        sota_service=sota_benchmark_service
    )

    quantum_cognitive_model = providers.Singleton(
        LazyClass("core.domain.services.quantum_cognitive_service", "QuantumCognitiveService"),
        dimension=4
    )

    synaptic_plasticity_simulator = providers.Singleton(
        LazyClass("core.domain.services.neuromorphic_plasticity_service", "SynapticPlasticityService"),
        num_concepts=10,
    ...
    hierarchical_graph_rag_service = providers.Singleton(
        LazyClass("core.domain.services.hierarchical_graph_rag", "HierarchicalGraphRAGService"),
        neo4j_manager=persistence.graph_persistence_port,
        llm_service=agentic.llm_service
    )
