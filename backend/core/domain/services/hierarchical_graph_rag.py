...
class HierarchicalGraphRAGService:
    ...
    def ensure_communities_loaded(self):
        ...
        try:
            ...
            logger.error(
                f"Error initializing partitioner in HierarchicalGraphRAGService: {e}"
            )

    def retrieve_community_contexts(
        ...
        try:
            matched_communities = self.partitioner.search_communities(
                query, limit=limit
            )
    ...
    def enrich_prompt_with_graphrag(self, query: str, base_context: str) -> str:
        ...
        for idx, comm in enumerate(communities):
            ...
            graphrag_context += (
                f"Entités clés : {', '.join(comm.get('entities', []))}\n"
            )
