...
class CompanionService:
    ...
    def generate_response(self, mentor_id: str, user_msg: str, context: str = "", history: Optional[List[dict]] = None) -> str:
        """
        Generates a response from a specific mentor.
        """
        if mentor_id not in self.MENTORS:
            raise ValueError(f"Unknown mentor ID: {mentor_id}. Available: {list(self.MENTORS.keys())}")
