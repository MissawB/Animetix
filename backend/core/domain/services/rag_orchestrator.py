...
class RAGOrchestrator:
    ...
    def community_partitioner(self):
        return self.processors[RAGState.GRAPH_EXPLORE].community_partitioner if RAGState.GRAPH_EXPLORE in self.processors else None
    ...
    def run_workflow(self, ctx: RAGContext, xai_collector=None):
        while ctx.current_state not in [RAGState.FINALIZE, RAGState.FAILED] and ctx.iteration < ctx.max_iterations:
            ...
            yield StreamStep(type="thought", content=f"[State Machine] État: {ctx.current_state}").model_dump()
            ...
            try:
                ...
                if ctx.current_state == RAGState.FALLBACK_RAG:
                    ...
                    yield StreamStep(type="thought", content=f"[Recovery] Échec critique du fallback : {str(e)}").model_dump()
                else:
                    yield StreamStep(type="thought", content=f"[Recovery] Erreur détectée dans l'état {ctx.current_state} : {str(e)}. Basculement en mode Fallback...").model_dump()
