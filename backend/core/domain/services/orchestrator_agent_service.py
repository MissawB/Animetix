...
class OrchestratorAgentService:
    ...
    def __init__(
        ...
        self.factory = (
            services_factory  # Accès aux autres services (RAG, Graph, Vision)
        )
    ...
    async def execute_workflow(self, query: str, media_type: str) -> str:
        ...
        if self.message_bus:
            await self.message_bus.publish_binary_message(
                "Orchestrator", "System", "WORKFLOW_START", {"query": query}
            )
    ...
    def _node_planner(self, state: State):
        ...
        prompt, system = self.prompt_manager.get_prompt(
            "orchestrator_planner", query=state.query
        )
    ...
    def _node_verifier(self, state: State):
        ...
        prompt, system = self.prompt_manager.get_prompt(
            "orchestrator_reliability", context=state.context[:500]
        )
        is_reliable = (
            "OUI"
            in self.inference_engine.generate(prompt, system_prompt=system).upper()
        )
    ...
    def _node_writer(self, state: State):
        ...
        prompt, system = self.prompt_manager.get_prompt(
            "orchestrator_final", query=state.query, context=state.context
        )
        state.final_answer = self.inference_engine.generate(
            prompt, system_prompt=system
        )

        # --- Uncertainty Quantification (XAI) ---
        confidence = self.factory.uncertainty_service.measure_confidence(
            prompt, state.final_answer
        )
        logger.info(f"📊 Confidence Score: {confidence['confidence_score']:.2f}")

        if not confidence["is_reliable"]:
            logger.warning(
                "⚠️ Confidence low. Redirecting to WEB search for external verification."
            )
