...
class MultiAgentBus:
    ...
    def __init__(self, redis_url: Optional[str] = None):
        ...
        if self.redis_url:
            try:
                ...
                logger.info(
                    "Connected to Async Redis for Shared Memory communication.",
                    extra={"bus_event": {"type": "system", "status": "connected"}},
                )
            except ImportError:
                logger.warning(
                    "redis.asyncio not found. Falling back to in-memory shared store.",
                    extra={"bus_event": {"type": "system", "status": "fallback"}},
                )
            except Exception as e:
                logger.error(
                    f"Failed to connect to Redis: {e}",
                    extra={"bus_event": {"type": "system", "status": "error"}},
                )
    ...
    async def _listen_loop(self, agent_id: str, callback: Callable):
        ...
        logger.debug(
            f"Agent listening on channel '{channel}'",
            extra={"bus_event": {"type": "listen", "agent_id": agent_id}},
        )
            ...
            while True:
                ...
                if message and message["type"] == "message":
                    ...
                    try:
                        ...
                        logger.error(
                            f"Callback error for agent: {callback_err}",
                            extra={
                                "bus_event": {
                                    "type": "callback_error",
                                    "agent_id": agent_id,
    ...
    def register_agent(self, agent_id: str, callback: Callable):
        ...
        if self._redis_client:
            ...
            try:
                ...
                logger.warning(
                    f"Could not register agent {agent_id} in background: no running event loop. {e}"
                )

        logger.info(
            f"Agent '{agent_id}' registered on MultiAgentBus.",
            extra={"bus_event": {"type": "registration", "agent_id": agent_id}},
        )

    async def publish_binary_message(
        self, sender_id: str, target_id: str, action: str, payload: Dict[str, Any]
    ) -> str:
        """
    ...
    async def close(self):
        ...
        logger.info(
            "MultiAgentBus shutdown complete.",
            extra={"bus_event": {"type": "system", "status": "shutdown"}},
        )
