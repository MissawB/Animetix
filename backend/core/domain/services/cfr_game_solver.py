...
class CFRGameSolver:
    ...
    def solve_with_history(
        self, questions: List[str], iterations: int = 100
    ) -> Dict[str, Any]:
        """
    ...
    def solve_best_question(
        self, questions: List[str], game_state: Dict[str, Any]
    ) -> Tuple[str, float]:
        """
        ...
        """
        logger.info(
            f"⚖️ CFR: Solving best question path among {len(questions)} options..."
        )
        ...
        best_question = (
            questions[best_action_idx]
            if best_action_idx < len(questions)
            else questions[0]
        )
        confidence = float(avg_strategy[best_action_idx])

        logger.info(
            f"🏆 CFR Choice: Chosen '{best_question}' (Nash strategy confidence: {confidence:.2f})"
        )
