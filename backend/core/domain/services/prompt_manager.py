...
class PromptManager:
    ...
    def add_few_shot_correction(
        self, prompt_key: str, bad_input: str, good_output: str
    ):
        ...
        try:
            with lock:
                ...
                logger.info(
                    f"🧠 Metacognition: New self-correction saved for {prompt_key} (Pending Verification)."
                )
        except Timeout:
    ...
    def update_system_prompt(self, key: str, new_system_prompt: str):
        ...
        try:
            with lock:
                ...
                logger.info(
                    f"🚀 Prompt '{key}' optimized and saved to {overrides_path}"
                )
        except Timeout:
    ...
    def get_prompt(self, key: str, **kwargs) -> Any:
        ...
        template = (
            ...
            else prompt_template.get("template", "")
        )
        system = (
            prompt_template.get("system_prompt", "")
        ...
        verified_examples = [
            ex
            for ex in self.few_shot_examples.get(key, [])
            if ex.get("verified", False)
        ]
