...
class VisualNovelService:
    ...
    def generate_script(self, fusion_id: int) -> Optional[VNScript]:
        ...
        try:
            script = self.llm_service.generate_structured(prompt, VNScript, system_prompt=system)
            return script
        except Exception as e:
            logger.error(f"VN script generation failed: {e}")
            return None

    def generate_scene_assets(self, scene: VNScene, session_seed: int) -> Dict[str, str]:
        """
        Génère les assets visuels (sprite et background) pour une scène donnée.
        """
        logger.info(f"Generating visual assets for scene: {scene.character} ({scene.mood})")
