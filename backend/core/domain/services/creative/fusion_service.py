...
class FusionDomainService:
    ...
    def generate_fusion_image(
        self, item_a: Dict, item_b: Dict, art_style: str = "Cyberpunk"
    ) -> Optional[str]:
        """
        ...
        if self.lora_manager:
            ...
            if os.path.exists(adapter_path):
                ...
                logger.debug(
                    f"ℹ️ No specific LoRA found for {art_style}, using base model + prompt."
                )
                self.lora_manager.disable_adapters()

        # 2. Génération du prompt
        prompt, _ = self.prompt_manager.get_prompt(
            "fusion_image", title_a=title_a, title_b=title_b, art_style=art_style
        )
