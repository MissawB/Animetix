...
from core.utils.security import is_safe_url  # noqa: E402
...
class VsBattleService:
    ...
    def _is_franchise_match(
        self, franchise: str, categories: List[str], found_title: str
    ) -> bool:
        if not franchise:
            return True
        f_norm = franchise.lower()
        if f_norm in found_title.lower():
            return True
        for cat in categories:
            if f_norm in cat.lower():
                return True
        return False
    ...
    def _is_name_match(
        ...
        if len(req_parts) > 1 and len(found_parts) > 1:
            ...
            if req_last not in found_norm and found_last not in req_norm:
                if difflib.SequenceMatcher(None, req_last, found_last).ratio() < 0.7:
                    logger.warning(
                        f"❌ Name conflict: '{requested_name}' vs '{found_title}' (Last name mismatch)"
                    )
                    return False
        ...
        if all(part in found_norm for part in req_parts):
            return True
        if difflib.SequenceMatcher(None, req_parts[0], found_parts[0]).ratio() > 0.85:
            return True
        return False

    def _recover_image(
        self, character_name: str, franchise: Optional[str]
    ) -> Optional[str]:
        if not self.web_search_port:
            return None
        logger.info(f"🖼️ [Image Recovery] Searching image for: {character_name}")
        try:
            results = self.web_search_port.search(
                f"{character_name} {franchise or ''} official portrait anime"
            )
    ...
    def _find_best_dict(self, obj: Any) -> Optional[Dict]:
        if isinstance(obj, dict):
            ...
            if (
                len([k for k in keys_to_match if k in obj or k.capitalize() in obj])
                >= 2
            ):
                return obj
            for v in obj.values():
    ...
    def _safe_generate_structured(
        self, prompt: str, system_prompt: str, response_model: Type[T]
    ) -> T:
        try:
            logger.info(
                f"Attempting structured generation for {response_model.__name__}"
            )
    ...
    def fetch_and_parse_character(
        self, name: str, franchise: Optional[str] = None
    ) -> CombatCharacter:
        versions = self.fetch_character_versions(name, franchise=franchise)
        if not versions:
            raise ValueError(
                f"Could not successfully parse any valid profile for '{name}'"
            )
        return max(versions, key=lambda c: c.stats.tier_value if c.stats else -1)

    def fetch_character_versions(
        self, name: str, franchise: Optional[str] = None, loose_pass: bool = False
    ) -> List[CombatCharacter]:
        pass_type = "RESCUE (Loose)" if loose_pass else "STRICT"
        logger.info(
            f"🔍 Finding Wiki versions for: {name} (Franchise: {franchise}, Mode: {pass_type})"
        )
            ...
            if candidates:
                for cand in candidates:
                    ...
                    if cand_name not in seen_titles:
                        if (
                            "Standard Format" in cand_name
                            or "Powerscaling" in cand_name
                        ):
                            continue
                        ...
                        if self._is_name_match(
                            ...
                            cand_name,
                            loose=loose_pass,
                            franchise_matched=is_franchise_ok,
                        ) or (loose_pass and "Verse" in cand_name):
        ...
        if not all_raw_data and not loose_pass:
            return self.fetch_character_versions(
                name, franchise=franchise, loose_pass=True
            )

        if not all_raw_data:
            logger.warning(
                f"🤖 Wiki page missing for '{name}'. Triggering Synthetic AI Generation..."
            )
            try:
                ...
                synthetic_char.name, synthetic_char.franchise = (
                    f"{name} (AI Generated)",
                    franchise,
                )
                from urllib.parse import quote  # noqa: E402

                synthetic_char.wiki_url = (
                    f"https://vsbattles.fandom.com/wiki/{quote(name.replace(' ', '_'))}"
                )
        ...
        for raw_data in all_raw_data:
            try:
                ...
                stats_match = _regex_module.search(
                    r"('''Tier:'''.*?)(?=\n==|\Z)",
                    wt,
                    _regex_module.IGNORECASE | _regex_module.DOTALL,
                )
                if stats_match:
                    stats_part = stats_match.group(1)[
                        :3000
                    ]  # Get up to 3000 chars of stats
                    focused_wikitext = (
                        summary_part + "\n\n--- STATS SECTION ---\n" + stats_part
                    )
                else:
                    # Fallback to a broader search around "Speed:" or "Durability:" if "Tier:" is missing
                    alt_match = _regex_module.search(
                        r"('''Speed:'''.*?)(?=\n==|\Z)",
                        wt,
                        _regex_module.IGNORECASE | _regex_module.DOTALL,
                    )
                ...
                prompt, system = self.prompt_manager.get_prompt(
                    "vs_battle_parser", wikitext=focused_wikitext
                )
                time.sleep(1.0)
                character = self._safe_generate_structured(
                    prompt=prompt, system_prompt=system, response_model=CombatCharacter
                )
                character.name, character.franchise = (
                    raw_data.get("name", name),
                    franchise,
                )
                character.wiki_url, character.image_url = (
                    raw_data.get("url", ""),
                    raw_data.get("image_url") or character.image_url,
                )
                ...
                logger.warning(
                    f"⚠️ Failed to parse version '{raw_data.get('name')}': {e}"
                )
    ...
    def _extract_max_tier(self, tier_str: str) -> str:
        parts = _regex_module.split(
            r"\||,|/|AND|OR", tier_str, flags=_regex_module.IGNORECASE
        )
        best_tier, max_val = tier_str, -1
        for part in parts:
            val = self._map_tier_to_value(part.strip())
            if val > max_val:
                max_val, best_tier = val, part
    ...
    def run_battle(
        ...
        char_a_name: str,
        char_b_name: str,
        char_a_franchise: Optional[str] = None,
        char_b_franchise: Optional[str] = None,
        ...
        debate_history = []
        for role, c, o in [
            ("Advocate_A", char_a, char_b),
            ("Advocate_B", char_b, char_a),
        ]:
            p, s = self.prompt_manager.get_prompt(
                "vs_battle_advocate",
                character_name=c.name,
                opponent_name=o.name,
                character_stats=c.stats.model_dump_json(indent=2),
                opponent_stats=o.stats.model_dump_json(indent=2),
                language=language,
            )
        ...
        prompt_j, system_j = self.prompt_manager.get_prompt(
            ...
            advocate_a_argument=debate_history[0].content,
            advocate_b_argument=debate_history[1].content,
            language=language,
        )
        ...
        verdict_data = self._safe_generate_structured(
            prompt=prompt_j, system_prompt=system_j, response_model=JudgeVerdict
        )
        debate_history.append(
            DebateTurn(
                agent="Judge",
                content=f"{verdict_data.analysis}\n\nVERDICT: {verdict_data.verdict}",
            )
        )
        return CombatResult(
            character_a=char_a,
