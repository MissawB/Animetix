...
class MangaFlowService:
    ...
    def translate_manga_page(
        self, image_data: bytes, target_lang: str = "French"
    ) -> str:
        # 1. OCR SOTA
        ...
        if not layout and full_text:
            prompt, system_prompt = self.prompt_manager.get_prompt(
                "manga_translation", target_lang=target_lang, original_text=full_text
            )
            translated_text = self.llm_service.generate(
            ...
            return self.inference_engine.inpaint_text_bubbles(
                image_data, [{"text": translated_text, "bbox": [0, 0, width, height]}]
            )
        ...
        for bubble in layout:
            ...
            translated_bubbles.append({"text": translated_text, "bbox": bbox})

        return self.inference_engine.inpaint_text_bubbles(
            image_data, translated_bubbles
        )
