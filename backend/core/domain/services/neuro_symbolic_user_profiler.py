...
class NeuroSymbolicUserProfiler:
    ...
    def deduce_preference_rules(self, user_feedbacks: List[Dict[str, Any]]) -> List[str]:
        """
        ...
        try:
            ...
            if solver.check() == z3.sat:
                ...
                self.rules = rules
                logger.info(f"🧩 Neuro-Symbolic Z3 Solver: Deduced {len(rules)} formal preference bounds.")
                return rules
            else:
                logger.warning("🧩 Z3 UNSAT: Preference constraints are contradictory. Softening logic bounds.")
                
        except ImportError:
            logger.warning("⚠️ Z3 logic solver package not pre-installed. Falling back to pure Python symbolic engine.")
