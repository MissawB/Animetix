...
class VideoLanguageIndexingService:
    ...
    """
    def __init__(self, inference_engine: InferencePort, prompt_manager: PromptManager, repository: Any = None, feedback_port: Optional[FeedbackRepositoryPort] = None):
    ...
    def record_recognition_failure(self, query: str, actual_output: str, correction: str, user_id: Any = None):
        """
        ...
        logger.info(f"🧠 Skill Bank: Correction experience recorded for query: {query[:50]}...")
    ...
    def _get_skill_bank_context(self, anime_title: str) -> str:
        ...
        recent_skills = self.feedback_port.get_recent_feedback(limit=5, feedback_type="skill_bank_entry")
        ...
        for skill in recent_skills:
            try:
                ...
                context_parts.append(f"- Échec précédent: {skill['input_context']} -> Correction: {data['correction']}")
            except Exception:
                continue
                
        return "\n".join(context_parts)

    def verify_and_evolve(self, video_data: bytes, generated_narrative: str, metadata: Dict[str, Any], user_id: Any = None):
        """
    ...
    def index_video_moment(self, video_data: bytes, metadata: Dict[str, Any]):
        ...
        if narrative and self.repository:
            try:
                ...
                logger.info(f"💾 Narrative indexed successfully for {title} (ID: {doc_id}).")
            except Exception as e:
