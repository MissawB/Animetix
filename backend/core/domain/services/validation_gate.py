...
class SyntheticValidationService:
    ...
    def validate_and_stage(self, 
        ...
        """
        logger.info(f"🚦 [HITL Gate] Validating {entry_type} entry: '{instruction[:50]}...'")

        # 1. Safety & Factual Guardrail Check
        safety_report = self.guardrail_service.validate_output(response, context=context, query=instruction)
        ...
        ai_critique, validation_score = self._run_ai_critique(context, instruction, response)
        ...
        logger.info(f"✅ [HITL Gate] Entry {entry_id} staged with AI Score: {validation_score:.2f} (Confidence: {confidence_score:.2f})")
        return entry_id

    def _run_ai_critique(self, context: str, instruction: str, response: str) -> (str, float):
        """
...
"""
        try:
            critique_res = self.inference_engine.generate(critique_prompt, system_prompt="Tu es l'Auditeur Suprême d'Animetix.")
            import json
            import re
            ...
            return data.get("critique", "No critique provided."), float(data.get("score", 0.5))
