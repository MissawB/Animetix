...
class StaticDiorama3DService:
    ...
    def reconstruct_static_diorama(
        self, image_data: bytes, title: str
    ) -> Dict[str, Any]:
        """
        Pipeline complet 2D fixe -> Estimation de Profondeur -> Diorama Spatial 3D (PLY Point Cloud).
        """
        logger.info(
            f"🌌 SGS Spatial: Starting Static 3D reconstruction for '{title}'..."
        )
        ...
        logger.info(
            f"✅ SGS Static 3D Diorama generated for '{title}'. Ready for VR exploration."
        )
