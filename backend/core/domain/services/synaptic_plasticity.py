...
class SynapticPlasticitySimulator:
    def __init__(
        self, num_concepts: int = 10, tau_plus: float = 20.0, tau_minus: float = 20.0
    ):
        """
    ...
    def update_stdp(
        ...
        if delta_t > 0:
            ...
            dW = -learning_rate * math.exp(
                delta_t / self.tau_minus
            )  # delta_t est négatif, donc division cohérente
        else:
