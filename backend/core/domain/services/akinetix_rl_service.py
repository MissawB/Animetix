...
class AkinetixRLService:
    ...
    def process_answer(
        self, catalog_db: List[Dict], state: Dict, raw_answer: str
    ) -> Dict:
        """
        ...
        mapping = {
            ...
            "NON": False,
            "PEUT-ÊTRE": None,
            "PROBABLEMENT PAS": None,
        }
        mapping.get(raw_answer.upper())
        ...
        for h in history:
            state.get(
                "current_attr"
            )  # Simplification: on devrait stocker l'attr de chaque Q
    ...
    def _simulate_info_gain(self, env: AkinetixRLEnvironment, action_idx: int) -> float:
        ...
        for item in env.active_candidates:
            if attr_type == "genre" and attr_val in item.get("genres", []):
                count += 1
            elif attr_type == "tag" and attr_val in item.get("micro_tags", []):
                count += 1
            elif attr_type == "studio" and attr_val in item.get("studios", []):
                count += 1
    ...
    def _format_question(self, attribute: str) -> str:
        ...
        if attr_type == "genre":
            return f"Est-ce que c'est un {val} ?"
        if attr_type == "studio":
            return f"Est-ce que c'est produit par {val} ?"
        if attr_type == "tag":
            return f"Est-ce que ça parle de {val} ?"
