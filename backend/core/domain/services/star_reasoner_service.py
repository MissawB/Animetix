...
class StarReasonerService:
    ...
    def solve_riddle_with_star(
        self, riddle: str, expected_answer: str, num_attempts: int = 3
    ) -> Dict:
        """
        Tente de résoudre une énigme en générant plusieurs chemins de raisonnement via SLM.
        """
        logger.info(
            f"🧠 STaR: Generating {num_attempts} reasoning paths using Compact Core..."
        )
        successful_paths = []
        ...
        for i in range(num_attempts):
            prompt, system = self.prompt_manager.get_prompt(
                "star_reasoner_thought", riddle=riddle
            )
        ...
        if successful_paths:
            ...
            logger.info(
                f"✅ STaR: Found {len(successful_paths)} correct reasoning paths. Traces saved for human validation."
            )
