...
class MangaService:
    ...
    def get_chapters(self, manga_id: str) -> List[MangaChapter]:
        ...
        chapters = list(MangaChapter.objects.filter(manga__external_id=manga_id).order_by('number'))
        
        if not chapters:
            logger.info(f"📚 No chapters found in DB for manga {manga_id}. Triggering dynamic fetch...")
    ...
    def get_chapter_details(self, manga_id: str, chapter_number: float) -> Optional[MangaChapter]:
        """Récupère les détails d'un chapitre (pages)."""
        try:
            chapter = MangaChapter.objects.get(manga__external_id=manga_id, number=chapter_number)
            
            # Si le chapitre n'a pas de pages, on tente de les charger
            if not chapter.pages.exists():
                logger.info(f"📄 No pages found for chapter {chapter_number} of manga {manga_id}. Fetching...")
                self._sync_pages_from_external(chapter)
                
            return chapter
        except MangaChapter.DoesNotExist:
            # Tenter de synchroniser si le manga existe
            if MediaItem.objects.filter(external_id=manga_id, media_type='Manga').exists():
                self._sync_chapters_from_external(manga_id)
                try:
                    return MangaChapter.objects.get(manga__external_id=manga_id, number=chapter_number)
                except MangaChapter.DoesNotExist:
                    return None
            return None
    ...
    def _sync_chapters_from_external(self, manga_id: str):
        ...
        try:
            ...
            logger.error(f"❌ Cannot sync chapters: Manga {manga_id} not found in catalog.")
