...
class AkinetixService:
    ...
    def __init__(
        ...
        if self.catalog_db is not None:
            ...
            self.items, self.attributes = self.engine._prepare_classical_data(
                self.catalog_db, fine_attrs
            )
            ...
            if probs is not None and len(probs) == self.n_items:
                ...
                self.probs = (
                    np.full(self.n_items, 1.0 / self.n_items)
                    if self.n_items > 0
                    else np.array([])
                )
    ...
    def update_probabilities(self, attribute: str, answer: str):
        ...
        self.probs = self.engine._update_classical_probs(
            self.items, self.probs, attribute, mapped_answer, fine_attrs
        )
    ...
    def propose_next_question(self) -> Optional[str]:
        if self.catalog_db is None:
            raise ValueError("Stateful operation requires catalog_db")
        fine_attrs = self.catalog_service.get_akinetix_attributes()
        return self.engine._propose_classical_question(
            self.items, self.attributes, self.probs, self.asked_attributes, fine_attrs
        )
    ...
    def get_probabilities(self) -> Dict[str, float]:
        if self.catalog_db is None:
            raise ValueError("Stateful operation requires catalog_db")
        return {
            (item.get("title") or item.get("name", "Inconnu")): prob
            for item, prob in zip(self.items, self.probs)
        }
    ...
    def get_state(self, port: GameStatePort) -> AkinetixGameState:
        ...
        history = [
            AkinetixQuestion(**q) if isinstance(q, dict) else q for q in history_raw
        ]
