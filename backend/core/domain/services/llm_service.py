...
class LLMService:
    def __init__(
        ...
        self.vision_engine = (
            vision_engine or inference_engine
        )  # Fallback to main if no Vision
    ...
    def _retry_json(
        ...
        res = self.generate(
            fix_prompt,
            system_prompt="Tu es un expert en correction de JSON. Retourne uniquement du JSON valide.",
            use_slm=use_slm,
        )
        try:
            return self._parse_json(res, schema)
            ...
            logger.error(
                f"JSON retry failed: {e}", extra={"context": {"output": res[:500]}}
            )
            raise ParsingError(f"JSON recovery failed after retry: {str(e)}")

    def generate_fusion_scenario(
        self,
        media_type: str,
        item1: Dict,
        item2: Dict,
        language: str,
        chaos_level: int = 50,
        universe_balance: int = 50,
        art_style: str = "Cyberpunk",
    ) -> str:
        ...
        if universe_balance < 40:
            ...
            balance_instruction = (
                "Les deux univers doivent être parfaitement équilibrés."
            )

        chaos_instruction = (
            "Garde un récit très logique et ancré dans le lore."
            if chaos_level < 30
            else (
                "N'hésite pas à être abstrait et à briser le 4ème mur."
                if chaos_level > 70
                else "Mélange les concepts de manière créative."
            )
        )
        ...
        return self.generate(
            ...
            forbidden_terms=[item1["title"], item2["title"]],
        )

    def generate_paradox_explanation(
        self,
        media_type: str,
        item_a: str,
        item_b: str,
        ...
        return self.generate(
            prompt, use_slm=use_slm, forbidden_terms=[item_a, item_b, intruder]
        )
    ...
    def generate_undercover_clue(
        self, media_type: str, item_a: str, item_b: str
    ) -> str:
        prompt, system = self.prompt_manager.get_prompt(
            "undercover_clue", media_type=media_type, item_a=item_a, item_b=item_b
        )
        return self.generate(
            prompt, system_prompt=system, forbidden_terms=[item_a, item_b]
        )
    ...
    def _get_token_usage(
        self, response_obj: Any, prompt: str, system_prompt: str
    ) -> tuple[int, int, int]:
