...
class FallbackRagProcessor(StateProcessor):
    ...
    def process(self, ctx: RAGContext) -> RAGState:
        # Based on RAGWorkflowManager._handle_fallback_rag
        ...
        ctx.full_answer = ""
        # Simplified generation, not streaming to adhere to interface
        ctx.full_answer = self.inference_engine.generate(prompt, system)
            
        return RAGState.FINALIZE

    def _get_expert_injections(self, query: str) -> list:
        expert_injections = []
        q_lower = query.lower()
