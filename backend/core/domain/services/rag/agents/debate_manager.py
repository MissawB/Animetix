...
def robust_json_loads(raw: str) -> dict:
    ...
    if "```" in clean_raw:
        match = re.search(
            r"```(?:json)?\s*(\{.*?\})\s*```", clean_raw, re.DOTALL | re.IGNORECASE
        )
        if match:
...
def sanitize_judge_data(data: dict) -> dict:
    ...
    if isinstance(is_rel, str):
        ...
        data["is_reliable"] = (not data["hallucination_detected"]) and (
            data["faithfulness_score"] > 0.5
        )
    ...
    def conduct_debate(
        ...
        if xai_collector:
            xai_collector.log_agent_thought(
                "DebateManager",
                f"Lancement du débat multi-experts ({len(judges)} juges)",
            )
        ...
        def run_single_judge(j_key: str):
            try:
                ...
                prompt, sys = self.prompt_manager.get_prompt(
                    j_key, query=query, context=context, answer=answer
                )
                ...
                try:
                    ...
                    logger.warning(
                        f"Judge {j_key} response parsed with errors: {parse_err}. Raw response: {raw}"
                    )
                    return j_key, None
