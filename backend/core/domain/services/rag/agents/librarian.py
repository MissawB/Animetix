    ...
    def identify_gap(self, query: str, context: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            prompt, system = self.prompt_manager.get_prompt(
                "librarian_mission", query=query, context=context
            )
            ...
            if json_start != -1 and json_end != -1:
                ...
                try:
                    ...
                    if "source_type" in gap_data:
                        ...
                        logger.info(
                            f"📍 Librarian: Gap identified! Source: {gap_data.get('source_type')}"
                        )
                        return gap_data
                except json.JSONDecodeError:
                    logger.warning(
                        f"⚠️ Librarian: Failed to parse JSON from response: {response}"
                    )
