...
class ContextCompressor:
    ...
    def compress(self, query: str, context: str) -> str:
        ...
        logger.info(
            f"⚡ Context Compressor: Compressing raw context of length {len(context)}..."
        )
        ...
        try:
            ...
            if compressed and len(compressed) > 100:
                ...
                logger.info(
                    f"✅ Context Compressor: Compressed context length from {len(context)} to {len(compressed)} (-{reduction:.1f}% tokens)."
                )
                return compressed
            return deduplicated
        except Exception as e:
            logger.error(
                f"❌ Context Compressor failed: {e}. Falling back to deduplicated context."
            )
            return deduplicated
