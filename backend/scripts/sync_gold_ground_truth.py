...
class CurationSyncSchema(BaseModel):
    is_accurate: bool = Field(description="True if the current ground_truth matches the Neo4j facts. False if there are factual differences.")
    updated_ground_truth: str = Field(description="If not accurate, the updated ground truth text reflecting the correct Neo4j facts. If accurate, repeat the original ground_truth.")
    reasoning: str = Field(description="Brief explanation of what facts (if any) changed or why it is accurate.")

class ExpectedFactsSchema(BaseModel):
    expected_facts: List[Any] = Field(description="List of expected facts or lists of synonyms. Example: [['74 episodes', '74 épisodes'], 'Wit Studio']")

def fetch_neo4j_facts(neo4j_manager, mid: str, neo4j_online: bool = True) -> Dict[str, Any]:
...
def update_regression_benchmark_file(new_gold_set: List[Dict[str, Any]]):
    path = os.path.join(PROJECT_ROOT, "backend", "pipeline", "evaluation", "regression_benchmark.py")
    ...
    if end_idx == -1:
        logger.error("Could not find matching closing bracket for GOLD_SET in regression_benchmark.py")
        return
        
    new_content = content[:start_idx] + "GOLD_SET = " + formatted_list + content[end_idx:]
    
    with open(path, "w", encoding="utf-8") as f:
        f.write(new_content)
    logger.info("Successfully updated regression_benchmark.py with updated expected facts.")
...
def run_synchronization(dry_run: bool = False) -> Dict[str, Any]:
    ...
    if not neo4j_online:
        logger.warning("⚠️ Neo4j knowledge graph is offline. Skipping database synchronization.")

    updated_count = 0
    stats = []

    # Let's import regression benchmark to sync in memory first
    from backend.pipeline.evaluation.regression_benchmark import GOLD_SET as benchmark_gold_set
                ...
                for b_entry in updated_benchmark_set:
                    if b_entry["query"].lower().strip() == query.lower().strip():
                        ...
                        try:
                            ...
                            logger.info(f"   Updated benchmark facts: {facts_obj.expected_facts}")
                            if not dry_run:
                                b_entry["expected_facts"] = facts_obj.expected_facts
                        except Exception as fe:
                            logger.error(f"   Failed to extract expected facts for benchmark query: {fe}")
                            
        except Exception as e:
            logger.error(f"Failed structured validation for query '{query[:50]}...': {e}")

    if updated_count > 0 and not dry_run:
        # Save gold dataset
        with open(gold_path, "w", encoding="utf-8") as f:
            json.dump(gold_data, f, indent=2, ensure_ascii=False)
        logger.info(f"Successfully synchronized {updated_count} entries in gold_dataset.json.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dry-run", action="store_true", help="Audit dataset without saving modifications")
