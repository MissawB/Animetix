...
def run_vectorization():
    try:
        ...
        # Collections
        existing_ids = chroma_manager.get_all_ids("manga_thematic")
        existing_vision_ids = chroma_manager.get_all_ids("manga_visual_vibe")
        existing_plot_ids = chroma_manager.get_all_ids("manga_plot")
        ...
        for i in range(0, len(new_items), BATCH_SIZE):
            ...
            if batch_texts:
                ...
                chroma_manager.add_to_collection("manga_thematic", batch_ids, embeddings, batch_metas)
            
            if plot_texts:
                p_embeddings = text_model.encode(plot_texts, convert_to_numpy=True)
                chroma_manager.add_to_collection("manga_plot", plot_ids, p_embeddings, plot_metas)
                
            if vision_images:
                v_embeddings = vision_model.encode(vision_images, convert_to_numpy=True)
                chroma_manager.add_to_collection("manga_visual_vibe", vision_ids, v_embeddings, vision_metas)
