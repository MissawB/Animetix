...
class DataIntelligence:
    ...
    def build_relation_graph(self, media_data, media_type):
        ...
        # Note: Pour le prototype, on extrait ce qui est déjà dans les données propres
        # Dans une version prod, on ferait des appels API supplémentaires pour le Staff.
        relations = {
            "studios": media_data.get('studios', []),
            "author": media_data.get('author'), # Souvent présent dans Manga
            "director": media_data.get('director'), # Souvent présent dans Movie
