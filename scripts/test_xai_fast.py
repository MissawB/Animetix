...
def main():
    ...
    logprobs_data = [
        ...
        TokenLogProb(
            token=" l'énergie", logprob=-0.08, top_logprobs=[{" l'énergie": -0.08}]
        ),
    ]

    mock_engine.stream_generate.return_value = [
        InferenceResponse(
            text="Le ", metadata=InferenceMetadata(logprobs=[logprobs_data[0]])
        ),
        InferenceResponse(
            text="Nen ", metadata=InferenceMetadata(logprobs=[logprobs_data[1]])
        ),
        InferenceResponse(
            text="est ", metadata=InferenceMetadata(logprobs=[logprobs_data[2]])
        ),
        InferenceResponse(
            text="l'énergie ", metadata=InferenceMetadata(logprobs=[logprobs_data[3]])
        ),
    ...
    def mock_run_workflow(ctx, xai_collector=None):
        ...
        if xai_collector:
            ...
            xai_collector.log_retrieval(
                [{"id": "doc1", "title": "Wiki Nen", "score": 0.95}]
            )
            xai_collector.log_agent_thought(
                "Planner", "Recherche d'informations basiques sur le Nen."
            )
    ...
    query = (
        "Explique moi le concept de Nen dans Hunter x Hunter. (TEST XAI UNIQUE 12345)"
    )
    print(f"\n🧠 Requête : {query}\n")
