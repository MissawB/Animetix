...
def run_targeted_quality_benchmark():
    print(
        "🚀 Starting Targeted Quality Benchmark (Faithfulness & Relational Accuracy)..."
    )
        ...
        try:
            for event in rag.plan_and_solve_stream(q, "Anime"):
                if event["type"] == "token":
                    ...
                    event["type"] == "thought" and "[State Machine]" in event["content"]
                ):
                    # Extract state name
    ...
    eval_llm = ChatGoogleGenerativeAI(
        model="gemini-3.1-flash-lite-preview", google_api_key=api_key
    )
    ...
    try:
        ...
        for i in range(0, len(dataset), batch_size):
            print(
                f"   ⏳ Evaluating batch {i // batch_size + 1}/{total_batches} (waiting to respect rate limits)..."
            )
            ...
            try:
                ...
                print(
                    f"   ⚠️ Batch {i // batch_size + 1} failed: {batch_e}. Skipping to next."
                )
        ...
        relational_acc = results_df[
            results_df["query_type"].isin(["graph", "cross-media"])
        ]["faithfulness"].mean()
        print(
