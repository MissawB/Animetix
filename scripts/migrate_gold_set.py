...
def migrate_gold_set():
    ...
    entity_mappings = {
        ...
        "Attack on Titan": ["Attack on Titan", "Wit Studio", "Shingeki no Kyojin", "Linked Horizon", "NHK"],
        "Ghost in the Shell": ["Ghost in the Shell", "Motoko Kusanagi", "cyberpunk"],
        "Monster": ["Monster", "Naoki Urasawa", "Kana", "Big Comic Original", "Shōgakukan"],
        "Naruto": ["Naruto", "Kakashi Hatake", "TV Tokyo"],
        "Death Note": ["Death Note", "Ryuk", "Yann Pichon", "Weekly Shōnen Jump", "Shūeisha"],
        "One Piece": ["One Piece", "Eiichiro Oda", "Luffy", "Stéphane Excoffier", "Trafalgar Law"],
        ...
        if 'expected_entities' not in entry:
            ...
            if not entities:
                for key in entity_mappings:
                    if key.lower() in entry['query'].lower() or key.lower() in entry['ground_truth'].lower():
    ...
    has_multi_turn_19 = any(e['query'] == "Dans quel magazine a-t-il été prépublié ?" for e in data)
    ...
    has_multi_turn_16498 = any(e['query'] == "Qui anime cette adaptation et a aussi produit Vinland Saga ?" for e in data)
    if not has_multi_turn_16498:
        data.append({
            ...
            "expected_entities": ["Wit Studio", "L'Attaque des Titans", "Vinland Saga"],
            "expected_contexts": [
                "Le studio Wit Studio a produit Vinland Saga et L'Attaque des Titans."
            ],
            "expected_chunks": ["16498"],
            "multi_turn_history": [
                {"role": "user", "content": "Quel studio a animé Shingeki no Kyojin au début ?"},
    ...
    print(f"Successfully migrated {migrated_count} queries and saved {len(data)} total queries to {gold_path}.")
