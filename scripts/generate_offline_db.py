...
OUTPUT_DIR = os.path.join(BASE_DIR, 'src', 'backend', 'animetix', 'static', 'animetix', 'data')
    ...
    for m_type, filename in files.items():
        ...
        with open(path, 'r', encoding='utf-8') as f:
            ...
            def get_pop(x):
                ...
                if isinstance(p, dict): return p.get('favourites', 0)
                return p or 0
            ...
            for item in sorted_data[:limit]:
                ...
                for g in genres:
                    cursor.execute("INSERT INTO tags (media_id, name, type) VALUES (?, ?, ?)", (m_id, g, 'genre'))
                for t in tags:
                    cursor.execute("INSERT INTO tags (media_id, name, type) VALUES (?, ?, ?)", (m_id, t, 'tag'))
