...
🌐 **Live at [animetix.xyz](https://animetix.xyz)**
...
- [💎 Rewarded Economy: The Berrix Model](#-rewarded-economy-the-berrix-model)
- [🧠 Core RAG 2.0 & Cognitive Architecture](#-core-rag-20--cognitive-architecture)
- [☁️ GCP Infrastructure](#%EF%B8%8F-gcp-infrastructure)
...
*   **Dependency Injection Everywhere**: Views, services, and adapters are wired through `dependency_injector` containers — every collaborator is constructor-injected and overridable in tests.
...
* **Secret Manager:** Every credential (database, Neo4j Aura, Hugging Face, Gemini) is mounted as a secret reference — no secrets ship in images or env files.
...
│   │   └── utils/          # Model registry & shared security utilities
│   ├── adapters/           # Infrastructure Layer (Driven Adapters)
│   │   ├── persistence/    # pgvector, Vertex AI, Neo4j and Suwayomi adapters
│   │   └── inference/      # Google GenAI, Brain API and Ollama adapters
│   └── api/                # Presentation Layer, split by domain
│       └── animetix/api/   # core/, labs/, games/ packages + SSE stream views
...
│   └── deployments.yaml    # Single declarative source for every deploy target
