...
interface UndercoverLobbySettingsProps {
  ...
  applySettings: (patch: Record<string, any>) => void;
    ...
    <>
      <div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <button
            ...
            className={`flex items-center justify-center gap-2 py-2.5 rounded-xl border-2 text-xs font-black uppercase transition-all ${
              isPublic ? 'border-green-500 bg-green-500/15 text-green-400' : 'border-white/10 text-white/40 hover:border-green-500/40'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center justify-center gap-2 py-2.5 rounded-xl border-2 text-xs font-black uppercase transition-all ${
              !isPublic ? 'border-red-500 bg-red-500/15 text-red-400' : 'border-white/10 text-white/40 hover:border-red-500/40'
            }`}
          >
        ...
        <p className="mt-2 text-[11px] text-white/35 italic">
          {isPublic
            ? t('games.undercover.room.public_hint', 'Visible dans la liste des salons publics — tout le monde peut rejoindre.')
            : t('games.undercover.room.private_hint', "Accessible uniquement via le code ou l'URL.")}
        </p>
      </div>
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 py-2.5 px-3 rounded-xl border-2 text-xs font-black uppercase transition-all ${
                  on ? 'border-red-500 bg-red-500/15 text-red-400' : 'border-white/10 text-white/40 hover:border-red-500/40'
                }`}
              >
                <span
      ...
      <div>
          ...
          {DIFFS.map((d) => (
            <button
              ...
              className={`py-2.5 rounded-xl border-2 text-xs font-black uppercase transition-all ${
                difficulty === d ? 'border-red-500 bg-red-500/15 text-red-400' : 'border-white/10 text-white/40 hover:border-red-500/40'
              }`}
            >
              {DIFF_LABEL[d]}
            </button>
        ...
        <p className="mt-2 text-[11px] text-white/35 italic">
          {DIFF_HINT[difficulty]} {t('games.undercover.room.difficulty_hint_suffix', "— plus c'est dur, plus les œuvres sont obscures.")}
        </p>
      </div>
      ...
      <div className="space-y-6">
        <div>
          <div className="flex items-center justify-between mb-2.5">
            <p className="text-[11px] font-black uppercase tracking-[0.25em] text-red-400/80 flex items-center gap-1.5">
              <Vote className="w-3.5 h-3.5" /> {t('games.undercover.room.intruders_label', 'Intrus')}
            </p>
          ...
          <div className="flex justify-between mt-1.5 text-[10px] font-bold text-white/25 tabular-nums">
            <span>1</span>
            <span>{t('games.undercover.room.max_value', { defaultValue: 'max {{value}}', value: maxUnderSlider })}</span>
          </div>
        </div>
        <div>
          <div className="flex items-center justify-between mb-2.5">
          ...
          <div className="flex justify-between mt-1.5 text-[10px] font-bold text-white/25 tabular-nums">
            <span>0</span>
            <span>{t('games.undercover.room.max_value', { defaultValue: 'max {{value}}', value: maxWhiteSlider })}</span>
          </div>
          <p className="mt-2 text-[11px] text-white/35 italic">
            {t(
        ...
        <div>
          <div className="flex h-2.5 rounded-full overflow-hidden bg-white/5">
            {civilsCount > 0 && (
              <div className="bg-green-500/70" style={{ width: `${(civilsCount / Math.max(1, players.length)) * 100}%` }} />
            )}
            {under > 0 && <div className="bg-red-500/70" style={{ width: `${(under / Math.max(1, players.length)) * 100}%` }} />}
            {white > 0 && (
              <div className="bg-purple-400/70" style={{ width: `${(white / Math.max(1, players.length)) * 100}%` }} />
            )}
          </div>
          <div className="flex flex-wrap items-center gap-2 mt-3 text-[11px] font-black uppercase tracking-wider">
            <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-green-500/10 text-green-400">
              ...
              {t('games.undercover.room.civils_count', { defaultValue: '{{count}} civils', count: civilsCount })}
            </span>
            <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-red-500/10 text-red-400">
              <span className="w-2 h-2 rounded-full bg-red-500" />
              {t('games.undercover.room.intruders_count', { defaultValue: '{{count}} intrus', count: under })}
            </span>
            {white > 0 && (
              <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-purple-500/10 text-purple-300">
                <span className="w-2 h-2 rounded-full bg-purple-400" />
                {t('games.undercover.room.mrwhite_count', { defaultValue: '{{count}} Mr. White', count: white })}
              </span>
            )}
          </div>
        </div>
      ...
      <div>
        ...
        {players.length >= MIN_PLAYERS && !hasAnchor && (
          <p className="mt-3 text-center text-[11px] font-bold uppercase tracking-widest text-red-400 flex items-center justify-center gap-1.5">
            <Lock className="w-3.5 h-3.5" />{' '}
            {t('games.undercover.room.anchor_required', 'Coche au moins une catégorie ancre (anime/manga/perso)')}
          </p>
        )}
      </div>
    </>
