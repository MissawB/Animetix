  ...
  return (
    <nav className="px-6 md:px-12 py-4 flex items-center justify-between sticky top-0 bg-[#fffcf0] dark:bg-[#1a1a2e] z-[1000] border-b border-black/5 dark:border-white/5 transition-colors duration-500">
      ...
      <div className="hidden lg:flex flex-wrap items-center justify-center gap-x-8 gap-y-2 flex-grow mx-8">
          <Link to="/games/hub/" className="flex items-center gap-2 no-underline text-xs font-black italic text-gray-600 dark:text-gray-400 hover:text-blue-500 dark:hover:text-blue-400 transition-all uppercase tracking-widest">
            <Gamepad2 className="w-4 h-4" /> {t('nav.games', 'Jeux')}
          </Link>
          <Link to="/search/" className="flex items-center gap-2 no-underline text-xs font-black italic text-gray-600 dark:text-gray-400 hover:text-cyan-500 dark:hover:text-cyan-400 transition-all uppercase tracking-widest">
            <Search className="w-4 h-4" /> {t('nav.search', 'Recherche')}
          </Link>
          <Link to="/social/dashboard/" className="flex items-center gap-2 no-underline text-xs font-black italic text-gray-600 dark:text-gray-400 hover:text-orange-500 dark:hover:text-orange-400 transition-all uppercase tracking-widest">
            <Users className="w-4 h-4" /> {t('nav.community', 'Communauté')}
          </Link>
          <Link to="/lab/latent-space/" className="flex items-center gap-2 no-underline text-xs font-black italic text-gray-600 dark:text-gray-400 hover:text-indigo-500 dark:hover:text-indigo-400 transition-all uppercase tracking-widest">
            <Network className="w-4 h-4" /> {t('navbar.latent', 'Espace Latent')}
          </Link>
          <Link to="/lab/forge-hub/" className="flex items-center gap-2 no-underline text-xs font-black italic text-gray-600 dark:text-gray-400 hover:text-purple-500 dark:hover:text-purple-400 transition-all uppercase tracking-widest">
      ...
      <div className="flex items-center gap-4 shrink-0">
          {user ? (
            ...
            <div className="flex items-center gap-3">
              <Link to="/auth/login/" className="flex items-center gap-2 no-underline text-xs font-black italic text-gray-600 dark:text-gray-400 hover:text-blue-500 transition-all uppercase tracking-widest">
