...
import { Sparkles, Users, Network, Orbit, BookOpen, Download } from 'lucide-react';
...
// ─── Detail Panel ────────────────────────────────────────────────────
const UniverseDetailPanel: React.FC<{ universe: Universe; onClose: () => void }> = ({
  universe,
  onClose,
}) => {
  ...
  return (
    <Modal
      ...
      contentClassName={`bg-[#0a0a14] border ${colorClasses.split(' ').pop()} rounded-3xl shadow-2xl custom-scrollbar`}
    >
      {/* Header */}
      <div
        className={`p-10 -mx-6 -mt-6 bg-gradient-to-br ${colorClasses} relative overflow-hidden`}
      >
      ...
      <div className="p-4 pt-8 space-y-8">
          ...
          <section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {universe.characters.map((c, i) => (
                ...
                >
                  <div
                    className={`w-10 h-10 rounded-full bg-white/10 border border-white/20 flex items-center justify-center text-sm font-black ${accent}`}
                  >
                    {c.name.charAt(0)}
                  </div>
                  ...
                  {c.power_level > 0 && (
                    <div className="shrink-0 text-right">
                      ...
                      <p className={`text-sm font-black italic ${accent}`}>
                        {c.power_level.toLocaleString()}
                      </p>
                    </div>
                  )}
                </div>
        ...
        <div className="pt-6 border-t border-white/5 flex gap-4">
          ...
          <Button
            onClick={() => {
              window.open(
                `/api/v1/multiverse/${encodeURIComponent(universe.name)}/export-pdf/`,
                '_blank',
              );
            }}
