...
import { GitBranch, Search, Loader2, ChevronRight, Info, X, Network, Sparkles } from 'lucide-react';
import { useMutation } from '@tanstack/react-query';
import { Link } from 'react-router-dom';
import { useSSE } from '../../hooks/useSSE';
import _ForceGraph2D, { type ForceGraphMethods } from 'react-force-graph-2d';
const ForceGraph2D =
  (_ForceGraph2D as unknown as { default: React.ElementType }).default || _ForceGraph2D;
...
const TreeOfThoughtsPage: React.FC = () => {
  ...
  const accRef = useRef({ nodes: [] as ToTNode[], links: [] as ToTLink[], lastSelectedId: 'root' });
              ...
              >
                ...
                <div className="space-y-3 relative z-10">
                  <p className="text-[9px] font-bold uppercase tracking-wider text-white/50 leading-relaxed">
                    <span className="text-emerald-400">Multi-Pensée :</span> L'IA n'écrit pas d'un
                    seul trait. Elle explore plusieurs "branches" de réflexion en parallèle pour
          ...
          <div className="mt-24 grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            >
              ...
              <div className="space-y-4 relative z-10">
                <p className="text-xs font-bold uppercase tracking-wider text-white/60 leading-relaxed">
                  <span className="text-emerald-400">La Question :</span> Posez un problème
                  complexe. L'IA ne répond pas d'un seul jet : elle explore plusieurs pistes de
                  réflexion en parallèle avant de conclure.
                </p>
                <p className="text-xs font-bold uppercase tracking-wider text-white/60 leading-relaxed">
                  <span className="text-emerald-400">La Lecture :</span> Chaque point est une idée
                  notée par l'IA. Vert = piste retenue, rouge = branche abandonnée, jaune =
                  conclusion. Cliquez sur un nœud pour lire sa trace de pensée.
                </p>
                <p className="text-xs font-bold uppercase tracking-wider text-white/60 leading-relaxed">
                  <span className="text-emerald-400">Les Deux Modes :</span> "Générer l'arbre"
                  renvoie l'arbre complet d'un coup ; "Stream en direct" affiche les nœuds au fur et
                  à mesure (connexion requise, consomme des Berrix).
                </p>
              </div>
            </Card>
            ...
            <div className="p-12 rounded-[4rem] bg-gradient-to-br from-emerald-600/10 to-transparent border border-white/5 flex flex-col justify-center text-center">
              <p className="text-sm font-black uppercase tracking-[0.15em] italic leading-relaxed text-emerald-200/60">
                Recherche arborescente type Tree-of-Thoughts : le modèle génère des pensées
                candidates, les évalue par un score de confiance et élague les branches faibles,
                comme dans une exploration MCTS. <br />
                Le mode direct diffuse chaque nœud via un flux SSE, rendu en temps réel dans un
