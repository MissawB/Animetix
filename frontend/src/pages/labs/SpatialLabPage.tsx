        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-12">
          ...
          <div className="lg:col-span-1 space-y-8">
              <Card padding="lg" className="h-fit">
                  ...
                  <input type="file" id="spatial-upload" aria-label={t('labs.spatial.choose_image', 'Choisir une image')} className="hidden" accept="image/*" onChange={handleUpload} />
                  ...
                  <div className="space-y-3">
                      ...
                      >
                          <Cuboid className="w-5 h-5" /> {t('labs.spatial.generate_3d_btn', 'GÉNÉRER MODÈLE 3D')}
                      </Button>
                  </div>
              </Card>
              ...
              <Card padding="lg" className="bg-gray-900 text-white/40 border-white/5">
                  ...
                  <p className="text-xs font-bold leading-relaxed italic mb-4">
                      {t('labs.spatial.desc1', "Reconstruisez la géométrie 3D à partir d'une image 2D grâce aux modèles monoculaires.")}
                  </p>
                  <p className="text-xs font-bold leading-relaxed italic text-purple-400">
                      {t('labs.spatial.desc2', 'NEW: La "Génération Modèle 3D" utilise l\'IA Gaussian Splatting pour créer un objet complet manipulable.')}
                  </p>
                  <Button 
                      as={Link}
                  ...
                  >
                      {t('labs.spatial.view_gallery', 'VOIR MA GALERIE 3D')} <ArrowRight className="ml-2 w-3 h-3" />
                  </Button>
                  ...
                  >
                      {t('labs.spatial.discover_dcs', 'DÉCOUVRIR LE DCS (3D VIDÉO)')} <ArrowRight className="ml-2 w-3 h-3" />
                  </Button>
              </Card>
          </div>
              ...
              <div className="bg-black rounded-[4rem] shadow-2xl overflow-hidden min-h-[600px] relative border-4 border-white/5 flex items-center justify-center p-8">
                  {isLoading && (
                      <div className="absolute inset-0 bg-black/80 backdrop-blur-sm z-50 flex flex-col items-center justify-center">
                          ...
                          <span className="text-white font-black italic uppercase tracking-[0.3em] animate-pulse">
                              {model3dMutation.isPending ? t('labs.spatial.generating_3d', 'Génération 3D en cours (peut prendre ~15s)...') : t('labs.spatial.analyzing_depth', 'Analyse IA Profondeur...')}
                          </span>
                      </div>
                  )}
                  ...
                  {!image ? (
                      <div className="text-center opacity-10 animate-pulse">
                          ...
                          <span className="text-2xl font-black italic manga-font uppercase">{t('labs.spatial.no_source', 'Aucune source chargée')}</span>
                      </div>
                  ) : (
                      <div className={`grid ${modelResult ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-2'} gap-8 w-full h-full animate-fade-in`}>
                          ...
                          {modelResult ? (
                              <div className="space-y-4 w-full h-[600px] flex flex-col items-center">
                                  <Badge variant="primary" className="bg-purple-600">{t('labs.spatial.interactive_model', 'Modèle 3D Interactif ({{format}})', { format: modelResult.format })}</Badge>
        ...
        <div className="mt-24 grid grid-cols-1 md:grid-cols-2 gap-8">
            <Card padding="lg" className="bg-white dark:bg-black/40 border-blue-500/20 shadow-[0_0_50px_rgba(59,130,246,0.1)] relative overflow-hidden group">
                ...
                <h4 className="text-xl font-black italic manga-font uppercase mb-4 flex items-center gap-3">
                    <Sparkles className="w-5 h-5 text-blue-600 dark:text-blue-400" /> {t('labs.spatial.guide_title', 'Guide Spatial')}
                </h4>
                <div className="space-y-4 relative z-10">
                    <p className="text-xs font-bold uppercase tracking-wider text-black/60 dark:text-white/60 leading-relaxed">
                        <span className="text-blue-600 dark:text-blue-400">{t('labs.spatial.guide_source_title', 'L\'Image Source :')}</span> {t('labs.spatial.guide_source_desc', 'Chargez n\'importe quelle image 2D (personnage, décor, objet). C\'est le point de départ de toute l\'analyse.')}
                    </p>
                    <p className="text-xs font-bold uppercase tracking-wider text-black/60 dark:text-white/60 leading-relaxed">
                        <span className="text-blue-600 dark:text-blue-400">{t('labs.spatial.guide_depth_title', 'La Profondeur :')}</span> {t('labs.spatial.guide_depth_desc', 'L\'IA estime la distance de chaque pixel et produit une carte de profondeur, comme si elle voyait le relief de l\'image.')}
                    </p>
                    <p className="text-xs font-bold uppercase tracking-wider text-black/60 dark:text-white/60 leading-relaxed">
                        <span className="text-blue-600 dark:text-blue-400">{t('labs.spatial.guide_model_title', 'Le Modèle 3D :')}</span> {t('labs.spatial.guide_model_desc', 'En un clic, l\'image devient un objet 3D complet que vous pouvez faire tourner à la souris et retrouver dans votre galerie.')}
                    </p>
                </div>
            </Card>
            ...
            <div className="p-12 rounded-[4rem] bg-gradient-to-br from-blue-600/10 to-transparent border border-black/5 dark:border-white/5 flex flex-col justify-center text-center">
                <p className="text-sm font-black uppercase tracking-[0.15em] italic leading-relaxed text-blue-800/70 dark:text-blue-200/60">
                    {t('labs.spatial.guide_footer_1', 'Estimation de profondeur monoculaire et reconstruction 3D (maillage GLB, Gaussian Splatting) à partir d\'une seule image.')} <br />
                    {t('labs.spatial.guide_footer_2', 'Le modèle généré est rendu dans un viewer WebGL interactif et archivé dans la galerie 3D.')}
                </p>
            </div>
        </div>
      </div>
