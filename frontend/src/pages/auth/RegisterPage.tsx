...
const RegisterPage: React.FC = () => {
  ...
  // Google is one call for both flows: signInWithPopup creates the account when
  // it does not exist yet. The sign-up page simply never offered it.
  ...
  // Discord/MAL OIDC providers aren't configured yet — keep them disabled (same as LoginPage).
  const socialBtnSoon =
        ...
        <main className="flex-1 flex items-center justify-center p-6 sm:p-10">
          <div className="w-full max-w-md">
            ...
            <h1 className="text-4xl md:text-5xl font-black italic font-manga tracking-tighter uppercase text-[#F4F1E8] leading-none">
              {t('auth.register.title_part1', 'REJOINS')}{' '}
              <span className="text-[#E8442B]">{t('auth.register.title_part2', "L'AVENTURE")}</span>
            </h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="relative flex py-1 items-center">
                ...
                <span className="flex-shrink mx-4 text-[10px] font-black uppercase tracking-widest text-[#8F94A5]">
                  {t('auth.register.social_title', 'Inscriptions sociales')}
                </span>
                <div className="flex-grow border-t border-[#F4F1E8]/10"></div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                <button
                  type="button"
                  aria-label={t('auth.register.google_aria', "S'inscrire avec Google")}
                  title="Google"
                  disabled={isLoading}
                ...
                <button
                  type="button"
                  aria-label={t('auth.login.discord_soon_aria', 'Discord — bientôt disponible')}
                  title={t('auth.login.soon', 'Bientôt disponible')}
                  disabled
                  className={socialBtnSoon}
                >
                ...
                <button
                  type="button"
                  aria-label={t('auth.login.mal_soon_aria', 'MyAnimeList — bientôt disponible')}
                  title={t('auth.login.soon', 'Bientôt disponible')}
                  disabled
                  className={socialBtnSoon}
                >
                  <svg
                    className="w-5 h-5 text-[#2e51a2]"
                    viewBox="0 0 24 24"
                    fill="currentColor"
                    aria-hidden="true"
                  >
