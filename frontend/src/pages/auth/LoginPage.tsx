...
const LoginPage: React.FC = () => {
  ...
  const handleSocialLogin = (provider: () => Promise<unknown>, failMsg: string) => async () => {
    setError('');
    try {
      await provider();
      navigate(from, { replace: true });
      ...
      const error = err as Error;
      setError(error.message || t('auth.loginFailed', failMsg));
    }
  };
        ...
        <main className="flex-1 flex items-center justify-center p-6 sm:p-10">
          <div className="w-full max-w-md animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <h1 className="text-4xl md:text-5xl font-black italic manga-font tracking-tighter uppercase text-black dark:text-white leading-none">
              CONTENT DE TE <span className="text-yellow-400">REVOIR</span>
            </h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="grid grid-cols-4 gap-3">
                <button type="button" aria-label="Se connecter avec Google" title="Google" disabled={isLoading} onClick={handleSocialLogin(loginWithGoogle, 'Connexion Google échouée.')} className={socialBtn}>
                ...
                <button type="button" aria-label="Se connecter avec Discord" title="Discord" disabled={isLoading} onClick={handleSocialLogin(loginWithDiscord, 'Connexion Discord échouée.')} className={socialBtn}>
                ...
                <button type="button" aria-label="Se connecter avec X" title="X" disabled={isLoading} onClick={handleSocialLogin(loginWithX, 'Connexion X échouée.')} className={socialBtn}>
                  <svg className="w-5 h-5 text-black dark:text-white" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                  </svg>
                </button>

                <button type="button" aria-label="Se connecter avec MyAnimeList" title="MyAnimeList" disabled={isLoading} onClick={handleSocialLogin(loginWithMyAnimeList, 'Connexion MyAnimeList échouée.')} className={socialBtn}>
