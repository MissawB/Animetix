  ...
  const forceSync = async () => {
    ...
    try {
      ...
      if (response && response.status === 'success') {
        ...
        throw new Error(response?.error || t('social.offline.unknown_error', 'Une erreur inconnue est survenue.'));
      }
    } catch (err) {
      const message = err instanceof Error ? err.message : '';
      setError(message || t('social.offline.connection_error', 'Erreur de connexion avec le serveur de synchronisation.'));
    } finally {
      setIsSyncing(false);
    }
  };
      ...
      <div className="max-w-4xl mx-auto px-6 py-16">
        ...
        <header className="mb-12">
          <Link to="/social/dashboard/" className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-widest opacity-40 hover:opacity-100 transition-opacity mb-8 no-underline text-current">
            <ArrowLeft className="w-4 h-4" /> {t('social.offline.back_dashboard', 'Retour au Dashboard')}
          </Link>
          <h1 className="text-5xl md:text-7xl font-black italic manga-font tracking-tighter uppercase mb-2">
            {t('social.offline.title_prefix', 'SYNCHRONISATION')} <span className="text-yellow-400 text-glow">{t('social.offline.title_accent', 'HORS-LIGNE')}</span>
          </h1>
          <p className="text-lg font-bold opacity-30 uppercase tracking-[0.2em]">
            {t('social.offline.subtitle', 'Visualisez et réconciliez vos sessions de jeu locales.')}
          </p>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          <Card padding="md" className="flex items-center justify-between bg-[#1a1a2e]/60 border-white/5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[10px] font-black uppercase opacity-40 m-0">{t('social.offline.network_status', 'Statut Réseau')}</p>
                <p className="text-sm font-bold m-0">
                  {activeOnline ? t('social.offline.online', 'En ligne (Connecté)') : t('social.offline.offline_label', 'Hors-ligne')}
                </p>
              </div>
            </div>
            <Button
            ...
            >
              {simulateOffline ? t('social.offline.reconnect', 'Reconnecter') : t('social.offline.simulate_offline', 'Simuler Offline')}
            </Button>
          </Card>
          ...
          <Card padding="md" className="flex items-center gap-4 bg-[#1a1a2e]/60 border-white/5">
            ...
            <div>
              <p className="text-[10px] font-black uppercase opacity-40 m-0">{t('social.offline.queue', 'File d\'attente')}</p>
              <p className="text-sm font-bold m-0">
                {t('social.offline.pending_count', '{{count}} partie(s) en attente', { count: offlineGames.length })}
              </p>
            </div>
          </Card>
          ...
          <Card padding="md" className="flex items-center gap-4 bg-[#1a1a2e]/60 border-white/5">
            ...
            <div>
              <p className="text-[10px] font-black uppercase opacity-40 m-0">{t('social.offline.daily_max', 'Gain maximum quotidien')}</p>
              <p className="text-sm font-bold m-0">{t('social.offline.daily_xp', '200 XP par jour')}</p>
            </div>
          </Card>
        </div>
        ...
        <div className="flex flex-wrap gap-4 justify-between items-center mb-10 bg-white/5 dark:bg-black/20 p-6 rounded-3xl border border-white/5">
          <div className="flex gap-4">
            <Button onClick={addSimulatedGame} className="gap-2">
              <Plus className="w-4 h-4" /> {t('social.offline.simulate_game', 'Simuler Partie Locale')}
            </Button>
            {offlineGames.length > 0 && (
              <Button variant="danger" onClick={clearQueue} className="gap-2">
                <Trash2 className="w-4 h-4" /> {t('social.offline.clear_queue', 'Vider la file')}
              </Button>
            )}
          </div>
          ...
          >
            ...
            {isSyncing ? t('social.offline.syncing', 'Synchronisation...') : t('social.offline.force_sync', 'Forcer la réconciliation')}
          </Button>
        </div>
        ...
        {syncResult && (
          <div className="p-6 bg-green-500/10 border border-green-500/30 text-green-500 rounded-2xl mb-10">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="text-lg font-black uppercase italic m-0">{t('social.offline.sync_success', 'Synchronisation Réussie !')}</h3>
            </div>
            <div className="grid grid-cols-3 gap-4 text-center mt-2">
              <div className="bg-green-500/5 p-4 rounded-xl border border-green-500/10">
                <p className="text-[10px] uppercase font-bold opacity-60 m-0">{t('social.offline.synced_items', 'Parties synchronisées')}</p>
                <p className="text-2xl font-black italic manga-font m-0">{syncResult.synced_items}</p>
              </div>
              <div className="bg-green-500/5 p-4 rounded-xl border border-green-500/10">
                <p className="text-[10px] uppercase font-bold opacity-60 m-0">{t('social.offline.xp_won', 'XP Remportés')}</p>
                <p className="text-2xl font-black italic manga-font m-0">+{syncResult.xp_gained} XP</p>
              </div>
              <div className="bg-green-500/5 p-4 rounded-xl border border-green-500/10">
                <p className="text-[10px] uppercase font-bold opacity-60 m-0">{t('social.offline.daily_total', 'Cumul quotidien')}</p>
        ...
        <Card padding="none" className="bg-[#1a1a2e]/30 border-white/5 overflow-hidden">
          <div className="p-6 border-b border-white/5">
            <h2 className="text-lg font-black uppercase italic m-0">{t('social.offline.pending_table_title', 'Parties en attente de synchronisation')}</h2>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-white/5 bg-white/5">
                  <th className="p-4 text-[10px] uppercase font-black tracking-wider opacity-60">{t('social.offline.th_game_mode', 'Mode de Jeu')}</th>
                  <th className="p-4 text-[10px] uppercase font-black tracking-wider opacity-60">{t('social.offline.th_media_type', 'Type Média')}</th>
                  <th className="p-4 text-[10px] uppercase font-black tracking-wider opacity-60">{t('common.score', 'Score')}</th>
                  <th className="p-4 text-[10px] uppercase font-black tracking-wider opacity-60">{t('social.offline.th_attempts', 'Tentatives')}</th>
                  <th className="p-4 text-[10px] uppercase font-black tracking-wider opacity-60">{t('social.offline.th_xp_estimated', 'XP Estimés')}</th>
                </tr>
              </thead>
              <tbody>
                ...
                {offlineGames.length === 0 && (
                  <tr>
                    <td colSpan={5} className="p-16 text-center opacity-30 text-xs uppercase font-bold tracking-widest">
                      {t('social.offline.empty_table', 'Aucune partie en attente. Utilisez le bouton "Simuler Partie Locale" pour en ajouter.')}
                    </td>
                  </tr>
                )}
              </tbody>
