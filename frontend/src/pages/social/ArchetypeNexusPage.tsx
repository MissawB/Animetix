...
const ArchetypeNexusPage: React.FC = () => {
  ...
  if (isError || !data) return (
    <div className="min-h-screen w-full bg-[#0a0a12] flex flex-col items-center justify-center text-center">
        <h2 className="text-3xl font-black text-red-500 italic uppercase">{t('social.nexus.cognitive_failure', 'Défaillance Cognitive')}</h2>
        <p className="text-white/40 mt-4 uppercase font-bold tracking-widest">{t('social.nexus.sync_error', 'Impossible de synchroniser le profil neuronal.')}</p>
    </div>
  );
      ...
      <AnimatedPage>
        ...
        <header className="mb-16 relative">
            ...
            <p className="text-xl font-bold opacity-30 uppercase tracking-[0.3em] max-w-2xl leading-relaxed">
                {t('social.nexus.subtitle', 'Visualisation de votre empreinte sémantique et de vos biais narratifs déduits par l\'IA.')}
            </p>
        </header>
            ...
            <div className="lg:col-span-4 space-y-8">
                <Card padding="none" className="bg-black border-white/10 overflow-hidden rounded-[3rem] relative group shadow-2xl">
                    ...
                    <div className="relative z-10 p-10 text-center">
                        ...
                        <h2 className="text-4xl font-black italic manga-font uppercase mb-2 tracking-tighter">{t('social.nexus.master_archetype', 'Archétype Maître')}</h2>
                        <p className="text-[10px] font-bold opacity-40 uppercase tracking-widest leading-relaxed">
                            {t('social.nexus.aura_intensity', 'Aura: {{type}} ({{percent}}% d\'intensité)', { type: archetype.aura_type, percent: Math.round(archetype.intensity * 100) })}
                        </p>
                    </div>

                    <div className="border-t border-white/5 bg-white/5 p-8 grid grid-cols-2 gap-4">
                        <div className="text-center">
                            <p className="text-[8px] font-black opacity-30 uppercase mb-1">{t('social.nexus.stability', 'Stabilité')}</p>
                            <p className="text-xl font-black italic text-blue-500">92%</p>
                        </div>
                        <div className="text-center border-l border-white/5">
                            <p className="text-[8px] font-black opacity-30 uppercase mb-1">{t('social.nexus.convergence', 'Convergence')}</p>
                            <p className="text-xl font-black italic text-emerald-500">{t('social.nexus.convergence_high', 'HAUTE')}</p>
                        </div>
                    </div>
                </Card>
                ...
                <Card padding="lg" className="bg-navy-900/40 border-white/5">
                    <h3 className="text-xs font-black uppercase opacity-40 mb-8 tracking-widest flex items-center gap-2">
                        <BarChart3 className="w-4 h-4 text-blue-500" /> {t('social.nexus.neural_profile', 'Profil Neuronal')}
                    </h3>
                    <div className="space-y-6">
                        <StatBar label={t('social.nexus.shonen_affinity', 'Affinité Shonen')} value={cognitive_stats.shonen_affinity} color="bg-orange-500" />
                        <StatBar label={t('social.nexus.seinen_affinity', 'Complexité Seinen')} value={cognitive_stats.seinen_affinity} color="bg-purple-500" />
                        <StatBar label={t('social.nexus.logic_consistency', 'Cohérence Logique')} value={cognitive_stats.logic_consistency} color="bg-emerald-500" />
                        <StatBar label={t('social.nexus.memory_depth', 'Profondeur Mémoire')} value={Math.min(cognitive_stats.memory_depth / 50, 1)} color="bg-blue-500" />
                    </div>
                </Card>
            </div>
                    ...
                    <div className="flex-grow min-h-[300px] relative">
                        {drift_history && drift_history.length > 0 ? (
                            ...
                            <div className="absolute inset-0 flex flex-col items-center justify-center text-center opacity-20 border-2 border-dashed border-white/5 rounded-3xl">
                                ...
                                <p className="text-[10px] font-black uppercase tracking-widest">{t('social.nexus.insufficient_history', 'Historique insuffisant pour projection')}</p>
                            </div>
                        )}
                    </div>
                </Card>
                ...
                <Card padding="lg" className="bg-black border-blue-500/20 shadow-[0_0_40px_rgba(59,130,246,0.1)]">
                    <header className="flex justify-between items-start mb-6">
                        <div>
                            ...
                            <p className="text-[10px] font-bold opacity-30 uppercase tracking-widest">{t('social.nexus.z3_desc', 'Contraintes formelles déduites de vos interactions.')}</p>
                        </div>
                        <div className="flex gap-2">
                            ...
                            >
                                {t('social.nexus.manage', 'Gérer')} <ChevronRight className="ml-1 w-3 h-3" />
                            </Button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-1 gap-8">
             ...
             <Card padding="lg" className="bg-navy-900/40 border-white/5">
                    <h3 className="text-xs font-black uppercase opacity-40 mb-10 tracking-widest flex items-center gap-2">
                        <Clock className="w-4 h-4 text-emerald-500" /> {t('social.nexus.episodic_memory', 'Signaux de Mémoire Épisodique')}
                    </h3>
                                ...
                                <div className="flex-grow pt-2">
                                    <div className="flex justify-between items-center mb-1">
                                        <p className={`text-[10px] font-black uppercase tracking-widest ${sig.is_positive ? 'text-emerald-400' : 'text-red-400'}`}>
                                            {sig.is_positive ? t('social.nexus.positive_signal', 'SIGNAL POSITIF') : t('social.nexus.negative_signal', 'SIGNAL NÉGATIF')}
                                        </p>
        ...
        <div className="mt-24 p-12 rounded-[4rem] bg-gradient-to-br from-blue-600/10 to-transparent border border-white/5 text-center">
            <p className="text-[10px] font-black uppercase tracking-[0.4em] opacity-30 italic">
                {t('social.nexus.warning_1', 'Avertissement : Les déductions neuro-symboliques sont basées sur des modèles stochastiques résolus en temps réel.')} <br />
                {t('social.nexus.warning_2', "Le drift d'archétype est recalculé après chaque session de forge ou de débat.")}
            </p>
        </div>
        </div>
      </AnimatedPage>
