...
const MarketWikiPage: React.FC = () => {
  ...
  const { data, isLoading } = useQuery<MarketData>({
    ...
    queryFn: () => exploreService.getMarketWiki(),
  });
      ...
      <div className="min-h-screen bg-[#05050a] text-white p-6 max-w-7xl mx-auto py-16">
        <header className="mb-12 text-center md:text-left">
          ...
          <p className="text-white/60 text-lg max-w-2xl">
            Explorez l'écosystème industriel du manga et de l'animation. Données structurées sur les
            éditeurs historiques, les diffuseurs majeurs et les comités de production.
          </p>
        </header>
            ...
            {isLoading ? (
              ...
              filteredEntities.map(([name, details]) => (
                ...
                >
                  <Card
                    padding="none"
                    className="group overflow-hidden bg-navy-900/40 border-white/5 hover:border-anime-accent/50 transition-all h-full flex flex-col"
                  >
                    <div className="p-6 border-b border-white/5 bg-white/[0.02]">
                      ...
                      <p className="text-xs text-white/40 font-bold uppercase tracking-widest leading-tight">
                        {activeCategory === 'publishers'
                          ? "Maison d'Édition"
                          : 'Diffuseur / Production'}
                      </p>
                    </div>

                    <div className="p-6 space-y-5 flex-grow">
                      <div className="space-y-1.5">
                        <span className="text-[10px] font-black text-anime-accent uppercase tracking-widest block">
                          Définition
                        </span>
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="text-[11px] text-white/60 bg-black/40 p-3 rounded-xl border border-white/5 leading-relaxed">
                          {details.examples.split(', ').map((ex, idx) => (
                            <span
                              key={idx}
                              className={`${idx > 0 ? 'before:content-["•"] before:mx-2 before:opacity-20' : ''}`}
                            >
                              {ex.replace(/\*/g, '')}
                            </span>
                          ))}
              ...
              <div className="col-span-full py-20 text-center">
                ...
                <p className="text-white/30 font-black italic uppercase tracking-widest">
