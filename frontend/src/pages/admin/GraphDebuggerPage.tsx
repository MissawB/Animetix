...
const GraphDebuggerPage: React.FC = () => {
  ...
  const {
    ...
    queryFn: () => graphService.getDebugger(),
  });

  const cleanupMutation = useMutation({
    mutationFn: () => graphService.postDebugger({ action: 'cleanup' }),
    onSuccess: () => refetch(),
  });

  const healMutation = useMutation({
    mutationFn: (mediaId: string) =>
      graphService.postDebugger({ action: 'heal', media_id: mediaId }),
    onSuccess: () => {
      setHealingId('');
      refetch();
    },
  ...
  const mergeDuplicatesMutation = useMutation({
    mutationFn: () => graphService.postDebugger({ action: 'merge_duplicates' }),
    onSuccess: () => refetch(),
  });
    ...
    return (
      <div className="max-w-7xl mx-auto px-6 py-32 flex flex-col items-center justify-center">
        ...
        <p className="text-sm font-black uppercase tracking-[0.3em] animate-pulse opacity-40">
          {t('admin.graph.loading', 'Auditing Knowledge Graph...')}
        </p>
      </div>
    );
        ...
        <div className="max-w-7xl mx-auto px-6 py-12">
          ...
          <header className="mb-16 relative text-black dark:text-white">
            ...
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-black/5 dark:border-white/10 text-[10px] font-black uppercase tracking-widest text-purple-500 mb-4">
              ...
              {t('admin.graph.protocol', 'Lore Consistency Protocol')}
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8 mb-16 text-black dark:text-white">
            <Card
              padding="lg"
              className="bg-white dark:bg-[#0f0f1a] border-none shadow-xl relative overflow-hidden group"
            >
              <div className="absolute -right-4 -bottom-4 opacity-5 group-hover:scale-110 transition-transform">
                <AlertTriangle className="w-32 h-32" />
              </div>
              <p className="text-[10px] font-black uppercase opacity-40 mb-4 tracking-widest">
                {t('admin.graph.isolated', 'Isolated Nodes')}
              </p>
              <div className="flex items-baseline gap-2">
                <span className="text-6xl font-black italic manga-font">
                  {audit.isolated_nodes}
                </span>
                <span className="text-xs font-bold opacity-30 uppercase">
                  {t('admin.graph.entities', 'Entities')}
                </span>
              </div>
              <p className="text-[10px] font-bold opacity-30 mt-4 leading-relaxed uppercase">
                {t(
                  'admin.graph.isolated_desc',
                  'Nœuds sans aucune relation sémantique ou temporelle.',
                )}
              </p>
            </Card>
            ...
            >
              ...
              <p className="text-[10px] font-black uppercase opacity-40 mb-4 tracking-widest text-red-500">
                {t('admin.graph.conflicts', 'Lore Conflicts')}
              </p>
              <div className="flex items-baseline gap-2">
                <span className="text-6xl font-black italic manga-font text-red-500">
                  {audit.temporal_conflicts}
                </span>
                <span className="text-xs font-bold opacity-30 uppercase">
                  {t('admin.graph.detected', 'Detected')}
                </span>
              </div>
              <p className="text-[10px] font-bold opacity-30 mt-4 leading-relaxed uppercase">
                {t(
                  'admin.graph.conflicts_desc',
                  "Contradictions temporelles (ex: suites antérieures à l'original).",
                )}
              </p>
            </Card>
            ...
            >
              ...
              <p className="text-[10px] font-black uppercase opacity-40 mb-4 tracking-widest text-blue-500">
                {t('admin.graph.orphans', 'Orphan Entities')}
              </p>
              <div className="flex items-baseline gap-2">
                <span className="text-6xl font-black italic manga-font text-blue-500">
                  {audit.orphan_entities}
                </span>
                <span className="text-xs font-bold opacity-30 uppercase">
                  {t('admin.graph.entities', 'Entities')}
                </span>
              ...
              <p className="text-[10px] font-bold opacity-30 mt-4 leading-relaxed uppercase">
                {t(
                  'admin.graph.orphan_desc',
                  'Entités extraites non rattachées à un média parent.',
                )}
              </p>
            </Card>
            ...
            >
              ...
              <p className="text-[10px] font-black uppercase opacity-40 mb-4 tracking-widest text-purple-500">
                {t('admin.graph.duplicates', 'Duplicate Entities')}
              </p>
              <div className="flex items-baseline gap-2">
                <span className="text-6xl font-black italic manga-font text-purple-500">
                  {audit.duplicate_entities ?? 0}
                </span>
                <span className="text-xs font-bold opacity-30 uppercase">
                  {t('admin.graph.entities', 'Entities')}
                </span>
              ...
              <p className="text-[10px] font-bold opacity-30 mt-4 leading-relaxed uppercase">
                {t('admin.graph.duplicate_desc', 'Entités doublons possédant le même nom/titre.')}
              </p>
            </Card>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 text-black dark:text-white">
            ...
            <div className="lg:col-span-4 space-y-8">
              <Card padding="lg" className="bg-purple-600 text-white border-none shadow-2xl">
                <h3 className="text-2xl font-black italic manga-font uppercase mb-6 flex items-center gap-3">
                  <ShieldCheck className="w-8 h-8" /> {t('admin.graph.auto_heal', 'Auto-Heal')}
                </h3>
                <p className="text-sm font-bold opacity-90 leading-relaxed uppercase mb-8">
                  {t(
                    'admin.graph.cleanup_desc',
                ...
                >
                  {cleanupMutation.isPending ? (
                    ...
                    t('admin.graph.cleanup_btn', 'EXECUTE CLEANUP')
                  )}
                </Button>
              </Card>
              ...
              <Card padding="lg" className="bg-indigo-600 text-white border-none shadow-2xl">
                <h3 className="text-2xl font-black italic manga-font uppercase mb-6 flex items-center gap-3">
                  <GitMerge className="w-8 h-8" /> {t('admin.graph.merge', 'Merge Duplicates')}
                </h3>
                <p className="text-sm font-bold opacity-90 leading-relaxed uppercase mb-8">
                  {t(
                    'admin.graph.merge_desc',
            ...
            <div className="lg:col-span-8">
              <Card
                padding="none"
                className="bg-white dark:bg-[#0f0f1a] border-none rounded-[3rem] overflow-hidden shadow-2xl"
              >
                <header className="px-12 py-8 border-b border-black/5 dark:border-white/5 flex justify-between items-center text-black dark:text-white">
                  ...
                  <Badge
                    variant="neutral"
                    className="bg-red-500/10 text-red-500 border-none uppercase font-black italic"
                  >
                    TOP 5
                  </Badge>
                </header>
                ...
                <div className="p-8">
                  {audit.details && audit.details.length > 0 ? (
                    <div className="space-y-4">
                      {audit.details.map(
                        (
                          conflict: { t1: string; y1: number; t2: string; y2: number },
                          i: number,
                        ) => (
                          <div
                            key={i}
                            className="group p-6 bg-gray-50 dark:bg-black/20 rounded-[2rem] border border-black/5 dark:border-white/5 hover:border-red-500/30 transition-all text-black dark:text-white"
                          >
                            <div className="flex items-center justify-between gap-6">
                              <div className="flex-1">
                                <div className="flex items-center gap-3 mb-2">
                                  <span className="text-sm font-black italic uppercase text-red-500">
                                    {conflict.t1}
                                  </span>
                                  <Badge
                                    variant="neutral"
                                    className="bg-black/5 dark:bg-white/5 border-none text-[8px] text-black dark:text-white"
                                  >
                                    {conflict.y1}
                                  </Badge>
                                </div>
                    ...
                    <div className="py-24 text-center text-black dark:text-white">
                      ...
                      <h4 className="text-2xl font-black italic manga-font uppercase opacity-20">
