...
const FinancialDashboardPage: React.FC = () => {
  ...
  const [bannerCpm, setBannerCpm] = useState<number>(1.0);
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
              ...
              <div className="flex justify-between items-start">
                <div>
                  <p className="text-[10px] font-black uppercase tracking-wider text-gray-500 mb-1">
                    {t('admin.financials.total_ai_cost', 'Coût Total IA')}
                  </p>
                  <h3 className="text-3xl font-black text-red-500">
                    ${data.total_ai_cost.toFixed(2)}
                  </h3>
                </div>
                <Coins className="text-red-500 w-5 h-5" />
              </div>
              <p className="text-[10px] text-gray-400 mt-3 font-mono">
                {t('admin.financials.cumulative_consumption', 'Consommation cumulée des API')}
              </p>
            </Card>
            ...
            <Card padding="md" className="border-2 border-green-500/20 bg-white dark:bg-[#0f0f1a]">
              <div className="flex justify-between items-start">
                <div>
                  <p className="text-[10px] font-black uppercase tracking-wider text-gray-500 mb-1">
                    {t('admin.financials.simulated_ad_revenue', 'Revenus Pubs (Simulés)')}
                  </p>
              ...
              <p className="text-[10px] text-gray-400 mt-3 font-mono">
                {t('admin.financials.ad_stats_summary', '{{videos}} videos • {{clicks}} clics', {
                  videos: data.ad_stats.video_impressions,
                  clicks: data.ad_stats.clicks,
                })}
              </p>
            </Card>
            ...
            <Card padding="md" className="border-2 border-yellow-500/20 bg-white dark:bg-[#0f0f1a]">
              <div className="flex justify-between items-start">
                <div>
                  <p className="text-[10px] font-black uppercase tracking-wider text-gray-500 mb-1">
                    {t('admin.financials.donations_gold_sponsors', 'Dons (Sponsors Or)')}
                  </p>
                  <h3 className="text-3xl font-black text-yellow-500">
                    ${simDonations.toFixed(2)}
                  </h3>
                </div>
                <Sparkles className="text-yellow-500 w-5 h-5" />
              </div>
              <p className="text-[10px] text-gray-400 mt-3 font-mono">
                {t('admin.financials.active_gold_sponsors', '{{count}} sponsors or actifs', {
                  count: data.donation_stats.gold_sponsors,
                })}
              </p>
            </Card>
            ...
            <Card
              padding="md"
              className={`border-2 bg-white dark:bg-[#0f0f1a] ${isDeficit ? 'border-red-500' : 'border-green-500'}`}
            >
              <div className="flex justify-between items-start">
                <div>
                  <p className="text-[10px] font-black uppercase tracking-wider text-gray-500 mb-1">
                    {t('admin.financials.simulated_net_margin', 'Solde Net (Simulation)')}
                  </p>
                  <h3
                    className={`text-3xl font-black ${isDeficit ? 'text-red-500' : 'text-green-500'}`}
                  >
                    {isDeficit ? '-' : '+'}${deficitVal.toFixed(2)}
                  </h3>
                </div>
                <div
                  className={`px-2 py-0.5 rounded text-[8px] font-black uppercase ${isDeficit ? 'bg-red-500/10 text-red-500 border border-red-500/30' : 'bg-green-500/10 text-green-500 border border-green-500/30'}`}
                >
                  {isDeficit
                    ? t('admin.financials.deficit', 'Déficit')
                    : t('admin.financials.profit', 'Bénéfice')}
                </div>
              </div>
              <p className="text-[10px] text-gray-400 mt-3 font-mono">
                {t('admin.financials.margin_explanation', 'Total revenus - Coûts IA')}
              </p>
            </Card>
          </div>
            ...
            <div className="lg:col-span-2 space-y-6">
              <Card padding="lg" className="bg-white dark:bg-[#0f0f1a]">
                <h3 className="text-xl font-black italic uppercase text-black dark:text-white mb-6 flex items-center gap-2">
                  ...
                  {t(
                    'admin.financials.interactive_equalizer_title',
                    "CONSOLE D'ÉGALISATION INTERACTIVE",
                  )}
                </h3>
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    <div className="flex justify-between text-xs font-bold uppercase tracking-wider text-gray-500 mb-2">
                      <span>
                        {t('admin.financials.video_cpm_label', 'CPM Vidéo (pour 1000 vues)')}
                      </span>
                  ...
                  <div>
                    <div className="flex justify-between text-xs font-bold uppercase tracking-wider text-gray-500 mb-2">
                      <span>
                        {t('admin.financials.banner_cpm_label', 'CPM Bannière (pour 1000 vues)')}
                      </span>
                  ...
                  <div>
                    <div className="flex justify-between text-xs font-bold uppercase tracking-wider text-gray-500 mb-2">
                      <span>
                        {t('admin.financials.donation_val_label', 'Valeur Don Unique (Sponsor Or)')}
                      </span>
            ...
            <div className="space-y-6">
              <Card padding="lg" className="bg-[#0b0c15] text-white border border-blue-500/20">
                <h3 className="text-md font-black italic uppercase text-yellow-500 mb-4 flex items-center gap-1.5">
                  ...
                  {t('admin.financials.breakeven_title', 'SEUIL DE RENTABILITÉ')}
                </h3>
                ...
                {isDeficit ? (
                  <div className="space-y-4">
                    <p className="text-xs text-gray-300 leading-relaxed font-mono">
                      {t(
                        'admin.financials.deficit_recom_desc',
                        'Pour compenser le déficit simulé de {{deficit}} avec les taux sélectionnés, le site doit générer au choix :',
                        { deficit: deficitVal.toFixed(2) },
                      )}
                    </p>
                    <div className="bg-black/40 p-4 rounded-xl border border-white/5 space-y-3 font-mono">
                      <div className="flex justify-between text-xs">
                        <span className="text-gray-400">
                          {t('admin.financials.video_views_needed', 'Vues Vidéo Recom. :')}
                        </span>
                        <span className="text-yellow-400 font-bold">
                          {neededVideoImpressions.toLocaleString()}
                        </span>
                      </div>
                      <div className="flex justify-between text-xs">
                        <span className="text-gray-400">
                          {t('admin.financials.ad_clicks_needed', 'OU Clics Pubs :')}
                        </span>
                        <span className="text-yellow-400 font-bold">
                          {neededClicks.toLocaleString()}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-green-500/10 border border-green-500/30 p-4 rounded-xl text-green-400 text-xs leading-relaxed font-mono font-bold">
                    {t(
                      'admin.financials.profit_message',
                      "Félicitations ! Les paramètres actuels permettent de dégager un bénéfice de {{profit}}. Aucun trafic publicitaire supplémentaire n'est nécessaire pour combler les coûts.",
                      { profit: deficitVal.toFixed(2) },
                    )}
                  </div>
                )}
              </Card>
              ...
              <Card padding="lg" className="bg-white dark:bg-[#0f0f1a]">
                <h3 className="text-md font-black italic uppercase text-blue-500 mb-4 flex items-center gap-1.5">
                  ...
                  {t('admin.financials.recommendation_title', 'RECOMMANDATION GLOBALE')}
                </h3>
            ...
            <div className="space-y-4">
              {Object.entries(data.cost_by_engine).length === 0 ? (
                <p className="text-xs text-gray-500 font-mono">
                  {t('admin.financials.no_costs_recorded', "Aucun coût d'IA enregistré.")}
                </p>
              ) : (
