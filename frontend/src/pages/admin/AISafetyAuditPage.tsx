...
const AISafetyAuditPage: React.FC = () => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useQuery<AISafetyEvent[]>({
    queryKey: ['safety-events'],
    queryFn: () => mlopsService.getSafetyEvents(),
  });
        ...
        <div className="max-w-7xl mx-auto px-6 py-12 relative z-10">
          ...
          <header className="mb-12 flex justify-between items-end">
            <div>
              ...
              <p className="text-lg font-bold opacity-30 uppercase tracking-[0.3em]">
                {t('admin.safety.subtitle', 'Surveillance des Guardrails et Détection de Menaces.')}
              </p>
            </div>
            <Button
              onClick={() => refetch()}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <div className="lg:col-span-7 space-y-4">
              <Card
                padding="none"
                className="bg-white dark:bg-navy-950/40 border-black/5 dark:border-white/5 rounded-[2.5rem] overflow-hidden shadow-xl"
              >
                <header className="px-8 py-6 border-b border-black/5 dark:border-white/5 flex justify-between items-center bg-gray-50/50 dark:bg-white/[0.02]">
                  ...
                  <div className="flex gap-2">
                    <Badge
                      variant="neutral"
                      className="bg-black/5 dark:bg-white/5 border-none text-[8px] italic"
                    >
                      REAL_TIME_FILTERING
                    </Badge>
                  </div>
                  ...
                  {isLoading ? (
                    ...
                    events.map((event) => (
                      <div
                        ...
                        tabIndex={0}
                        aria-label={t(
                          'admin.safety.event_details_aria',
                          "Détails de l'événement {{id}}",
                          { id: event.id },
                        )}
                      ...
                      >
                        <div
                          className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 border ${getActionColor(event.action)} shadow-lg shadow-current/10`}
                        >
                          {event.action === 'block' ? (
                        ...
                        <div className="flex-grow min-w-0">
                          <div className="flex justify-between items-center mb-2">
                            <div className="flex items-center gap-3">
                              ...
                              <Badge
                                variant="neutral"
                                className="bg-black/5 dark:bg-white/10 text-black dark:text-white border-none text-[8px] font-black uppercase tracking-widest"
                              >
                                {event.event_type}
                              </Badge>
                            </div>
                          ...
                          <div className="flex flex-wrap gap-2">
                            {event.detected_categories.map((cat, idx) => (
                              <span
                                key={idx}
                                className="text-[8px] font-black uppercase tracking-tighter text-red-500/60"
                              >
                                {cat}
                              </span>
                            ))}
              ...
              <AnimatePresence mode="wait">
                {selectedEvent ? (
                  ...
                  >
                    <Card
                      padding="lg"
                      className="bg-white dark:bg-navy-950 border-2 border-red-500/20 rounded-[3rem] shadow-2xl relative overflow-hidden"
                    >
                      ...
                      <header className="mb-10 flex justify-between items-start">
                        <div>
                          <Badge
                            className={`${getActionColor(selectedEvent.action)} border-none uppercase font-black italic px-4 py-1.5 mb-4`}
                          >
                            Action: {selectedEvent.action.toUpperCase()}
                          </Badge>
                          <h2 className="text-4xl font-black italic manga-font uppercase tracking-tighter text-black dark:text-white">
                            Detailed Report
                          </h2>
                        </div>
                        <button
                          onClick={() => setSelectedEvent(null)}
                          className="p-2 hover:bg-black/5 dark:hover:bg-white/5 rounded-xl transition-all text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white"
                      ...
                      <div className="space-y-8 relative z-10">
                        ...
                        <section className="p-6 bg-gray-50 dark:bg-white/[0.03] rounded-3xl border border-black/5 dark:border-white/5">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <span className="text-sm font-bold uppercase tracking-wider text-black dark:text-white/80">
                              {selectedEvent.username || 'Guest / Anonymous'}
                            </span>
                          </div>
                        </section>
                        ...
                        <section className="space-y-4">
                          ...
                          <div className="p-6 bg-red-500/[0.03] dark:bg-red-500/[0.07] rounded-3xl border border-red-500/10 text-sm font-medium text-black dark:text-white/60 leading-relaxed italic">
                            {selectedEvent.reasoning ||
                              'No detailed reasoning provided by the safety engine.'}
                          </div>
                        </section>
                      </div>
                    </Card>
                  ...
                  <div className="h-[600px] flex flex-col items-center justify-center opacity-10 text-center border-4 border-dashed border-black/5 dark:border-white/5 rounded-[4rem]">
                    ...
                    <p className="text-sm font-bold uppercase tracking-[0.3em]">
                      Select an event to initiate deep audit.
                    </p>
                  </div>
                )}
              </AnimatePresence>
            ...
            {[
              {
                ...
                value: events?.filter((e) => e.action === 'block').length || 0,
                color: 'text-red-500',
              },
              {
                label: 'Guardrail Warnings',
                value: events?.filter((e) => e.action === 'warn').length || 0,
                color: 'text-amber-500',
              },
              {
                label: 'Auto-Rewrites',
                value: events?.filter((e) => e.action === 'rewrite').length || 0,
                color: 'text-blue-500',
              },
              {
                label: 'Safe Interactions',
                value: events?.filter((e) => e.action === 'none').length || 0,
              ...
              <Card
                key={i}
                padding="md"
                className="bg-white dark:bg-navy-950 border-none shadow-lg"
              >
                <p className="text-[8px] font-black uppercase opacity-30 tracking-[0.2em] mb-1">
                  {stat.label}
                </p>
                <p className={`text-4xl font-black italic manga-font ${stat.color}`}>
                  {stat.value}
                </p>
              </Card>
            ))}
