...
const AdminDSPyDashboard: React.FC = () => {
  ...
  const [template, setTemplate] = useState<string>(
    t(
      'admin.dspy.default_template',
      "Réponds à la question suivante sur l'univers de l'anime : {query}",
    ),
  );
  ...
  const { data: failures, isLoading: isLoadingFailures } = useQuery<EvalFailure[]>({
    ...
    queryFn: () => mlopsService.getEvalFailures(),
  });
  ...
  const optimizeMutation = useMutation({
    mutationFn: async () => {
      ...
      return mlopsService.runDSPyOptimizer({ template, dataset });
    },
    onSuccess: (data) => {
      setResult(data);
    },
        ...
        <div className="max-w-7xl mx-auto px-6 py-12">
          ...
          <header className="mb-12 flex justify-between items-end text-black dark:text-white">
            <div>
              ...
              <p className="text-lg font-bold opacity-30 uppercase tracking-[0.3em]">
                {t('admin.dspy.subtitle', 'Auto-tuning des prompts par mutation sémantique.')}
              </p>
            </div>
            <div className="flex gap-4">
              <Badge
                variant="neutral"
                className="bg-white/5 border-black/5 dark:border-white/10 text-[10px] py-2 px-4 italic font-black text-black dark:text-white"
              >
                PHASE G.1 : SEMANTIC MUTATION
              </Badge>
            </div>
          </header>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 text-black dark:text-white">
            ...
            <div className="lg:col-span-5 space-y-8">
              ...
              <Card
                padding="lg"
                className="bg-white dark:bg-[#0f0f1a] border-none shadow-2xl rounded-[3rem]"
              >
                <h3 className="text-xs font-black uppercase opacity-40 mb-8 tracking-widest flex items-center gap-2">
                  <Settings className="w-4 h-4 text-blue-500" />{' '}
                  {t('admin.dspy.config_title', "Configuration de l'Optimiseur")}
                </h3>
                ...
                <form onSubmit={onSubmit} className="space-y-6">
                  <div>
                    <label
                      ...
                      className="text-[10px] font-black uppercase opacity-30 mb-2 block tracking-widest"
                    >
                      {t('admin.dspy.original_template', "Template d'origine")}
                    </label>
                    <textarea
                      id="template-editor"
                      ...
                      placeholder={t(
                        'admin.dspy.template_placeholder',
                        'Entrez le template avec le placeholder {query}...',
                      )}
                    />
                  </div>
                    ...
                    <div className="p-4 bg-red-500/5 border border-red-500/20 rounded-2xl relative group">
                      <div className="flex justify-between items-start mb-2">
                        <span className="text-[8px] font-black uppercase text-red-500 tracking-widest flex items-center gap-1">
                          <ShieldAlert className="w-2 h-2" /> Seeded from Failure #
                          {selectedFailure.id}
                        </span>
                        <button
                          onClick={() => setSelectedFailure(null)}
                          className="text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-300 transition-colors"
                          aria-label="Supprimer le contexte d'échec"
                        >
                          <X className="w-3 h-3" />
                        </button>
                      </div>
                      <p className="text-[10px] font-bold opacity-60 italic truncate">
                        "{selectedFailure.input_context}"
                      </p>
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 bg-gray-50 dark:bg-white/5 rounded-2xl border border-black/5 dark:border-white/5">
                      <p className="text-[8px] font-black opacity-30 uppercase mb-1">
                        {t('admin.dspy.strategy', 'Stratégie')}
                      </p>
                    ...
                    <div className="p-4 bg-gray-50 dark:bg-white/5 rounded-2xl border border-black/5 dark:border-white/5">
                      ...
                      <p className="text-xs font-bold text-emerald-600 dark:text-emerald-400">
                        {t('admin.dspy.variants_val', '3 par cycle')}
                      </p>
                    </div>
                  </div>
              ...
              <Card
                padding="lg"
                className="bg-white dark:bg-[#0f0f1a] border-none shadow-xl rounded-[3rem] overflow-hidden"
              >
                <h3 className="text-xs font-black uppercase opacity-40 mb-6 tracking-widest flex items-center gap-2">
                  <ShieldAlert className="w-4 h-4 text-red-500" />{' '}
                  {t('admin.dspy.prod_failures', 'Erreurs de Raisonnement (Prod)')}
                </h3>

                <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                  {isLoadingFailures ? (
                    <div className="py-12 text-center">
                      <RefreshCw className="w-6 h-6 animate-spin mx-auto opacity-20" />
                    </div>
                    ...
                    failures.map((fail) => (
                      ...
                      >
                        <div className="flex justify-between items-center mb-2">
                          <Badge
                            variant="neutral"
                            className="bg-red-500/10 text-red-500 border-none text-[8px] font-black italic"
                          >
                        ...
                        <div className="flex items-center gap-4 text-[9px] font-black uppercase tracking-widest">
                          <span className="text-red-400">
                            Score: {Math.round(fail.faithfulness * 100)}%
                          </span>
                    ...
                    <div className="py-12 text-center opacity-20">
                      ...
                      <p className="text-[10px] font-black uppercase tracking-widest">
                        {t('admin.dspy.no_errors', 'Aucune erreur détectée')}
                      </p>
                    </div>
                  )}
                </div>
              ...
              <Card
                padding="lg"
                className="bg-white dark:bg-[#0f0f1a] border-none shadow-xl rounded-[2.5rem]"
              >
                <h3 className="text-xs font-black uppercase opacity-40 mb-6 tracking-widest flex items-center gap-2">
                ...
                <p className="text-[10px] font-bold opacity-40 leading-relaxed uppercase italic">
                  {t(
                    'admin.dspy.eval_dataset_desc',
                    "L'optimiseur utilise le Gold Dataset v2 complété par les échecs de production pour valider la robustesse.",
                  )}
                </p>
              </Card>
            </div>
              ...
              <AnimatePresence mode="wait">
                {result ? (
                  ...
                  >
                    ...
                    <Card
                      padding="lg"
                      className="bg-emerald-500/5 dark:bg-emerald-500/10 border-2 border-emerald-500/20 rounded-[3rem] relative overflow-hidden group"
                    >
                      ...
                      <div className="flex justify-between items-start mb-6">
                        <div>
                          <Badge variant="success" className="mb-2">
                            CHAMPION VARIANT DETECTED
                          </Badge>
                          <h3 className="text-3xl font-black italic manga-font uppercase">
                            Best Score: {(result.best_score * 100).toFixed(1)}%
                          </h3>
                        </div>
                        <div className="text-right">
                          <p className="text-[8px] font-black opacity-40 uppercase mb-1 tracking-widest">
                            {t('admin.dspy.gain_label', 'Gain de performance')}
                          </p>
                    ...
                    <div>
                      <h3 className="text-xs font-black uppercase opacity-40 mb-6 tracking-widest flex items-center gap-2 px-4">
                        ...
                        {t('admin.dspy.all_mutations', 'Toutes les Mutations')}
                      </h3>
                      <div className="space-y-4">
                        {result.all_mutations?.map((mut: string, i: number) => (
                          ...
                          >
                            <Card
                              padding="md"
                              className={`bg-white dark:bg-[#0f0f1a] border-none shadow-md hover:scale-[1.01] transition-all ${mut === result.best_template ? 'ring-2 ring-emerald-500/20 shadow-[0_0_20px_rgba(16,185,129,0.1)]' : ''}`}
                            >
                              <div className="flex justify-between items-center mb-2">
                                ...
                                <div className="flex items-center gap-3">
                                  <div className="w-20 h-1 bg-gray-100 dark:bg-white/5 rounded-full overflow-hidden">
                                    <div
                                      className={`h-full ${mut === result.best_template ? 'bg-emerald-500' : 'bg-blue-500/40'}`}
                                      style={{ width: `${mutationStats[i]?.width || 70}%` }}
                                    />
                                  </div>
                                  <span className="text-[10px] font-black italic">
                                    {(mutationStats[i]?.score || 0.7).toFixed(2)}
                                  </span>
                                </div>
                              </div>
                    ...
                    <Card padding="lg" className="bg-white dark:bg-[#0f0f1a] border-none shadow-xl">
                      <div className="flex gap-8">
                        ...
                        <div>
                          <h4 className="text-xl font-black italic manga-font uppercase mb-1">
                            {t('admin.dspy.hyper_params_title', 'Analyse des Hyper-paramètres')}
                          </h4>
                          <p className="text-[10px] font-bold opacity-30 leading-relaxed uppercase tracking-widest">
                            {t(
                              'admin.dspy.hyper_params_desc',
                  ...
                  <div className="h-full flex flex-col items-center justify-center opacity-10 text-center py-48 border-4 border-dashed border-black/5 dark:border-white/5 rounded-[4rem]">
                    ...
                    <p className="text-sm font-bold uppercase tracking-[0.3em]">
                      {t(
                        'admin.dspy.idle_desc',
                        "Configurez un template pour démarrer l'auto-tuning.",
                      )}
                    </p>
                  </div>
                )}
          ...
          <div className="mt-24 p-12 rounded-[4rem] bg-white dark:bg-[#0f0f1a] shadow-xl border border-black/5 dark:border-white/5 text-center">
            <p className="text-[10px] font-black uppercase tracking-[0.4em] opacity-30 italic leading-relaxed">
              {t(
                'admin.dspy.experimental_warning',
                "Avertissement : L'Optimisation DSPy est une phase expérimentale. Les mutations sont générées par le modèle Champion actuel et validées via LLM-as-a-Judge.",
              )}
            </p>
          </div>
        </div>
