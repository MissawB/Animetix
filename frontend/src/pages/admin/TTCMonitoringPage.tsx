...
const fetchTTCStats = async (): Promise<TTCMonitoringData> => {
  return adminService.getTTCMonitoring();
};
    ...
    <AnimatedPage>
      <div className="min-h-[calc(100vh-64px)] bg-[#fffcf0] dark:bg-[#1a1a2e] transition-colors duration-500 bg-manga-overlay">
        <div className="max-w-7xl mx-auto px-6 py-16">
          <Link
            to="/admin/dashboard/"
            className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-gray-500 hover:text-black dark:hover:text-white mb-8 no-underline"
          >
            <ArrowLeft className="w-4 h-4" />{' '}
            {t('admin.common.back_admin', 'Retour Administration')}
          </Link>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12 text-black dark:text-white">
            <Card padding="lg" className="bg-white dark:bg-[#0f0f1a] border-none shadow-xl">
              <p className="text-[10px] font-black uppercase opacity-40 mb-2">
                {t('admin.ttc.budget_allocated', 'Budget Alloué (24h)')}
              </p>
              <p className="text-4xl font-black italic">{summary.total_allocated}</p>
            </Card>
            <Card padding="lg" className="bg-white dark:bg-[#0f0f1a] border-none shadow-xl">
              <p className="text-[10px] font-black uppercase opacity-40 mb-2">
                {t('admin.ttc.actual_consumed', 'Consommation Réelle')}
              </p>
              <p className="text-4xl font-black italic">{summary.total_consumed}</p>
            </Card>
            <Card
              padding="lg"
              className={`bg-white dark:bg-[#0f0f1a] border-none shadow-xl ${summary.efficiency > 100 ? 'text-red-500' : 'text-emerald-500'}`}
            >
              <p className="text-[10px] font-black uppercase opacity-40 mb-2 text-black dark:text-white">
                {t('admin.ttc.cognitive_efficiency', 'Efficacité Cognitive')}
              </p>
          ...
          <Card
            padding="none"
            className="bg-white dark:bg-[#0f0f1a] border-none shadow-2xl overflow-hidden"
          >
                ...
                <tbody className="divide-y divide-black/5 dark:divide-white/5">
                  {logs.map((log: TTCLog) => {
                    ...
                    return (
                      ...
                      >
                        <td className="p-6 font-mono text-[10px] font-black uppercase text-blue-500">
                          {log.engine}
                        </td>
                        ...
                        <td className="p-6 text-right">
                          {overBudget ? (
                            <span className="inline-flex items-center gap-2 bg-red-500/10 text-red-500 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border border-red-500/20">
                              <AlertTriangle className="w-3 h-3" /> Over Budget
                            </span>
                          ) : (
                            <span className="inline-flex items-center gap-2 bg-emerald-500/10 text-emerald-500 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border border-emerald-500/20">
