...
const EconomicAuditPage: React.FC = () => {
  ...
  const { data, isLoading } = useQuery<EconomicData>({
    ...
    queryFn: () => adminService.getEconomics(),
  });
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          ...
          {isLoading ? (
            <div className="py-32 text-center">
              <RefreshCw className="w-12 h-12 animate-spin mx-auto opacity-20 text-amber-500" />
            </div>
          ) : data ? (
            <div className="space-y-12">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                ...
                <div className="lg:col-span-7 space-y-8">
                  <Card
                    padding="lg"
                    className="bg-white dark:bg-navy-950/40 border-none shadow-2xl rounded-[3rem] relative overflow-hidden group"
                  >
                    ...
                    <h3 className="text-xs font-black uppercase tracking-widest flex items-center gap-2 opacity-40 mb-10">
                      ...
                      {t(
                        'admin.economics.historical_flux_repartition',
                        'Répartition des Flux Historiques',
                      )}
                    </h3>
                      ...
                      {Object.entries(data.repartition).map(([type, amount]) => (
                        <div key={type} className="group/item">
                          <div className="flex justify-between items-end mb-2">
                            ...
                            <span
                              className={`text-sm font-black italic manga-font ${amount >= 0 ? 'text-emerald-500' : 'text-red-500'}`}
                            >
                              {amount >= 0 ? '+' : ''}
                              {amount.toLocaleString()} Bx
                            </span>
                          </div>
                          <div className="w-full h-2 bg-black/5 dark:bg-white/5 rounded-full overflow-hidden">
                            <motion.div
                              initial={{ width: 0 }}
                              animate={{
                                width: `${Math.min(100, (Math.abs(amount) / data.total_circulation) * 1000)}%`,
                              }}
                  ...
                  <Card
                    padding="lg"
                    className="bg-amber-600 text-white border-none shadow-2xl rounded-[3rem] flex flex-col justify-between min-h-[250px]"
                  >
                    <div>
                      <ShieldCheck className="w-12 h-12 mb-8 opacity-40" />
                      <h3 className="text-3xl font-black italic manga-font uppercase mb-4 leading-tight">
                        Berrix Policy Engine
                      </h3>
                      <p className="text-sm font-bold italic opacity-90 leading-relaxed uppercase">
                        {t(
                          'admin.economics.policy_engine_desc',
                          "Le taux d'inflation actuel ({{inflation}}) indique un écosystème {{status}}. {{detail}}",
                          {
                            inflation: data.inflation_index,
                            status: data.status.toLowerCase(),
                            detail:
                              data.inflation_index > 1.5
                                ? t(
                                    'admin.economics.overissue_warning',
                                    'Attention : Sur-émission de tokens détectée via le minage passif.',
                                  )
                                : t(
                                    'admin.economics.equilibrium_message',
                                    "L'équilibre entre la consommation IA et les revenus publicitaires est maintenu.",
                                  ),
                          },
                        )}
                      </p>
                ...
                <div className="lg:col-span-5 space-y-8">
                  <Card
                    padding="lg"
                    className="bg-white dark:bg-navy-950 border-none shadow-2xl rounded-[3rem] h-full"
                  >
                    <h3 className="text-xs font-black uppercase tracking-widest flex items-center gap-2 opacity-40 mb-10">
                      <Zap className="w-4 h-4 text-amber-500" />{' '}
                      {t('admin.economics.flux_24h_live', 'Flux 24h (Live)')}
                    </h3>
                    ...
                    <div className="space-y-12">
                      <div className="flex items-center gap-8">
                        ...
                        <div>
                          ...
                          <p className="text-4xl font-black italic manga-font text-emerald-500">
                            +{data.flux_24h.minted.toLocaleString()}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-8">
                        ...
                        <div>
                          ...
                          <p className="text-4xl font-black italic manga-font text-red-500">
                            -{data.flux_24h.burned.toLocaleString()}
                          </p>
                        </div>
                      </div>
                        ...
                        <div className="p-8 bg-gray-50 dark:bg-white/[0.03] rounded-[2.5rem] border border-black/5 dark:border-white/5">
                          <div className="flex justify-between items-center mb-4">
                            <span className="text-[10px] font-black uppercase tracking-widest opacity-30">
                              {t('admin.economics.wealth_ceiling', 'Plafond de Richesse')}
                            </span>
                            <Badge
                              variant="neutral"
                              className="bg-amber-500/10 text-amber-600 dark:text-amber-500 border-none text-[8px] font-black italic"
                            >
                              TOP_WHALE
                            </Badge>
                          </div>
                          <p className="text-2xl font-black italic manga-font">
                            {data.max_balance.toLocaleString()} Bx
                          </p>
                          <p className="text-[8px] font-bold opacity-30 uppercase mt-2">
                            {t(
                              'admin.economics.largest_wallet',
                              "Plus gros portefeuille de l'écosystème",
                            )}
                          </p>
                        </div>
                      </div>
              ...
              <div className="mt-20 p-12 bg-black text-white rounded-[4rem] border border-white/5 flex flex-col md:flex-row items-center gap-12 text-center md:text-left">
                ...
                <div className="flex-grow">
                  <h4 className="text-xl font-black italic manga-font uppercase mb-2">
                    {t('admin.economics.intrinsic_value_note', 'Note sur la Valeur Intrinsèque')}
                  </h4>
                  <p className="text-[10px] font-bold opacity-40 uppercase leading-relaxed max-w-3xl">
                    {t(
                      'admin.economics.intrinsic_value_desc',
                ...
                <Button
                  variant="outline"
                  className="border-white/10 text-[10px] font-black uppercase tracking-widest whitespace-nowrap px-8 py-4 hover:bg-white/5 transition-all"
                >
            ...
            <div className="py-32 text-center opacity-20">
              {t('admin.economics.load_error', 'Erreur de chargement des données.')}
            </div>
          )}
        </div>
      </AnimatedPage>
...
const EconomicMetricCard: React.FC<EconomicMetricCardProps> = ({
  label,
  value,
  icon: Icon,
  color,
  ...
  <Card
    padding="lg"
    className="bg-white dark:bg-navy-950 border-none shadow-xl rounded-[2.5rem] relative overflow-hidden group hover:scale-105 transition-all duration-300"
  >
