...
import { Trophy, Cpu, BarChart3, Globe, Zap, Activity, Code, Sparkles } from 'lucide-react';
import { useQuery } from '@tanstack/react-query';
import { mlopsService } from '../../features/admin/services/mlopsService';
...
const SOTABenchmarkPage: React.FC = () => {
  ...
  const { data, isLoading, isError } = useQuery<BenchmarkData>({
    ...
    queryFn: () => mlopsService.getSotaBenchmarks(),
  });
  ...
  if (isLoading)
    return (
      <div className="max-w-7xl mx-auto px-6 py-32 flex flex-col items-center justify-center">
        ...
        <p className="text-sm font-black uppercase tracking-[0.3em] animate-pulse opacity-40">
          Syncing SOTA Metrics...
        </p>
      </div>
    );
    ...
    return (
      <div className="max-w-7xl mx-auto px-6 py-32 text-center text-red-500">
        ...
        <p className="opacity-50 mt-4">
          {t('admin.sota.sync_error', 'Impossible de récupérer les benchmarks en temps réel.')}
        </p>
      </div>
    );
      ...
      <div className="min-h-[calc(100vh-64px)] bg-[#fffcf0] dark:bg-[#1a1a2e] transition-colors duration-500 bg-manga-overlay">
        <div className="max-w-7xl mx-auto px-6 py-12">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-16 text-black dark:text-white">
            <Card
              padding="none"
              className="bg-white dark:bg-[#0f0f1a] border-none shadow-2xl rounded-[3.5rem] overflow-hidden relative group"
            >
              ...
              <div className="bg-blue-600 px-12 py-6 flex items-center justify-between">
                ...
                <Badge
                  variant="neutral"
                  className="bg-black/20 text-white border-none uppercase font-black italic"
                >
                  Leaderboard #1
                </Badge>
              </div>
              <div className="p-12 relative z-10">
                <h2 className="text-5xl font-black italic manga-font uppercase mb-2 tracking-tighter text-black dark:text-white">
                  {top_model.model_id}
                </h2>
                <p className="text-[10px] font-black uppercase tracking-widest text-blue-500 mb-8">
                  Provider: {top_model.provider}
                </p>

                <div className="grid grid-cols-2 gap-8">
                  <div>
                    <p className="text-[8px] font-black opacity-30 uppercase mb-2">
                      Arena ELO Score
                    </p>
                    <p className="text-4xl font-black italic text-black dark:text-white">
                      {top_model.elo_score}
                    </p>
                  </div>
            ...
            >
              <div className="bg-gray-50 dark:bg-white/5 px-12 py-6 flex items-center justify-between border-b border-black/5 dark:border-white/5">
                ...
                <Badge
                  variant="neutral"
                  className="bg-emerald-500/10 text-emerald-500 border-none uppercase font-black italic"
                >
                  Weights Available
                </Badge>
              </div>
                ...
                <div className="grid grid-cols-2 gap-8">
                  ...
                  <div className="border-l border-black/5 dark:border-white/5 pl-8">
                    ...
                    <p className="text-4xl font-black italic text-blue-400">
                      {Math.floor(best_open_source.context_window / 1000)}k
                    </p>
                  </div>
                </div>
              </div>
          ...
          <Card
            padding="none"
            className="bg-white dark:bg-[#0f0f1a] border-none rounded-[3rem] overflow-hidden mb-16 shadow-2xl"
          >
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-black dark:text-white">
                <thead className="bg-gray-50 dark:bg-black/40">
                  <tr>
                    <th className="px-12 py-6 text-[10px] font-black uppercase tracking-[0.2em] opacity-40">
                      Rank
                    </th>
                    <th className="px-8 py-6 text-[10px] font-black uppercase tracking-[0.2em] opacity-40">
                      Model ID
                    </th>
                    <th className="px-8 py-6 text-[10px] font-black uppercase tracking-[0.2em] opacity-40 text-center">
                      ELO Score
                    </th>
                    <th className="px-8 py-6 text-[10px] font-black uppercase tracking-[0.2em] opacity-40 text-center">
                      MMLU
                    ...
                    <th className="px-8 py-6 text-[10px] font-black uppercase tracking-[0.2em] opacity-40 text-right">
                      License
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-black/5 dark:divide-white/5">
                  {[...benchmarks]
                    .sort((a: ModelBenchmark, b: ModelBenchmark) => b.elo_score - a.elo_score)
                    .map((m: ModelBenchmark, i: number) => (
                      <tr
                        key={m.model_id}
                        aria-label={`Classement du modèle ${m.model_id}`}
                        className="group hover:bg-gray-50/50 dark:hover:bg-white/[0.02] transition-colors"
                      >
                        <td className="px-12 py-6">
                          <span
                            className={`text-xl font-black italic manga-font ${i === 0 ? 'text-yellow-500' : 'opacity-20'}`}
                          >
                            #0{i + 1}
                          </span>
                        </td>
                        <td aria-label={`Modèle ${m.model_id}`} className="px-8 py-6">
                          <div className="flex flex-col">
                            <span className="text-sm font-black italic uppercase tracking-wider group-hover:text-blue-500 transition-colors">
                              {m.model_id}
                            </span>
                        ...
                        <td className="px-8 py-6 text-center">
                          <Badge
                            variant="neutral"
                            className="bg-black/5 dark:bg-white/5 border-none font-mono text-xs text-black dark:text-white"
                          >
                            {m.elo_score}
                          </Badge>
                        </td>
                        ...
                        <td className="px-8 py-6 text-center">
                          <span className="text-xs font-bold opacity-50 uppercase">
                            {Math.floor(m.context_window / 1000)}k tokens
                          </span>
                        </td>
                        <td className="px-8 py-6 text-right">
                          <Badge
                            variant="neutral"
                            className={`border-none text-[8px] uppercase tracking-widest ${m.is_open_source ? 'bg-emerald-500/10 text-emerald-500' : 'bg-red-500/10 text-red-500'}`}
                          >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-black dark:text-white">
            <Card padding="lg" className="bg-white dark:bg-[#0f0f1a] border-none shadow-xl">
              ...
              <h4 className="text-xs font-black uppercase tracking-widest mb-2">
                {t('admin.sota.footer_anthropic_title', 'Domination Anthropic')}
              </h4>
              <p className="text-[10px] font-bold opacity-30 uppercase leading-relaxed">
                {t(
                  'admin.sota.footer_anthropic_desc',
                  'Claude 3.5 Sonnet maintient un ratio de préférence utilisateur exceptionnel dans le domaine de la génération de code et du raisonnement complexe.',
                )}
              </p>
            </Card>
            <Card padding="lg" className="bg-white dark:bg-[#0f0f1a] border-none shadow-xl">
              ...
              <h4 className="text-xs font-black uppercase tracking-widest mb-2">
                {t('admin.sota.footer_scaling_title', 'Scaling Laws')}
              </h4>
              <p className="text-[10px] font-bold opacity-30 uppercase leading-relaxed">
                {t(
                  'admin.sota.footer_scaling_desc',
                  'Llama-3.1-405B redéfinit la limite supérieure des modèles Open Weights, égalant les performances des solutions propriétaires fermées.',
                )}
              </p>
            </Card>
            <Card padding="lg" className="bg-white dark:bg-[#0f0f1a] border-none shadow-xl">
              ...
              <h4 className="text-xs font-black uppercase tracking-widest mb-2">
                {t('admin.sota.footer_multimodel_title', 'Animetix Multi-Model')}
              </h4>
              <p className="text-[10px] font-bold opacity-30 uppercase leading-relaxed">
                {t(
                  'admin.sota.footer_multimodel_desc',
                  'Le backend Animetix route dynamiquement vos requêtes vers les modèles du Top 3 pour garantir une précision sémantique maximale.',
                )}
              </p>
            </Card>
          </div>
