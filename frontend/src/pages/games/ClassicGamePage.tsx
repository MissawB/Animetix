...
const ClassicGamePage: React.FC = () => {
  ...
  useEffect(() => {
    let active = true;
    classicGameService.getTitles().then((t) => { if (active) setTitles(t); }).catch(() => {});
    return () => { active = false; };
  }, []);
                        ...
                        <div className="flex items-center justify-between gap-3 mb-2.5">
                          ...
                          {g.is_correct ? (
                            ...
                            <div className="shrink-0 flex items-center gap-2">
                              <span className={`text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-full ${heat.chip}`}>
                                {heat.label}
                              </span>
                    ...
                    >
                      ...
                      {!revealed && !canReveal && (
                        <div className="mt-2.5 h-1 rounded-full bg-black/5 dark:bg-white/10 overflow-hidden">
                          <div className="h-full rounded-full bg-yellow-500/60 transition-all duration-500" style={{ width: `${progress * 100}%` }} />
                        </div>
                      )}
                    </div>
                  );
...
const StatusPill: React.FC<{ icon: React.ElementType; label: string; value: string }> = ({ icon: Icon, label, value }) => (
