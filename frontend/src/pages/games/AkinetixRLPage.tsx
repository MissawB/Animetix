        ...
        <div className="max-w-5xl mx-auto px-6 py-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
            <Card padding="lg" className="bg-navy-900/50 border-white/5 text-center">
              ...
              <h3 className="text-xl font-black italic manga-font uppercase mb-4">
          ...
          <div className="bg-black p-12 rounded-[4rem] border-4 border-cyan-500/20 shadow-[0_0_50px_rgba(6,182,212,0.1)] flex flex-col items-center">
            <h4 className="text-2xl font-black italic manga-font uppercase mb-8">
  ...
  if (!state && loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh]">
        ...
        <span className="font-black italic manga-font animate-pulse">
      ...
      <div className="max-w-5xl mx-auto px-6 py-16">
        <div className="flex items-center justify-between mb-12">
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="text-right hidden sm:block">
              <div className="text-[10px] font-black opacity-30 uppercase mb-1 text-right">
              ...
              <div className="flex gap-1 justify-end">
                {[1, 2, 3, 4, 5].map((i) => (
                  <div
                    key={i}
                    className={`h-1.5 w-6 rounded-full ${i <= 5 - (state?.steps || 0) / 4 ? 'bg-cyan-500 shadow-[0_0_10px_rgba(6,182,212,0.5)]' : 'bg-white/10'}`}
                  />
                ))}
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-8 space-y-8">
            <Card
              padding="none"
              className="bg-navy-900 border-white/10 relative overflow-hidden rounded-[3rem] shadow-2xl"
            >
                ...
                {!state?.game_over ? (
                  <>
                    <div className="mb-12">
                      <Badge
                        variant="primary"
                        className="bg-cyan-500/20 text-cyan-500 border-cyan-500/20 mb-6 font-black italic uppercase tracking-widest"
                      >
                      ...
                      <h2 className="text-4xl font-black italic manga-font leading-tight text-white uppercase tracking-tighter">
                        {state?.current_q ||
                          t('games.akinetix.rl.computing', 'Calcul de la trajectoire optimale...')}
                      </h2>
                    </div>
                  ...
                  <div className="text-center py-8 animate-fade-in">
                    ...
                    <h2 className="text-6xl font-black italic manga-font mb-6 uppercase tracking-tighter">
                    ...
                    <div className="bg-cyan-500/10 border-4 border-cyan-500/30 p-12 rounded-[3.5rem] mb-12 shadow-inner">
                      <p className="text-sm font-black opacity-40 uppercase mb-4 tracking-[0.3em]">
                        {t(
                          'games.akinetix.rl.prediction',
                          "L'agent RL prédit avec 98.4% de certitude :",
                        )}
                      </p>
                      <h3 className="text-5xl font-black italic manga-font text-cyan-400 uppercase tracking-tighter text-glow">
                        {state.ai_guess}
                      </h3>
                    </div>
                    <div className="flex flex-col sm:flex-row gap-6 w-full max-w-xl mx-auto">
                      <Button
                        onClick={() => startRLGame()}
                        variant="primary"
                        className="flex-1 py-8 text-xl rounded-2xl shadow-xl hover:scale-105 border-none bg-white text-black font-black uppercase italic"
                      >
                        REBOOT AGENT
                      </Button>
                      <Button
                        variant="outline"
                        className="p-8 border-white/10 rounded-2xl hover:bg-emerald-500/10 hover:text-emerald-500 transition-all"
                      >
            ...
            <div className="bg-black/40 rounded-[2.5rem] p-8 border border-white/5">
              ...
              <div className="font-mono text-[10px] space-y-2 opacity-60">
                ...
                <p className="flex justify-between">
                  ...
                  <span className="text-amber-500">GreedyInformationGain</span>
                </p>
                <p className="flex justify-between">
                  <span>{`> INFERENCE_STATUS:`}</span>{' '}
                  <span className="text-blue-500">OPTIMIZING_NODE_{state?.steps || 0}</span>
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="bg-white/5 rounded-[3rem] p-8 shadow-xl border border-white/5 flex-grow">
              ...
              <div className="space-y-4 max-h-[600px] overflow-y-auto pr-2 no-scrollbar">
                {state?.history.map((h, i) => (
                  <div
                    key={i}
                    className="p-5 bg-navy-900/50 rounded-2xl border border-white/5 hover:border-white/10 transition-colors animate-fade-in group"
                  >
                    <p className="text-[9px] font-black opacity-30 uppercase mb-2 tracking-widest group-hover:text-cyan-500 transition-colors">
                      Inf #{i + 1}
                    </p>
                    <p className="text-xs font-bold text-white/60 mb-2 leading-relaxed uppercase italic">
                      "{h.q}"
                    </p>
                    <div className="flex justify-end">
                      <Badge
                        variant="neutral"
                        className="bg-white/5 border-white/10 text-white font-black italic"
                      >
                        {h.a}
                      </Badge>
                    </div>
