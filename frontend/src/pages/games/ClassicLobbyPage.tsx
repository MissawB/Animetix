...
const ClassicLobbyPage: React.FC = () => {
  ...
  const HINT_MODES: { key: HintMode; label: string; sub: string; icon: React.ElementType }[] = [
    { key: 'classic', label: t('games.classic.lobby.modes.classic.label', 'Classique'), sub: t('games.classic.lobby.modes.classic.sub', '4 indices clés'), icon: Trophy },
    { key: 'custom', label: t('games.classic.lobby.modes.custom.label', 'Personnalisé'), sub: t('games.classic.lobby.modes.custom.sub', 'À ta façon'), icon: SlidersHorizontal },
    { key: 'tryhard', label: t('games.classic.lobby.modes.tryhard.label', 'Tryhard'), sub: t('games.classic.lobby.modes.tryhard.sub', 'Aucun indice'), icon: Skull },
  ];

  const UNIVERSES: { key: Universe; label: string; sub: string; icon: React.ElementType }[] = [
    { key: 'Anime', label: t('games.classic.lobby.universes.anime.label', 'Anime'), sub: t('games.classic.lobby.universes.anime.sub', 'Séries animées'), icon: Clapperboard },
    { key: 'Manga', label: t('games.classic.lobby.universes.manga.label', 'Manga'), sub: t('games.classic.lobby.universes.manga.sub', 'Œuvres papier'), icon: BookOpen },
    { key: 'Character', label: t('games.classic.lobby.universes.character.label', 'Personnages'), sub: t('games.classic.lobby.universes.character.sub', 'Héros & figures'), icon: Users },
  ];

  const DIFFICULTIES: { key: Difficulty; label: string; sub: string; active: string }[] = [
    { key: 'Easy', label: t('games.classic.lobby.difficulties.easy.label', 'Facile'), sub: t('games.classic.lobby.difficulties.easy.sub', 'Titres très connus'), active: 'border-green-500 bg-green-500/10 text-green-600 dark:text-green-400' },
    { key: 'Normal', label: t('games.classic.lobby.difficulties.normal.label', 'Normal'), sub: t('games.classic.lobby.difficulties.normal.sub', 'Grand public'), active: 'border-blue-500 bg-blue-500/10 text-blue-600 dark:text-blue-400' },
    { key: 'Hard', label: t('games.classic.lobby.difficulties.hard.label', 'Difficile'), sub: t('games.classic.lobby.difficulties.hard.sub', 'Pour connaisseurs'), active: 'border-orange-500 bg-orange-500/10 text-orange-600 dark:text-orange-400' },
    { key: 'Impossible', label: t('games.classic.lobby.difficulties.impossible.label', 'Impossible'), sub: t('games.classic.lobby.difficulties.impossible.sub', 'Pépites obscures'), active: 'border-red-600 bg-red-600/10 text-red-600 dark:text-red-400' },
  ];

  const HINT_DEFS: Record<ClassicHintKey, { label: string; desc: string; icon: React.ElementType; tone: string }> = {
    year:   { label: t('games.classic.lobby.hints.year.label', 'Année de sortie'), desc: t('games.classic.lobby.hints.year.desc', 'Année de parution'), icon: Calendar, tone: 'text-blue-500' },
    origin: { label: t('games.classic.lobby.hints.origin.label', 'Origine'), desc: t('games.classic.lobby.hints.origin.desc', "Œuvre / pays d'origine"), icon: Globe, tone: 'text-teal-500' },
    tags:   { label: t('games.classic.lobby.hints.tags.label', 'Tags'), desc: t('games.classic.lobby.hints.tags.desc', 'Principaux thèmes'), icon: Tags, tone: 'text-yellow-500' },
    genres: { label: t('games.classic.lobby.hints.genres.label', 'Genres'), desc: t('games.classic.lobby.hints.genres.desc', 'Catégories principales'), icon: Shapes, tone: 'text-orange-500' },
    studio: { label: t('games.classic.lobby.hints.studio.label', 'Studio'), desc: t('games.classic.lobby.hints.studio.desc', "Studio d'animation / éditeur"), icon: Clapperboard, tone: 'text-purple-500' },
    letter: { label: t('games.classic.lobby.hints.letter.label', 'Première lettre'), desc: t('games.classic.lobby.hints.letter.desc', 'La lettre initiale du titre'), icon: CaseSensitive, tone: 'text-pink-500' },
    words:  { label: t('games.classic.lobby.hints.words.label', 'Nombre de mots'), desc: t('games.classic.lobby.hints.words.desc', 'Combien de mots dans le titre'), icon: Hash, tone: 'text-cyan-500' },
    desc:   { label: t('games.classic.lobby.hints.desc.label', 'Description'), desc: t('games.classic.lobby.hints.desc.desc', 'Extrait du synopsis'), icon: ScrollText, tone: 'text-green-500' },
  };
  ...
  const launch = async () => {
    ...
    try {
      ...
      addToast(t('games.classic.lobby.toast_launch_failed', 'Impossible de lancer la partie. Réessayez.'), 'error');
      setLaunching(false);
    }
  };
  ...
  const hintCountLabel = (count: number) =>
    count === 0
      ...
      : t('games.classic.lobby.hint_count', { defaultValue: '{{count}} indice{{plural}}', count, plural: count > 1 ? 's' : '' });
    ...
    <div className="max-w-3xl mx-auto px-6 py-14">
      ...
      <div className="text-center mb-12">
        ...
        <h1 className="text-5xl md:text-6xl font-black italic manga-font tracking-tighter uppercase text-black dark:text-white leading-none">
          ANIMETIX <span className="text-blue-500">{t('games.classic.lobby.title_highlight', 'CLASSIQUE')}</span>
        </h1>
        <p className="mt-4 text-base font-medium text-gray-500 dark:text-white/50">
          {t('games.classic.lobby.subtitle', "Configure ta traque, puis pars démasquer l'œuvre mystère.")}
        </p>
      </div>
      ...
      <div className="rounded-[2.5rem] border-2 border-black/5 dark:border-white/10 bg-surface-card p-7 md:p-9 shadow-token-card space-y-10">
        ...
        {/* Difficulté */}
        <Section step={2} title={t('games.classic.lobby.section_difficulty', 'Difficulté')} hint={t('games.classic.lobby.section_difficulty_hint', "Rareté de l'œuvre à trouver")}>
        ...
        >
                ...
                return (
                  <li key={key} className="flex items-center gap-3 rounded-2xl border-2 border-blue-500/30 bg-blue-500/[0.05] p-3">
                    ...
                    <span className="shrink-0 text-[10px] font-black uppercase tracking-wider text-blue-500 bg-blue-500/10 px-2.5 py-1 rounded-full tabular-nums">
                      {t('games.classic.lobby.tries_count', { defaultValue: '{{count}} essais', count: (i + 1) * UNLOCK_STEP })}
                    </span>
                  </li>
                );
              })}
          ...
          {hintMode === 'tryhard' && (
            <div className="rounded-2xl border-2 border-red-500/30 bg-red-500/[0.06] p-6 text-center">
              ...
              <p className="font-black italic uppercase text-red-500">{t('games.classic.lobby.tryhard_title', 'Aucun indice')}</p>
              <p className="text-xs font-bold opacity-50 mt-1">{t('games.classic.lobby.tryhard_desc', 'Juste toi, ton instinct et la chaleur des tentatives. Pour les vrais.')}</p>
            </div>
          )}
          ...
          {hintMode === 'custom' && (
            <>
              <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-4">
                {t('games.classic.lobby.custom_help', { defaultValue: 'Active les indices voulus et range-les : ils se débloqueront dans cet ordre, un toutes les {{step}} tentatives.', step: UNLOCK_STEP })}
              </p>
              <ul className="space-y-2.5">
                {hintOrder.map((key, i) => {
                  const def = HINT_DEFS[key];
                    ...
                    >
                      ...
                      <div className="flex flex-col">
                        <button
                          onClick={() => move(i, -1)}
                          disabled={i === 0}
                          aria-label={t('games.classic.lobby.aria_move_up', { defaultValue: 'Monter {{label}}', label: def.label })}
                        ...
                        <button
                          ...
                          aria-label={t('games.classic.lobby.aria_move_down', { defaultValue: 'Descendre {{label}}', label: def.label })}
                          className="p-0.5 rounded text-gray-400 hover:text-blue-500 disabled:opacity-20 disabled:hover:text-gray-400 transition-colors"
                        >
                      ...
                      {on && (
                        <span className="shrink-0 text-[10px] font-black uppercase tracking-wider text-blue-500 bg-blue-500/10 px-2.5 py-1 rounded-full tabular-nums">
                          {t('games.classic.lobby.tries_count', { defaultValue: '{{count}} essais', count: unlockAt(key) })}
                        </span>
                      )}
        ...
        >
          {launching ? (
            <><Loader2 className="w-6 h-6 animate-spin" /> {t('games.classic.lobby.launching', 'Préparation…')}</>
          ) : (
            <><Play className="w-6 h-6 fill-current" /> {t('games.classic.lobby.launch', 'Lancer la partie')}</>
          )}
        </button>
        <p className="-mt-4 text-center text-[11px] font-bold uppercase tracking-widest text-gray-400 flex items-center justify-center gap-1.5">
          ...
          {UNIVERSES.find((u) => u.key === universe)?.label} · {DIFFICULTIES.find((d) => d.key === difficulty)?.label} · {HINT_MODES.find((m) => m.key === hintMode)?.label} ({t('games.classic.lobby.hint_count', { defaultValue: '{{count}} indice{{plural}}', count: config.length, plural: config.length > 1 ? 's' : '' })})
        </p>
      </div>
    </div>
  );
