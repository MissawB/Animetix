          ...
          {(['blue', 'red'] as const).map((team) => (
            <div key={team} className={`rounded-2xl border-2 p-3 ${team === 'blue' ? 'border-blue-500/30 bg-blue-500/5' : 'border-red-500/30 bg-red-500/5'}`}>
              <div className="flex items-center justify-between mb-2">
                <p className={`text-[11px] font-black uppercase tracking-widest ${team === 'blue' ? 'text-blue-300' : 'text-red-300'}`}>Équipe {TEAM_LABEL[team]}</p>
                {state === 'lobby' && (
                  <div className="flex gap-1">
                    <button onClick={() => setPlayer({ team, role: 'spymaster' })} title="Espion (voit les couleurs)"
                      className={`px-2 py-1 rounded-lg text-[9px] font-black uppercase border ${myTeam === team && myRole === 'spymaster' ? 'bg-white/90 text-black border-white' : 'border-white/20 text-white/50 hover:border-white/50'}`}>
                      <Eye className="w-3 h-3" />
                    </button>
                    <button onClick={() => setPlayer({ team, role: 'operative' })} title="Agent (devine)"
                      className={`px-2 py-1 rounded-lg text-[9px] font-black uppercase border ${myTeam === team && myRole === 'operative' ? 'bg-white/90 text-black border-white' : 'border-white/20 text-white/50 hover:border-white/50'}`}>
