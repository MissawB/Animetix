      ...
      <div className="max-w-7xl mx-auto px-6 py-16">
        <header className="mb-16 flex flex-col md:flex-row justify-between items-end gap-8">
            <div>
                ...
                <p className="text-xl font-bold opacity-30 uppercase tracking-[0.3em] max-w-2xl leading-relaxed">
                    {t('games.theater.subtitle', 'Explorez la bibliothèque des Visual Novels générés par la Forge.')}
                </p>
            </div>
        ...
        {!vns || vns.length === 0 ? (
          <div className="text-center py-32 opacity-20 border-4 border-dashed border-white/5 rounded-[4rem]">
              ...
              <p className="text-2xl font-black italic manga-font uppercase">{t('games.theater.empty', 'Aucun Visual Novel archivé pour le moment')}</p>
          </div>
        ) : (
                ...
                <div className="p-8 space-y-6 flex-grow flex flex-col justify-between">
                  <div>
                    <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="text-[9px] font-black text-white/30 uppercase tracking-widest">{vn.creator_name || t('games.theater.anonymous', 'Anonyme')}</span>
                    </div>
        ...
        <div className="mt-32 p-12 rounded-[4rem] bg-navy-900/40 border border-white/5 flex flex-col md:flex-row items-center gap-12 text-center md:text-left">
            ...
            <div>
                ...
                <p className="text-sm font-bold opacity-40 uppercase leading-relaxed max-w-3xl italic text-justify">
                    {t('games.theater.legend_text', "Chaque Visual Novel est une création unique générée par des réseaux de neurones récurrents. Ils s'adaptent dynamiquement à vos choix et peuvent être remixés par la communauté pour explorer des embranchements narratifs alternatifs.")}
                </p>
            </div>
        </div>
      </div>
