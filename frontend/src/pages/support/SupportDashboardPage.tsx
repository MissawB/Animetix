...
const SupportDashboardPage: React.FC = () => {
  ...
  const createTicketMutation = useMutation({
    mutationFn: async (newTicket: { subject: string; query: string }) => {
      return supportService.createTicket(newTicket);
    },
    onSuccess: () => {
  ...
  const rateTicketMutation = useMutation({
    mutationFn: async ({ id, score }: { id: number; score: number }) => {
      return supportService.rateTicket(id, score);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['support-tickets'] });
    },
  ...
  return (
    <AnimatedPage>
      <div className="max-w-6xl mx-auto p-6 space-y-8 pt-24 pb-32">
        <Link
          to="/"
          className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-gray-500 hover:text-white transition-colors mb-4 no-underline group"
        >
          <ArrowLeft className="w-3 h-3 group-hover:-translate-x-1 transition-transform" /> Retour
          au Lobby
        </Link>
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-blue-500/20 pb-8 gap-6">
          <div>
            ...
            <p className="text-xs uppercase tracking-widest opacity-40 font-bold mt-2">
              Protocole de Diagnostic & Résolution IA
            </p>
          </div>
          <Button
            onClick={() => setShowNewTicketForm(true)}
            className="bg-blue-600 hover:bg-blue-500 shadow-lg shadow-blue-500/20 py-4 px-8 font-black uppercase italic tracking-widest"
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <aside className="space-y-6">
            <h2 className="text-[10px] font-black uppercase opacity-60 px-2 tracking-[0.3em]">
              Signaux Actifs
            </h2>
            <div className="space-y-4 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
              {isLoading ? (
                ...
                <p className="text-xs opacity-30 italic px-2 font-bold uppercase">
                ...
                tickets?.map((ticket: SupportTicket) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-2 gap-4">
                      <h3 className="font-black text-xs uppercase italic truncate flex-1">
                        {ticket.subject}
                      </h3>
                      <span
                        className={`text-[8px] px-2 py-0.5 rounded-full font-black uppercase shrink-0 ${ticket.status === 'resolved' ? 'bg-green-500/20 text-green-500 border border-green-500/30' : ticket.status === 'closed' ? 'bg-gray-500/20 text-gray-400 border border-gray-500/30' : 'bg-yellow-500/20 text-yellow-500 border border-yellow-500/30'}`}
                      >
                        {ticket.status}
                      </span>
                    </div>
                    <p className="text-[10px] opacity-40 uppercase font-bold font-mono">
          ...
          <main className="lg:col-span-2">
            {showNewTicketForm ? (
              <Card
                padding="lg"
                className="border-blue-500/30 bg-blue-500/5 shadow-2xl shadow-blue-500/10"
              >
                <h2 className="manga-font text-2xl mb-8 uppercase italic italic text-white font-black">
                  Initialiser Nouveau Diagnostic
                </h2>
              ...
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                {selectedTicket.ai_response && (
                  <Card
                    padding="lg"
                    className="border-cyan-500/30 bg-cyan-500/5 relative overflow-hidden shadow-2xl shadow-cyan-500/5"
                  >
                    ...
                    <div className="prose prose-invert max-w-none">
                      <p className="text-gray-200 leading-relaxed font-semibold text-lg italic">
                        {selectedTicket.ai_response}
                      </p>
                    </div>
                    ...
                    <div className="mt-12 pt-8 border-t border-white/10 flex flex-col sm:flex-row items-center justify-between gap-6">
                      ...
                      {selectedTicket.feedback_score === null && (
                        <div className="flex gap-4">
                          <button
                            onClick={() =>
                              rateTicketMutation.mutate({ id: selectedTicket.id, score: 1 })
                            }
                          ...
                          <button
                            onClick={() =>
                              rateTicketMutation.mutate({ id: selectedTicket.id, score: 0 })
                            }
                      ...
                      {selectedTicket.feedback_score !== null && (
                        <div
                          className={`flex items-center gap-2 px-6 py-3 rounded-xl font-black uppercase text-[10px] tracking-widest ${selectedTicket.feedback_score === 1 ? 'text-green-500' : 'text-red-500'}`}
                        >
                          {selectedTicket.feedback_score === 1 ? (
                            <ThumbsUp className="w-4 h-4" />
                          ) : (
                            <ThumbsDown className="w-4 h-4" />
                          )}
                          SIGNAL {selectedTicket.feedback_score === 1 ? 'POSITIF' : 'NÉGATIF'}{' '}
                          ENREGISTRÉ
                        </div>
                      )}
                    </div>
              ...
              <div className="h-[400px] flex flex-col items-center justify-center opacity-20 border-2 border-dashed border-white/10 rounded-3xl">
                ...
                <p className="font-black italic uppercase tracking-[0.4em] text-center text-white">
                  Sélectionnez un signal pour débuter le diagnostic
                </p>
              </div>
            )}
          </main>
