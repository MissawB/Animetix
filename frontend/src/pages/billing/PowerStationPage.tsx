...
import {
  ...
  ShieldCheck, CreditCard, ToggleLeft, ToggleRight,
  ArrowUpRight, ArrowDownLeft, ChevronLeft, ChevronRight, Volume2, VolumeX
} from 'lucide-react';
...
const PowerStationPage: React.FC = () => {
  ...
  const { isEnabled, setEnabled, timeLeft, totalMined, status: passiveStatus } = usePassiveMiningStore();
  ...
  const fetchLedger = useCallback(async (page: number, type: string, direction: string) => {
    if (!user) return;
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div className="space-y-8">
              ...
              >
                ...
                <div>
                  <span className="text-[9px] font-black uppercase text-gray-500 tracking-widest block mb-1">Total Balance</span>
                  <span className="text-4xl font-black italic manga-font text-white">{user?.wallet_balance?.toLocaleString() || 0} <span className="text-cyan-400">Bx</span></span>
                </div>
            ...
            <div className="lg:col-span-2 space-y-8">
              <h3 className="text-sm font-black uppercase tracking-[0.3em] text-gray-400 border-l-4 border-cyan-400 pl-4">Attention Mining & Reward Center</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <Card className="bg-gradient-to-br from-purple-950/20 to-black border-white/10 p-8 flex flex-col justify-between h-96 relative overflow-hidden">
                  ...
                  <div className="flex flex-col items-center justify-center space-y-2 py-4">
                    <div className="relative w-28 h-28 flex items-center justify-center">
                      ...
                      <div className="text-center z-10">
                        <span className="text-2xl font-black italic manga-font">{isEnabled ? `${Math.floor(timeLeft / 60)}:${String(timeLeft % 60).padStart(2, '0')}` : '--:--'}</span>
                        <span className="text-[8px] font-black uppercase tracking-widest text-gray-500 block mt-0.5">Cycle Timer</span>
                      </div>
                    </div>
                    <span className={`text-[9px] font-black uppercase tracking-widest px-3 py-1 rounded-full border ${
                      passiveStatus === 'ONLINE' ? 'bg-cyan-500/10 border-cyan-500/30 text-cyan-400 animate-pulse' :
                      passiveStatus === 'COOLDOWN' ? 'bg-yellow-500/10 border-yellow-500/30 text-yellow-400' :
                  ...
                  <div className="flex justify-between items-center text-[9px] text-gray-500 font-black uppercase tracking-widest">
                    <span>Total passive: <span className="text-cyan-400">{totalMined} Bx</span></span>
            ...
            <div className="bg-white/5 border border-white/10 rounded-[2rem] overflow-hidden">
               {isLoadingLedger ? (
                   ...
                   <table className="w-full text-left border-collapse min-w-[600px]">
                      <thead>
                        <tr className="border-b border-white/5 bg-white/5">
                          <th className="p-6 text-[10px] font-black uppercase tracking-[0.2em] text-gray-500">Transaction Description</th>
                          <th className="p-6 text-[10px] font-black uppercase tracking-[0.2em] text-gray-500">Source type</th>
                          <th className="p-6 text-[10px] font-black uppercase tracking-[0.2em] text-gray-500">Timestamp</th>
                          <th className="p-6 text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 text-right">Amount</th>
                        </tr>
                      </thead>
                      <tbody>
                          ...
                          <tr key={i} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                            ...
                            <td className="p-6">
                              <span className={`text-[8px] font-black uppercase px-3 py-1 rounded-full border ${
                                t.amount > 0 ? 'border-cyan-500/30 text-cyan-400' : 'border-purple-500/30 text-purple-400'
                              }`}>
                                {t.type.replace('_', ' ')}
                              </span>
                            </td>
                            <td className="p-6 text-xs text-gray-500 font-bold">{new Date(t.date).toLocaleString()}</td>
                            <td className={`p-6 text-right font-black italic manga-font flex items-center justify-end gap-1.5 ${t.amount > 0 ? 'text-green-400' : 'text-purple-400'}`}>
                              {t.amount > 0 ? (
                 ...
                 <div className="p-20 text-center space-y-4">
                    ...
                    <p className="text-gray-500 font-bold uppercase tracking-widest text-sm">No transaction ledger entries matched your query.</p>
                 </div>
               )}
