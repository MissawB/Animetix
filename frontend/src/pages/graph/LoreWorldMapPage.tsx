...
const LoreWorldMapPage: React.FC = () => {
  ...
  if (isError) return (
    <div className="max-w-7xl mx-auto px-6 py-32 text-center">
        <h2 className="text-3xl font-black text-red-500 italic uppercase">Erreur de Cartographie</h2>
        <p className="text-white/40 mt-4 uppercase font-bold tracking-widest">Impossible de charger les données macro-conceptuelles.</p>
    </div>
  );
                ...
                <Card padding="lg" className="bg-navy-950/50 border-white/10 rounded-[3rem] shadow-2xl">
                    ...
                    <div className="space-y-6">
                        <div className="flex justify-between items-end border-b border-white/5 pb-4">
                            ...
                            <span className="text-2xl font-black italic text-emerald-500">{communities?.length || 0}</span>
                        </div>
                        <div className="flex justify-between items-end border-b border-white/5 pb-4">
                    ...
                    <div className="mt-10 p-6 bg-emerald-500/5 rounded-3xl border border-emerald-500/10">
                        <p className="text-[9px] font-bold leading-relaxed opacity-40 uppercase italic">
                            Le partitionnement regroupe les œuvres non pas par titre, mais par densité de relations (thèmes communs, auteurs, studios, inspirations).
                        </p>
                    </div>
                </Card>

                <Card padding="lg" className="bg-white/5 border-white/5 opacity-50">
                    <h4 className="text-[10px] font-black uppercase tracking-widest mb-4">Légende Dynamique</h4>
                    <div className="space-y-4">
                        <div className="flex items-center gap-3">
                            <div className="w-3 h-3 rounded-full bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]" />
                            <span className="text-[10px] font-bold uppercase opacity-60">Forte Cohérence</span>
                        </div>
                        <div className="flex items-center gap-3">
                            <div className="w-3 h-3 rounded-full bg-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.5)]" />
                            <span className="text-[10px] font-bold uppercase opacity-60">Inter-connectivité</span>
                        </div>
                        <div className="flex items-center gap-3">
                            <div className="w-3 h-3 rounded-full bg-purple-500 shadow-[0_0_10px_rgba(168,85,247,0.5)]" />
                            <span className="text-[10px] font-bold uppercase opacity-60">Zone Expérimentale</span>
                        </div>
                    </div>
                </Card>
            </div>
            ...
            <div className="lg:col-span-8 space-y-12">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {communities?.map((community, idx) => (
                        ...
                        >
                            <Card padding="none" className="group overflow-hidden bg-black border-white/10 hover:border-emerald-500/40 transition-all duration-500 rounded-[2.5rem] h-full flex flex-col relative">
                                {/* Visual Accent */}
                                ...
                                <div className="p-8 space-y-6 flex-grow">
                                    <div className="flex justify-between items-start">
                                        ...
                                        <Badge variant="primary" className="bg-emerald-500/10 text-emerald-500 border-emerald-500/20 text-[8px] font-black uppercase">CLUSTER_{idx + 1}</Badge>
                                    </div>
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {community.entities?.slice(0, 3).map((entity: string) => (
                                            <span key={entity} className="text-[8px] font-black uppercase tracking-widest px-3 py-1 bg-white/5 border border-white/5 rounded-full opacity-40">
                                                {entity}
                                            </span>
                                        ))}
                ...
                <Card padding="lg" className="bg-emerald-500/5 border-emerald-500/20 flex flex-col md:flex-row items-center gap-8 rounded-[3rem]">
                    <div className="p-6 bg-emerald-500 rounded-3xl">
                        <Search className="w-12 h-12 text-white" />
                    </div>
                    <div className="text-center md:text-left">
                        <h4 className="text-2xl font-black italic manga-font uppercase mb-2">Macro-RAG Enabled</h4>
                        <p className="text-sm font-bold opacity-50 uppercase leading-relaxed tracking-wide">
                            Ces communautés ne sont pas seulement visuelles. Elles sont utilisées par l'Expert Nexus pour obtenir un contexte macro-conceptuel sur les sagas complètes avant d'affiner sa recherche.
                        </p>
                    </div>
                </Card>
            </div>
