...
export const GameHistoryPanel: React.FC = () => {
  const {
    ...
    queryFn: () => socialService.getGameplayHistory(10),
  });
  ...
  if (isError || !sessions) {
    return (
      <div className="text-center opacity-50 italic">
        Impossible de charger l'historique des jeux.
      </div>
    );
  }
  ...
  return (
    <Card padding="lg" className="bg-navy-900/40 border-white/5 h-full">
      <div className="flex items-center justify-between mb-8">
        ...
        <span className="text-[10px] font-black uppercase bg-blue-500/20 text-blue-400 px-3 py-1 rounded-full">
          10 Derniers
        </span>
      </div>
      ...
      <div className="space-y-4 overflow-y-auto pr-2" style={{ maxHeight: '400px' }}>
        {sessions.map((session) => (
          <div
            key={session.id}
            className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 bg-white/5 rounded-2xl border border-white/5 hover:bg-white/10 transition-colors"
          >
            <div className="flex items-center gap-4">
              <div
                className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${session.was_won ? 'bg-green-500/20 text-green-500' : 'bg-red-500/20 text-red-500'}`}
              >
                {session.was_won ? <Check className="w-6 h-6" /> : <X className="w-6 h-6" />}
              </div>
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-sm font-black uppercase tracking-widest bg-white/10 px-2 py-0.5 rounded text-white">
                    {session.game_mode}
                  </span>
                  <span className="text-xs opacity-60 font-bold">{session.media_type}</span>
                </div>
                <p className="font-bold text-sm">
                  Cible : <span className="text-yellow-500 italic">{session.target_item}</span>
                </p>
              </div>
            </div>
