...
- [x] **~~Une dépendance ajoutée au brain n'arrive JAMAIS en prod — et le workflow reste vert~~** _(constaté 2026-07-14, corrigé 2026-07-15)_
  - **Fix livré** : `Dockerfile.brain` est découpé en couches cacheables (tooling → merge → quantize), `cloudbuild.yaml` passe `--cache-from` sur l'étape brain, et `ci.yml` a un nouveau job `deploy-brain-to-prod` avec checkbox `deploy_brain` indépendante. Le canary (no-traffic → smoke `/health` → promote) est identique au web. `Dockerfile.brain.encoders` (image dérivée temporaire) a été supprimé.
  - **Premier build** : sera un build froid (~2 h, comme avant). Les suivants touchent le cache gguf et terminent en ~10 min. Pour amorcer le cache, pousser `brain:otaku3` comme `:latest` avant le premier run CI.
...
- [x] **~~Backend — vue `multiverse.py` de 280 lignes : Cypher + PDF inline~~** _(audit dette 2026-07-11)_
...
- [ ] **`hybrid_search` rebranché sur CLIP — reste à MESURER la pertinence avant de déployer** _(rebranché 2026-07-15, commit 27c662d5)_
  - **Fait (option a).** La moitié « sémantique » de `hybrid_search` encode désormais la requête par la tour texte de CLIP (`VisualIndexService.encode_text`, qui n'avait aucun appelant — l'autre TODO est donc **fermé**) et cherche dans `unified_clip_space` (9 165 vraies jaquettes), filtrée par `media_type`. Avant, elle interrogeait `anime_thematic` (vide) et `UnifiedRepositoryAdapter` retombait en silence sur un `ILIKE` SQL — une « recherche sémantique » qui n'en était pas une. `search_by_vector` (port + adapter) et `VisualIndexService.search` ont gagné un filtre `where` rétrocompatible.
  - **Le piège d'id, évité et testé.** Le lexical connaît une œuvre par son id AniList (101922), l'index CLIP l'a écrite sous son MAL id (38000) ; RRF fusionne sur l'id. On normalise la clé lexicale sur `idMal or id` — la MÊME règle que `build_visual_index`. Un test d'aller-retour prouve qu'une œuvre présente des deux côtés fusionne en UNE entrée. 52 tests verts, mypy/ruff/black OK, conteneur DI sans cycle.
  - **DETTE, à faire AVANT tout déploiement** : la **pertinence** n'est pas mesurée. `unified_clip_space` indexe des **jaquettes** — c'est de la proximité visuelle. Qu'une requête texte (« une romance mélancolique ») y ramène des œuvres thématiquement sensées reste à prouver sur les 9 165 vecteurs de prod (tunnel Cloud SQL injoignable le 07-15 au soir). Si la proximité visuelle s'avère à côté du sujet pour du RAG texte, l'option **(b)** reste ouverte : assumer deux recherches lexicales, renommer `semantic_results`, rendre le fallback `ILIKE` visible.

- [ ] **Les collections thématiques (`anime_thematic`, `manga_thematic`, `character_vibe`…) sont vides** _(inféré 2026-07-15, compte DB non confirmé : tunnel Cloud SQL injoignable ce soir)_
  - Cette branche n'a peuplé que `unified_clip_space` (9 165 jaquettes). Les collections que `search_media_items` interroge par `media_type` ([pgvector_repository_adapter.py:55](backend/adapters/persistence/pgvector_repository_adapter.py#L55) : `Anime→anime_thematic`, etc.) n'ont jamais été alimentées — l'ancien `vectorize_anime.py` ne pouvait pas tourner. Donc toute recherche sémantique par `media_type` rend `[]` **légitimement** (collection vide), pas à cause d'un bug.
  - **À confirmer dès que la DB est joignable** : `select collection_name, count(*) from animetix_vectorrecord group by 1`. Si `anime_thematic` = 0, décider si on le peuple (le pipeline thématique ne sert plus la proximité du jeu classique depuis la refonte du 2026-07-13) ou si on retire ces chemins.
