...
- [~] **Couverture de tests frontend** — ✅ **outillage ajouté** : script `test:coverage` (`vitest run --project unit --coverage`), config v8 (reporters text/html/lcov, include/exclude), **seuils plancher anti-régression** (stmts 18 / branches 10 / funcs 14 / lines 18), `coverage/` gitignoré. **Baseline mesurée ≈ 18 % stmts** (1088/5981 ; 188 tests). ✅ **Campagne ROI élevé (round 1)** : +112 tests (services API, utils purs queryClient/SoundManager/observability/apiClient, hooks React Query) → **18.19 % → 21.58 % stmts** (188 → 300 tests), plancher remonté à 21/12/19/21. ⏭️ Reste : (a) **élargir** davantage (composants/pages — plus lourd), (b) **câbler les tests unit front en CI** (constat : `vitest` ne tourne **pas** en CI aujourd'hui, seuls check-types + lint + e2e a11y).
