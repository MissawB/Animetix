...
- [ ] **Backend — exposer le streaming async aux endpoints HTTP** _(suite du streaming async natif)_
  ...
  - ⏳ **Reste** : AgenticRAG SP2+ — remplacer le pont par des `aprocess` natifs par processor (FallbackRag via `astream_generate`, Synthesize via variante async, Research/Graph/Saga agents+Neo4j en async/`to_thread`) pour du vrai async I/O ; puis nettoyage du sync inutilisé. (Aparté : harmoniser `Cache-Control: no-cache` sur toutes les vues SSE.)
