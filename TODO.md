...
- [ ] **Index visuel : phase 2 — peupler `character_ccip_space` (~35 000 portraits)** _(phase 1 livrée 2026-07-14 ; chaîne CCIP dé-risquée 2026-07-16, cf. HISTORY)_
  - `character_ccip_space` est **vide** (recherche perso → 503 honnête, sans facturer) ; la chaîne `--target character` est prouvée de bout en bout par un test d'intégration réel. **Reste** : lancer le job prod `python manage.py build_visual_index --target character` (Brain up ; reprenable, image morte sautée). Décision « tour texte CLIP non exposée » déjà tranchée (cf. HISTORY) — ne pas rebrancher `encode_text`.
