...
- [ ] **`advanced_rag_service.hybrid_search` : la moitié sémantique est morte, et le service n'est exposé nulle part** _(vérifié par le code 2026-07-15 ; correction d'une fausse alerte du 2026-07-14)_
  - Le fix métadonnées du 2026-07-14 (`search_media_items` faisait `dict(meta)` sur une **chaîne** — Django désactive le décodage `jsonb` de psycopg2, tout curseur SQL brut reçoit une `str` ; l'exception était avalée en `[]`) était réel et nécessaire, **mais il ne « rallume » rien de visible**, contrairement à ce que j'avais écrit. Vérification faite :
    - Le chemin de recherche **que voient les utilisateurs**, `GET /api/v1/search/?q=`, passe par `catalog_service` câblé sur `django_repository` = **SQL `LIKE` pur** ([core_services.py:133](backend/api/animetix/containers/core_services.py#L133)). Il ne touche jamais la branche sémantique. Il marche et n'a jamais été concerné.
    - La branche sémantique corrigée n'a que **deux** appelants : `advanced_rag_service.hybrid_search`, qui interroge `anime_thematic` (voir ci-dessous) et **n'est exposé par aucun endpoint HTTP** (zéro appelant dans `backend/api/`) — code dormant ; et `video_rag_service.search_video_segment` ([labs/video.py](backend/api/animetix/api/labs/video.py)), qui a un garde `is_available()` **levant** `SearchIndexUnavailableError` sur collection vide, **avant** de facturer — donc pas de `[]` silencieux, refus franc.
  - **À faire** : soit exposer `hybrid_search`, soit le retirer comme le reste de la surface morte. Le fix métadonnées reste acquis (prouvé sur `unified_clip_space`, empêche un 500 le jour où une collection thématique sera peuplée).
