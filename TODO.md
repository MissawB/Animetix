...
- [ ] **Backend — exposer le streaming async aux endpoints HTTP** _(suite du streaming async natif)_
  ...
  - ⏳ **Reste** : AgenticRAG SP2+ — remplacer le pont par des `aprocess` natifs par processor (FallbackRag via `astream_generate`, Synthesize via variante async, Research/Graph/Saga agents+Neo4j en async/`to_thread`) pour du vrai async I/O ; puis nettoyage du sync inutilisé (+ retrait du legacy `animetix.views.api.agentic_rag_stream`, non routé). (Aparté : `Cache-Control: no-cache` est désormais posé par `sse_stream_response` → couvre Emoji/Paradox/ToT/AgenticRAG ; reste seulement `AniminatorStreamView` qui construit son `StreamingHttpResponse` directement.)
