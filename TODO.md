...
- [/] **Index visuel : phase 2 — peuplement `character_ccip_space` LANCÉ (~35 000 portraits)** _(phase 1 livrée 2026-07-14 ; run lancé 2026-07-17)_
  - **En cours** : job Cloud Run `animetix-build-character-index` **créé** (config scopée) ; **smoke test OK** (48/50 vecteurs écrits, Brain CCIP ~0,3 s/portrait) ; **run complet en cours** (exécution `…-lznfm`, ~3 h attendu, reprenable). Décision « tour texte CLIP non exposée » déjà tranchée (cf. HISTORY) — ne pas rebrancher `encode_text`.
  - **Reste** : laisser le run finir ; si timeout 6 h avant les 35k, **re-exécuter** (`gcloud run jobs execute animetix-build-character-index --region=europe-west9`) jusqu'à couverture complète ; vérifier que la recherche perso ne renvoie plus 503.
