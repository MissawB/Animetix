...
- [ ] **Backend — exposer le streaming async aux endpoints HTTP** _(suite du streaming async natif)_
  ...
  - ⏳ **Reste** : convertir les 4 autres vues SSE (Emoji/Paradox/AgenticRAG/ToT) — chacune nécessite d'abord son chemin service async (`a*_stream`), puis la même bascule vue Django async. Décider si on généralise (helper commun) ou au cas par cas. (Aparté : harmoniser `Cache-Control: no-cache` sur toutes les vues SSE — seul AgenticRAG le pose aujourd'hui.)
