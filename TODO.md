...
- [ ] **Backend — exposer le streaming async aux endpoints HTTP** _(suite du streaming async natif)_
  ...
  - ⏳ **Reste** : réécriture HTTP async des vues SSE bloquée par DRF 3.17 sync-only (pas d'`adrf`) → décider `adrf` vs vue Django native quand un consommateur `gather` réel existera.
