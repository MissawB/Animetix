...
- [/] **Frontend — régression du plafond « 0 pages > 500 lignes » — verrou eslint posé ✅, décomposition restante** _(audit dette 2026-07-22)_
  - **Fait (2026-07-23)** : règle `max-lines: 500` (lignes brutes, comme le `wc -l` des audits) dans `eslint.config.js`, avec 3 exemptions explicites = la dette connue (`CovertestPage.tsx` 694, `ClassicLobbyPage.tsx` 519, `types/index.ts` 704 — ce dernier tracké dans l'item moyen dédié). Discrimination vérifiée (sans exemption → erreur). Bonus : artefacts Playwright/coverage ajoutés aux ignores (le lint local explosait à ~4000 erreurs vendored).
  - **Reste** : décomposer `CovertestPage.tsx` (FX inline l.35-70 extractibles vers un util testable) et `ClassicLobbyPage.tsx` (pattern PR 93-96), puis retirer leurs exemptions. Ne jamais AJOUTER d'exemption — décomposer à la place (commentaire en place dans la config).
