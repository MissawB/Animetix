...
def test_brain_api_host_default_logic():
    ...
    with patch.dict(os.environ, {}, clear=True):
        ...
        assert host == "127.0.0.1", "Host should default to 127.0.0.1 for security (B104)"
...
def test_brain_api_host_override_logic():
    ...
    with patch.dict(os.environ, {"BRAIN_API_HOST": custom_host}):
        host = os.getenv("BRAIN_API_HOST", "127.0.0.1")
        assert host == custom_host, f"Host should respect BRAIN_API_HOST env var, got {host}"
...
def test_production_file_contains_logic():
    ...
    brain_api_path = project_root / "backend" / "adapters" / "inference" / "brain_api.py"
    ...
    assert expected_logic in content, f"Production file {brain_api_path} must contain secure host logic: {expected_logic}"
