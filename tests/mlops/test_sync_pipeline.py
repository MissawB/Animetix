...
from scripts.sync_gold_ground_truth import run_synchronization, fetch_neo4j_facts, update_regression_benchmark_file
...
def mock_neo4j():
    ...
    mock_rels = [{"rel_type": "PRODUCED_BY", "target_name": "ufotable", "target_labels": ["Studio"]}]
...
def mock_inference_engine():
    ...
    mock_sync_result.updated_ground_truth = "Demon Slayer has 74 episodes and is produced by ufotable."
...
def test_sync_gold_ground_truth_pipeline(mock_get_container, mock_neo4j, mock_inference_engine, tmp_path):
    ...
    with patch("scripts.sync_gold_ground_truth.PROJECT_ROOT", str(tmp_path)), \
         patch("backend.pipeline.evaluation.regression_benchmark.GOLD_SET", mock_benchmark_gold_set), \
         patch("scripts.sync_gold_ground_truth.update_regression_benchmark_file") as mock_update_file:
        ...
        assert updated_data[0]["ground_truth"] == "Demon Slayer has 74 episodes and is produced by ufotable."
        ...
        assert updated_benchmark_list[0]["expected_facts"] == [["74 episodes", "74 épisodes"], "ufotable"]
...
"""
    ...
    os.makedirs(os.path.join(tmp_path, "backend", "pipeline", "evaluation"), exist_ok=True)
    real_file_path = os.path.join(tmp_path, "backend", "pipeline", "evaluation", "regression_benchmark.py")
