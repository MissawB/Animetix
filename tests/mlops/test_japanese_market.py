...
from backend.pipeline.mlops.index_otaku_knowledge import OtakuKnowledgeIndexer
...
def test_japanese_market_db_size():
    ...
    assert len(JAPANESE_MANGA_PUBLISHERS) == 15, "Doit contenir exactement 15 éditeurs japonais"
    assert len(JAPANESE_ANIME_DISTRIBUTORS) == 10, "Doit contenir exactement 10 diffuseurs japonais"
    assert len(JAPANESE_MARKET_RELATIONS) == 40, "Doit contenir exactement 40 relations japonaises"
    assert len(JAPANESE_VOICE_ACTORS) >= 10, "Doit réutiliser les seiyuu japonais (au moins 10)"
...
def test_indexer_integration():
    ...
    jp_publishers_facts = [f for f in facts if f["category"] == "Japanese Manga Publishers"]
    jp_distributors_facts = [f for f in facts if f["category"] == "Japanese Anime Distributors (JP)"]
...
def test_sft_instruction_generators():
    ...
    assert len(profile_instructions) == 600, f"Attendu 600 profils, obtenu {len(profile_instructions)}"
    # 40 relations * 4 variations = 160
    assert len(relations_instructions) == 160, f"Attendu 160 relations, obtenu {len(relations_instructions)}"
...
def test_dpo_corruption_regional_partition():
    ...
    for jp_pub in JAPANESE_PUBLISHERS_SET:
        ...
        for item in related:
            assert item in JAPANESE_PUBLISHERS_SET, f"Un éditeur japonais ({jp_pub}) ne peut être remplacé que par un autre éditeur japonais. Obtenu: {item}"
            
    for jp_dist in JAPANESE_DISTRIBUTORS_SET:
        related = RELATED_ENTITIES_MAP.get(jp_dist, [])
        for item in related:
            assert item in JAPANESE_DISTRIBUTORS_SET, f"Un diffuseur japonais ({jp_dist}) ne peut être remplacé que par un autre diffuseur japonais. Obtenu: {item}"
    ...
    for fr_pub in french_pubs:
        ...
        for item in related:
            assert item not in JAPANESE_PUBLISHERS_SET, f"Un éditeur français ({fr_pub}) ne peut pas être remplacé par un éditeur japonais. Obtenu: {item}"
