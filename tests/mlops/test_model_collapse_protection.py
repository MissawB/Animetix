...
def mock_gold_port():
    ...
    port.get_unprocessed_validated_entries.side_effect = (
        get_unprocessed_validated_entries
    )
...
def test_promotion_service_execution(mock_gold_port, mock_neo4j):
    ...
    mock_gold_port.save_synthetic_entry(
        "MULTIVERSE", "Context", "Instr", "Resp", universe_data
    )
    ...
    promotion_service = SyntheticDataPromotionService(
        gold_dataset_port=mock_gold_port, domain_synthesizer=mock_synthesizer
    )
