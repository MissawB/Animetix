...
def test_deploy_security_calls_gcloud(mock_run):
    ...
    assert any("compute security-policies describe animetix-edge-security-policy" in c for c in flat_calls)
    ...
    assert any("compute security-policies rules update 1000" in c or "compute security-policies rules create 1000" in c for c in flat_calls)
