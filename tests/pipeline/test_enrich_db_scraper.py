...
def test_run_enrichment_flow(mock_update_json, mock_fetch_jikan, mock_translate, mock_media_item, mock_django_media_item):
    from pipeline.enrich_db_scraper import run_enrichment
    
    # Mocking Django querysets
    mock_qs = MagicMock()
    mock_qs.exclude.return_value.order_by.return_value.__getitem__.return_value = [mock_django_media_item]
