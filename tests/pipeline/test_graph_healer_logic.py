...
def mock_django_models(mocker):
    mock_ticket = mocker.patch(
        "animetix.models.DataCurationTicket.objects.get_or_create"
    )
...
def test_run_graph_healer_no_anomalies(
    mock_django_models, mock_neo4j, tmp_path, mocker
):
    ...
    with patch(
        "pipeline.mlops.graph_healer.BASE_DIR", str(tmp_path.parent.parent)
    ):  # Adjust BASE_DIR
...
def test_run_graph_healer_with_anomaly(mock_django_models, mock_neo4j, tmp_path):
    ...
    with (
        ...
        patch(
            "builtins.open",
            pytest.importorskip("unittest.mock").mock_open(read_data=json.dumps(data)),
        ),
    ):
        run_graph_healer(limit=1)
