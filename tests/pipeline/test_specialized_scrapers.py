...
def test_run_tripartite_enrichment_flow(
    ...
    from pipeline.specialized_scrapers import run_tripartite_enrichment  # noqa: E402
    ...
    mock_scrape_casting.return_value = [
        {
            "character": "Ichigo",
            "role": "Main",
            "seiyuu_vo": "Morita",
            "doubleur_vf": "de Bouard",
        }
    ]
    ...
    assert (
        mock_django_media_item.metadata["themes_musicaux"][0]["song_title"]
        == "Asterisk"
    )
    assert (
        mock_django_media_item.metadata["critiques_fr"]["score_global_fr"] == "9.0/10"
    )
