...
def test_scraper_e_igdb(mock_post):
    ...
    with patch.dict(
        "os.environ", {"IGDB_CLIENT_ID": "id_123", "IGDB_CLIENT_SECRET": "sec_123"}
    ):
...
def test_run_tripartite_enrichment_flow(
    ...
    from pipeline.advanced_scrapers import run_tripartite_enrichment  # noqa: E402

    # Mocking Django QuerySet
    mock_media_item.objects.filter.return_value.order_by.return_value = [
        mock_django_media_item
    ]
    ...
    mock_get_tropes.return_value = [
        {"trope": "Test Trope", "description": "Trope description."}
    ]
    ...
    assert (
        mock_django_media_item.metadata["narrative_arcs"]["arcs"][0]["arc_name"]
        == "Test Arc"
    )
    assert mock_django_media_item.metadata["video_games"][0]["name"] == "Naruto Game"
    assert (
        mock_django_media_item.metadata["narrative_tropes"][0]["trope"] == "Test Trope"
    )
