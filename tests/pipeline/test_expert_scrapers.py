...
def test_run_tripartite_enrichment_flow(
    ...
    from pipeline.expert_scrapers import run_tripartite_enrichment  # noqa: E402
    ...
    mock_get_recs.return_value = [
        {"recommended_title": "Hunter X Hunter", "reason": "Adventure."}
    ]
    ...
    assert (
        mock_django_media_item.metadata["streaming_platforms"][0]["platform"]
        == "Netflix"
    )
    assert (
        mock_django_media_item.metadata["human_recommendations"][0]["recommended_title"]
        == "Hunter X Hunter"
    )
    assert (
        mock_django_media_item.metadata["real_locations_pilgrimage"][0]["location_name"]
        == "Landmark"
    )
