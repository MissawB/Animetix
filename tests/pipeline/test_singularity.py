...
def test_self_evolving_compiler():
    from src.core.domain.services.self_evolving_compiler import SelfEvolvingCompiler
    
    compiler = SelfEvolvingCompiler(build_dir="data/mlops/build_test")
    
    # 1. Optimisation d'un goulot d'étranglement fictif
    slow_logic = "double similarity = compute(a, b);"
    optimized_fn = compiler.analyze_and_optimize("cosine_sim", slow_logic)
...
def test_synaptic_plasticity_hebbian_and_stdp():
    from src.core.domain.services.synaptic_plasticity import SynapticPlasticitySimulator
...
def test_autonomous_domain_synthesizer():
    from src.core.domain.services.domain_synthesizer import AutonomousDomainSynthesizer
    ...
    assert universe["episodes"][0]["summary"] == "Shinji active le réacteur antigravité."
    ...
    synthesizer_no_llm = AutonomousDomainSynthesizer(inference_engine=None, neo4j_manager=None)
    ...
    poor_persistence_success = synthesizer_no_llm.persist_universe_to_graph(poor_universe)
