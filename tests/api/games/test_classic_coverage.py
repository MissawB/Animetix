...
def _reset_container_overrides():
    # dependency_injector overrides mutate the global container singleton; reset
    # before and after each test so a leaked override can't cross test boundaries
    # (the `_override` helper applies overrides eagerly without a `with`).
