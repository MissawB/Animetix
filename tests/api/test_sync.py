...
def test_sync_offline_data_invalid_json(api_client):
    ...
    response = api_client.post(url, data="malformed{json", content_type="application/json")
...
def test_sync_offline_data_daily_cap(api_client):
    ...
    assert response.json().get("error") == "Daily offline XP limit reached. Play online for more!"
