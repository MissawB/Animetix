...
from animetix.admin import GoldDatasetEntryAdmin, AIFeedbackAdmin, AISafetyEventAdmin, AIREvalResultAdmin
...
def test_gold_dataset_entry_admin_promote_selected(mock_get_container, admin_site, request_factory):
    entry = GoldDatasetEntry.objects.create(
...
@pytest.mark.django_db
@patch('animetix.admin.get_container')
def test_gold_dataset_entry_admin_promote_view(mock_get_container, admin_site, request_factory):
    mock_promotion_service = MagicMock()
    mock_promotion_service.promote_validated_entries.return_value = {
        "promoted": 2, 
        "details": {"QA": 2}
...
def test_changelist_view_adds_stats(admin_site, request_factory):
    GoldDatasetEntry.objects.create(instruction="I1", context="C1", response="R1", entry_type="QA", is_validated=False)
    GoldDatasetEntry.objects.create(instruction="I2", context="C2", response="R2", entry_type="MULTIVERSE", is_validated=False)
    GoldDatasetEntry.objects.create(instruction="I3", context="C3", response="R3", entry_type="QA", is_validated=True)
    ...
    with patch('django.contrib.admin.ModelAdmin.changelist_view') as mock_super_changelist:
