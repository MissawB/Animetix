...
def test_neural_diagnostics_api_success(api_client):
    ...
    with patch('animetix.api.labs.get_container') as mock_get_container:
        ...
        mock_response = InferenceResponse(
            ...
            metadata=InferenceMetadata(logprobs=[TokenLogProb(token="Naruto", logprob=-0.1)])
        )
        ...
        mock_xai.get_diagnostics_report.assert_called_once_with(payload['prompt'], mock_response)
