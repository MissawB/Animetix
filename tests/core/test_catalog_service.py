...
def catalog_service(mock_repository, mock_sql_repository):
    return CatalogService(repository=mock_repository, sql_repository=mock_sql_repository)

def test_get_catalog_sql_success(catalog_service, mock_sql_repository):
    mock_sql_repository.get_catalog_by_type.return_value = [{'id': 1, 'title': 'Naruto'}]
...
def test_get_catalog_fallback_to_file(catalog_service, mock_sql_repository, mock_repository):
