...
@pytest.fixture
def ragas_service(mock_engine):
    return RagasEvalService(judge_engine=mock_engine)

def test_evaluate_response_success(ragas_service, mock_engine):
    ...
    with patch("animetix.models.AIREvalResult.objects.create") as mock_create:
        res = ragas_service.evaluate_response("q", "c", "r")
        assert res["faithfulness"] == 0.8
        assert res["answer_relevancy"] == 0.9
        assert res["context_precision"] == 1.0
        mock_create.assert_called_once()

def test_score_faithfulness_fallback(ragas_service, mock_engine):
    mock_engine.generate.return_value = "invalid"
    assert ragas_service._score_faithfulness("c", "r") == 0.5

def test_score_precision_fallback(ragas_service, mock_engine):
    mock_engine.generate.return_value = "error"
    assert ragas_service._score_precision("q", "c") == 0.0
