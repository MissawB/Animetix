...
from adapters.persistence.django_repository_adapter import DjangoRepositoryAdapter, is_alloydb_nl_query_supported
...
def test_sql_guard_accepts_valid_queries():
    ...
    assert validate_sql_query("SELECT title, release_year, rating FROM animetix_mediaitem WHERE rating > 8.5") is True
...
def test_sql_guard_rejects_non_select():
    ...
    # Mutating statements
    assert validate_sql_query("INSERT INTO animetix_mediaitem (title) VALUES ('Attack on Titan')") is False
    assert validate_sql_query("UPDATE animetix_mediaitem SET rating = 9.0 WHERE id = 1") is False
...
def test_sql_guard_rejects_comments_and_chaining():
    # SQL Comments
    assert validate_sql_query("SELECT * FROM animetix_mediaitem -- comment here") is False
    assert validate_sql_query("SELECT * FROM animetix_mediaitem /* block comment */") is False
    # Multiple statements / Query chaining
    assert validate_sql_query("SELECT * FROM animetix_mediaitem; SELECT * FROM auth_user") is False
    assert validate_sql_query("SELECT * FROM animetix_mediaitem; DROP TABLE animetix_profile;") is False
...
def test_sql_guard_rejects_forbidden_tables():
    ...
    assert validate_sql_query("SELECT * FROM animetix_mediaitem JOIN auth_user ON animetix_mediaitem.id = auth_user.id") is False

def test_sql_guard_rejects_union_injection():
    # Union injection detection
    assert validate_sql_query("SELECT title FROM animetix_mediaitem UNION SELECT username FROM auth_user") is False
...
def test_is_alloydb_nl_query_supported_success():
    with patch('django.db.connection.cursor') as mock_cursor:
        ...
        import adapters.persistence.django_repository_adapter
...
@patch('adapters.persistence.django_repository_adapter.is_alloydb_nl_query_supported', return_value=True)
...
def test_query_data_natural_language_native_flow(mock_supported, mock_cursor):
    ...
    mock_cursor_obj.fetchone.return_value = ("SELECT title FROM animetix_mediaitem WHERE rating > 9.0;",)
...
@patch('django.db.connection.cursor')
@patch('adapters.persistence.django_repository_adapter.is_alloydb_nl_query_supported', return_value=False)
@pytest.mark.django_db
def test_query_data_natural_language_fallback_flow(mock_supported, mock_cursor):
    ...
    # Mock LLMService
    mock_llm = MagicMock()
    mock_llm.generate.return_value = "SELECT title, release_year FROM animetix_mediaitem WHERE release_year = 2024"
    ...
    adapter = DjangoRepositoryAdapter()
    results = adapter.query_data_natural_language("Show anime released in 2024", llm_service=mock_llm)
    ...
    mock_cursor_obj.execute.assert_called_with("SELECT title, release_year FROM animetix_mediaitem WHERE release_year = 2024")
...
def test_query_data_natural_language_blocked_by_guardrail(mock_supported, mock_cursor):
    ...
    adapter = DjangoRepositoryAdapter()
    results = adapter.query_data_natural_language("Show all users", llm_service=mock_llm)
    ...
    mock_cursor_obj = mock_cursor.return_value.__enter__.return_value
    assert not any("auth_user" in str(call) for call in mock_cursor_obj.execute.call_args_list)
...
@patch('adapters.persistence.django_repository_adapter.DjangoRepositoryAdapter.query_data_natural_language')
