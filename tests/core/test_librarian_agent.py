...
from core.domain.entities.ai_schemas import RAGState, JudgeAction, SearchPlan, DebateOutcome, JudgeEvaluation, StreamStep
...
def mock_dependencies():
    ...
    inference_engine.generate.return_value = '{"thinking_budget": 100, "complexity_score": 3}'
...
def test_librarian_loop_integration(mock_dependencies):
    ...
    with patch.object(service, '_execute_search', return_value=([], "Initial context")):
        ...
        # Run the stream
        events = list(service.plan_and_solve_stream("Quand est sorti Naruto Ep 5 ?", "anime"))
        ...
        assert any("[Uncertainty] Basse confiance détectée (0.30). Déclenchement du Librarian..." in t for t in thoughts)
