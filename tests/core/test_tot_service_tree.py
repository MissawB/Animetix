...
def test_tot_returns_full_tree_structure():
    ...
    assert "nodes" in result["full_tree"], "L'arbre complet doit contenir une liste de nœuds"
    assert "links" in result["full_tree"], "L'arbre complet doit contenir une liste de liens"
...
def test_tot_captures_generated_nodes():
    ...
    assert len(links) == 3
    assert any(l["source"] == "node_0_0" and l["target"] == "node_1_0_0" for l in links)
    assert any(l["source"] == "node_0_0" and l["target"] == "node_1_0_1" for l in links)
