...
def vision_quest_service(mock_inference_engine, mock_vision_service):
    return VisionQuestDomainService(inference_engine=mock_inference_engine, vision_service=mock_vision_service)
...
def test_get_image_style_info(vision_quest_service, mock_vision_service):
    ...
    mock_vision_service.identify_artist_style.assert_called_once_with(b"fake_image_data")
...
def test_calculate_score(vision_quest_service, mock_inference_engine):
    ...
    score = vision_quest_service.calculate_score("This is My Secret", "id1", "My Secret", "Anime")
    assert score == 100.0
