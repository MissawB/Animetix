...
def sample_catalog():
    return [
        {'id': '1', 'title': 'Naruto', 'genres': ['Action'], 'micro_tags': ['ninja'], 'studios': ['Pierrot']},
        {'id': '2', 'title': 'One Piece', 'genres': ['Adventure'], 'micro_tags': ['pirate'], 'studios': ['Toei']}
    ]
...
def test_rl_env_step_incorrect(rl_env):
    ...
    action_idx = rl_env.attributes.index('genre:Action') # Is it Action? No, One Piece is Adventure.
    state, reward, terminated, truncated, info = rl_env.step(action_idx)
