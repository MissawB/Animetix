...
from core.domain.entities.ai_schemas import RAGState, ForgeHypothesis, DebateOutcome, JudgeAction, JudgeEvaluation, SearchPlan
...
def mock_rag():
    rag = MagicMock()
    rag.hybrid_search.return_value = [{'title': 'Local Info', 'description': 'Some local data'}]
    return rag
...
@pytest.fixture
def agentic_rag(mock_engine, mock_rag, mock_web, mock_prompt_manager, mock_librarian, mock_forge, mock_uncertainty, mock_debate_manager):
...
def test_forge_speculation_e2e(agentic_rag, mock_engine, mock_librarian, mock_forge, mock_uncertainty, mock_debate_manager):
    """
    ...
    agentic_rag.synthesizer.synthesize_stream.return_value = iter(["First ", "attempt."])
    ...
    mock_librarian.identify_gap.return_value = {"query": "missing detail", "source_type": "Web"}
    ...
    mock_debate_manager.conduct_debate.return_value = DebateOutcome(
        ...
        final_reasoning="Looks good"
    )

    # Execute
    events = list(agentic_rag.plan_and_solve_stream("Will GTA 6 be on PC at launch?", "Game"))
    ...
    assert any("[Librarian] Aucune donnée supplémentaire trouvée. Passage en mode spéculation..." in t for t in thoughts)
    assert any("[The Forge] Hypothèse générée : Forged hypothesis" in t for t in thoughts)
