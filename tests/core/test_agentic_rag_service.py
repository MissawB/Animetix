...
from core.domain.services.agentic_rag_service import AgenticRAGService, RAGState, StreamStep
...
def test_plan_and_solve_local_path(agentic_rag, mock_engine, mock_rag):
    ...
    outcome = DebateOutcome(consensus_action=JudgeAction.APPROVE, final_reasoning="ok", critiques={})
...
def test_plan_and_solve_with_reformulation(agentic_rag, mock_engine, mock_rag, mock_web):
    ...
    outcome1 = DebateOutcome(consensus_action=JudgeAction.REPLAN, final_reasoning="Need more", critiques={})
...
def test_vlm_rerank_path(agentic_rag, mock_engine, mock_rag):
    plan = SearchPlan(optimized_query="visual query", requires_web=False, is_visual_query=True, reasoning="visual search")
    agentic_rag.planner.plan.return_value = plan
    agentic_rag.scout.find_truth_path.return_value = "Truth Path"
    ...
    for step in agentic_rag.plan_and_solve_stream("Visual query", "Anime"):
        if isinstance(step, dict) and step['type'] == 'thought' and "[State Machine]" in step['content']:
...
def test_thinking_mode_streaming(agentic_rag, mock_engine):
    ...
    plan = SearchPlan(optimized_query="complex query", requires_web=False, reasoning="R")
    agentic_rag.planner.plan.return_value = plan
    agentic_rag.scout.find_truth_path.return_value = "Truth Path"
    ...
    thoughts = [s['content'] for s in steps if isinstance(s, dict) and s['type'] == 'thought' and "process" in str(s['content'])]
    assert len(thoughts) > 0
    
    # Check that tokens are yielded as tokens
    tokens = "".join([str(s['content']) for s in steps if isinstance(s, dict) and s['type'] == 'token'])
...
def test_fallback_on_inference_error(agentic_rag, mock_engine, mock_rag):
    ...
    assert any("[Recovery] Erreur" in str(s['content']) for s in steps if isinstance(s, dict) and s['type'] == 'thought')
    assert any("RAGState.FALLBACK_RAG" in str(s['content']) for s in steps if isinstance(s, dict) and s['type'] == 'thought')
