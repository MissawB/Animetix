...
def test_calculate_drift_no_user():
    service = ArchetypeDriftService(
        feedback_port=MagicMock(), memory_service=MagicMock(), repository=MagicMock()
    )
...
def test_calculate_drift_with_user():
    ...
    service = ArchetypeDriftService(
        feedback_port=feedback_mock, memory_service=MagicMock(), repository=repo_mock
    )
