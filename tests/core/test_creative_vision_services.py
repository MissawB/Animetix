...
def test_identify_episode_from_clip(video_service, mock_engine):
    mock_engine.localize_video_actions.return_value = [
        {"answer": "Episode 5: The First Duel", "confidence": 0.95}
    ]
...
def test_studio_transform_video(studio_service, mock_engine):
    ...
    assert (
        studio_service.transform_video_to_anime_sota(b"data", "Ufotable") == "vid_url"
    )
...
def test_translate_manga_page(manga_service, mock_engine):
    ...
    mock_engine.process_manga_page.return_value = {"text": "Hello", "layout": []}
