...
from src.core.domain.services.agentic_rag_service import AgenticRAGService
...
def test_agentic_rag_triggers_thinking_mode_on_complex_query(mock_dependencies):
    ...
    mock_llm_service.generate.return_value = '{"complexity_score": 2, "thinking_budget": 1000}'
    
    # Fix: Mock prompt_manager to return (prompt, system)
    mock_dependencies['prompt_manager'].get_prompt.return_value = ("mock prompt", "mock system")
    ...
    service.critic.evaluate.return_value = MagicMock(is_relevant=True, suggested_action="PROCEED")
    
    # Mock judge
    service.judge = MagicMock()
    service.judge.evaluate.return_value = MagicMock(is_reliable=True, hallucination_detected=False)
