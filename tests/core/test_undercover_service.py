...
def undercover_service(mock_catalog_service, mock_similarity_service):
    return UndercoverService(
        catalog_service=mock_catalog_service, similarity_service=mock_similarity_service
    )


def test_start_game_success(
    undercover_service, mock_catalog_service, mock_similarity_service
):
    ...
    mock_similarity_service.find_similar_items.return_value = {"metadatas": [[item2]]}

    res = undercover_service.start_game(
        "Anime", "Normal", ["P1", "P2"], {"Anime": {"Normal": 100}}
    )
