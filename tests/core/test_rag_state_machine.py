...
def mock_deps():
    ...
    mock_processors = {
        ...
        RAGState.RESEARCH: MagicMock(
            spec=ResearchProcessor,
        ...
        RAGState.SYNTHESIZE: MagicMock(
            spec=SynthesizeProcessor,
            synthesizer=mock_synthesizer,
            xai_service=mock_xai,
            rag_service=mock_rag,
        ),
        RAGState.JUDGE: MagicMock(
            spec=JudgeProcessor, debate_manager=mock_debate_manager
        ),
        RAGState.ACQUIRE_KNOWLEDGE: MagicMock(
            spec=AcquireKnowledgeProcessor, librarian=mock_librarian
        ),
        RAGState.SPECULATE: MagicMock(spec=SpeculateProcessor, forge=mock_forge),
        RAGState.VLM_RERANK: MagicMock(
            spec=VlmRerankProcessor,
            prompt_manager=mock_pm,
            inference_engine=MagicMock(),
        ),
        RAGState.SAGA_LOOKUP: MagicMock(
            spec=SagaLookupProcessor, saga_agent=mock_saga_agent
        ),
        RAGState.GRAPH_EXPLORE: MagicMock(
            spec=GraphExploreProcessor,
            community_partitioner=MagicMock(),
            graph_expert=mock_graph_expert,
            neo4j_manager=MagicMock(),
        ),
        RAGState.FALLBACK_RAG: MagicMock(
            spec=FallbackRagProcessor,
            rag_service=mock_rag,
            inference_engine=MagicMock(),
            expert_facts=[],
        ),
...
def test_research_more_loop_integration(mock_deps):
    ...
    def judge_process_side_effect(ctx):
        ...
        outcome = (
            mock_deps["workflow_orchestrator"]
            .processors[RAGState.JUDGE]
