...
def test_prepare_challenge_daily(paradox_service):
    ...
    t1, t2, intruder = paradox_service.prepare_challenge(catalog, is_daily=True, secret_title='D')
    assert intruder == 'D'
    assert t1 in catalog['titles']
    assert t2 in catalog['titles']

def test_generate_logic(paradox_service, mock_llm_service):
    mock_llm_service.generate_paradox_explanation.return_value = '{"reasoning": "R", "scenario": "S"}'
    res = paradox_service.generate_logic(
        "Anime", 
        {"title": "A"}, {"title": "B"}, {"title": "I"}, 
        "Français"
    )
