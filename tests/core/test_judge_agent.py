...
def test_judge_evaluate_success(mock_llm, mock_pm):
    ...
    res = agent.evaluate("Who is Naruto?", "Naruto is a ninja.", "Naruto is the main character and a ninja.")
...
def test_judge_evaluate_with_obs_service(mock_llm, mock_pm):
    mock_obs = MagicMock()
    agent = ResponseJudge(llm_service=mock_llm, prompt_manager=mock_pm, obs_service=mock_obs)
