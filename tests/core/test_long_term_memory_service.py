...
def mock_prompt_manager():
    ...
    manager.get_prompt.return_value = ("Formatted history summary", "System prompt memory")
    return manager

@pytest.fixture
def memory_service(mock_chroma, mock_engine, mock_prompt_manager):
    return LongTermMemoryService(chroma_resource=mock_chroma, inference_engine=mock_engine, prompt_manager=mock_prompt_manager)
...
def test_store_memory(memory_service, mock_chroma, mock_engine, mock_prompt_manager):
    ...
    mock_engine.generate.assert_called_once_with("Formatted history summary", system_prompt="System prompt memory")
