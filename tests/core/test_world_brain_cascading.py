...
from core.domain.entities.ai_schemas import SearchPlan, RAGState, StreamStep, DebateOutcome, JudgeAction
...
def test_world_brain_cascading_flow(mock_dependencies):
    ...
    with patch.object(AgenticRAGService, '_assess_complexity', return_value=(500, 3)):
        ...
        mock_dependencies["saga_agent"].get_saga_context.return_value = "One Piece est l'histoire de Monkey D. Luffy qui veut devenir le Roi des Pirates."
        ...
        service.rag_service.hybrid_search.return_value = [{"title": "Luffy à Egghead", "description": "Luffy active le Gear 5."}]
        service.scout.find_truth_path = MagicMock(return_value="Détails sur Luffy à Egghead récupérés.")
        
        # 6. Mock Synthesizer
        service.synthesizer.synthesize_stream = MagicMock(return_value=iter(["L'objectif de Luffy ", "est devenu plus concret ", "à Egghead."]))
        ...
        # Assertions
        states_reached = [e['content'] for e in events if e['type'] == 'thought' and "[State Machine]" in e['content']]
