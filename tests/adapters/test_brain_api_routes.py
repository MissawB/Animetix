...
from adapters.inference.brain_api import app, verify_api_key, brain_engine
...
def test_authenticated_generate(monkeypatch):
    ...
    monkeypatch.setattr(brain_engine, "generate", lambda *args, **kwargs: MockInferenceResponse())
    
    response = client.post("/generate", json={"prompt": "Bonjour"}, headers={"X-API-Key": "test-env-key-12345"})
    assert response.status_code == 200
