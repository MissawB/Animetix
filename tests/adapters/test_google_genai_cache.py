...
def test_cache_not_triggered_below_threshold(mock_genai_client, mocker):
    ...
    with patch.dict(os.environ, {"GEMINI_CACHE_THRESHOLD": "100", "GEMINI_MODEL_NAME": "gemini-3.5-flash"}):
        ...
        res = adapter.generate(prompt, system_prompt=system_prompt)
        ...
        assert not hasattr(call_args["config"], "cached_content") or call_args["config"].cached_content is None
...
def test_cache_created_and_used_above_threshold(mock_genai_client, mocker):
    ...
    with patch.dict(os.environ, {"GEMINI_CACHE_THRESHOLD": "50", "GEMINI_CACHE_TTL": "120", "GEMINI_MODEL_NAME": "gemini-3.5-flash"}):
        adapter = GoogleGenAIAdapter(api_key="fake-key")
        
        system_prompt = "Tu es un expert Otaku très sage et instruit avec de grandes connaissances."
        ...
        mock_genai_client.models.generate_content.assert_called_once()
        call_args = mock_genai_client.models.generate_content.call_args[1]
        assert call_args["config"].cached_content == "cachedContents/mock-cache-id-12345"
        assert call_args["config"].system_instruction is None

def test_cache_reuse_on_repeated_calls(mock_genai_client, mocker):
    with patch.dict(os.environ, {"GEMINI_CACHE_THRESHOLD": "50", "GEMINI_MODEL_NAME": "gemini-3.5-flash"}):
        adapter = GoogleGenAIAdapter(api_key="fake-key")
        ...
        call1_config = mock_genai_client.models.generate_content.call_args_list[0][1]["config"]
        call2_config = mock_genai_client.models.generate_content.call_args_list[1][1]["config"]
...
def test_cache_expiration_and_deletion(mock_genai_client, mocker):
    with patch.dict(os.environ, {"GEMINI_CACHE_THRESHOLD": "50", "GEMINI_CACHE_TTL": "1", "GEMINI_MODEL_NAME": "gemini-3.5-flash"}):
        adapter = GoogleGenAIAdapter(api_key="fake-key")
        ...
        mock_genai_client.caches.delete.assert_called_once_with(name="cachedContents/mock-cache-id-12345")
