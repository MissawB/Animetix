...
def test_local_text_adapter_moderate_content_keyword_fallback():
    ...
    res = adapter.moderate_content(
        "This content contains extreme violence and hentai.", ["INAPPROPRIATE"]
    )
    ...
    # Clean text
    res_clean = adapter.moderate_content(
        "Hello Otaku! Welcome to Animetix.", ["INAPPROPRIATE"]
    )
...
def test_unified_inference_adapter_moderate_content_semantic():
    ...
    mock_json_response = (
        '{"is_safe": true, "detected_categories": [], "reason": "Safe text"}'
    )

    with patch.object(adapter, "generate", return_value=mock_json_response):
