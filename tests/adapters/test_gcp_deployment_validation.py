...
def test_postgresql_iam_auth_success(mocker):
    ...
    mock_auth_default = mocker.patch(
        "google.auth.default", return_value=(mock_creds, "mock-project")
    )
    ...
    mock_super_connect = mocker.patch(
...
def test_gcs_storage_lifecycle_and_restart(mocker):
    ...
    def mock_blob_factory(name):
        mb = MagicMock()
        mb.name = name
        mb.upload_from_file = lambda file_obj, **kwargs: in_memory_blobs.__setitem__(
            name, file_obj.read()
        )
        mb.download_to_file = lambda file_obj, **kwargs: file_obj.write(
            in_memory_blobs[name]
        )
        mb.delete = lambda **kwargs: in_memory_blobs.pop(name, None)
        return mb

    # Mock bucket methods
    mock_bucket.get_blob.side_effect = lambda name, **kwargs: (
        mock_blob_factory(name) if name in in_memory_blobs else None
    )
    mock_bucket.blob.side_effect = mock_blob_factory
    mock_bucket.delete_blob.side_effect = lambda name, **kwargs: in_memory_blobs.pop(
        name, None
    )

    # Mock Blob constructor in gcloud.py to return our mock blob
    mocker.patch(
        "storages.backends.gcloud.Blob",
        side_effect=lambda name, bucket, **kwargs: mock_blob_factory(name),
    )
...
def test_settings_production_requires_secrets():
    ...
    try:
        with patch.dict(
            os.environ, {"DJANGO_ENV": "production", "DJANGO_SECRET_KEY": ""}
        ):
...
def test_gcs_storage_custom_endpoint(mocker):
    ...
    gcs_storage = GoogleCloudStorage(
        bucket_name="animetix-test-bucket", custom_endpoint="https://cdn.animetix.com"
    )
