...
def mock_sys_modules(monkeypatch):
    ...
    monkeypatch.setitem(
        sys.modules, "diffusers.models.attention_processor", mock_diffusers_attn
    )
    ...
    from core.utils.lazy_import import _loaded_modules  # noqa: E402
...
def test_transform_video_to_anime_mocked(diffusers_adapter):
    ...
    mock_diffusers.AutoPipelineForImage2Image.from_pretrained.return_value = (
        pipe_instance
    )
    ...
    with patch("builtins.open", MagicMock()):
        with patch("os.unlink", MagicMock()):
            with patch("base64.b64encode", return_value=b"fake_base64"):
                res = diffusers_adapter.transform_video_to_anime(
                    b"fake_video_data", "Ghibli"
                )
...
def test_generate_soundscape_success(xtts_adapter):
    ...
    with patch("scipy.io.wavfile.write", MagicMock()):
        with patch("base64.b64encode", return_value=b"fake_audio_base64"):
            res = xtts_adapter.generate_soundscape(
                {"scene": "forest"}, prompt="forest sounds"
            )
...
def test_generate_soundscape_failure(xtts_adapter):
    ...
    with patch("scipy.io.wavfile.write", side_effect=ImportError("No scipy")):
        with pytest.raises(InferenceError) as excinfo:
            xtts_adapter.generate_soundscape(
                {"scene": "forest"}, prompt="forest sounds"
            )
...
def test_vision_3d_scene_mocked(vision_adapter):
    ...
    rgb_buf = io.BytesIO()
    rgb_img.save(rgb_buf, format="PNG")
    rgb_data = rgb_buf.getvalue()
    depth_buf = io.BytesIO()
    depth_img.save(depth_buf, format="PNG")
    depth_data = depth_buf.getvalue()
...
def test_fatezero_video_logic(diffusers_adapter):
    ...
    with (
        patch("imageio.get_reader") as mock_reader,
...
def test_video_temporal_embeddings_includes_vectors(vision_adapter):
    ...
    with (
        patch.object(vision_adapter, "_load_video_vlm"),
        ...
        vision_adapter._video_processor.batch_decode.return_value = [
            "A scene with combat"
        ]
