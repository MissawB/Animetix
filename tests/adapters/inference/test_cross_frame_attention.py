...
def test_cross_frame_attention_processor_call():
    ...
    assert modified_encoder_hidden_states.shape == (batch_size, sequence_length * 2, channels)
    
    # First half should be original hidden_states
    assert torch.equal(modified_encoder_hidden_states[:, :sequence_length, :], hidden_states)
    
    # Second half should be anchor frame (first frame repeated for batch)
    anchor_frame = hidden_states[0:1].expand(batch_size, -1, -1)
    assert torch.equal(modified_encoder_hidden_states[:, sequence_length:, :], anchor_frame)
