...
def test_check_quota_within_limits():
    ...
    # Log some usage
    adapter.log_usage(
        engine="gpt-3.5-turbo", input_tokens=1000, output_tokens=1000, user_id=user.id
    )
...
def test_check_quota_exceed_tokens():
    ...
    adapter.log_usage(
        engine="gpt-3.5-turbo", input_tokens=30000, output_tokens=20001, user_id=user.id
    )
...
def test_check_quota_exceed_requests():
    ...
    for _ in range(30):
        adapter.log_usage(
            engine="gpt-3.5-turbo", input_tokens=1, output_tokens=1, user_id=user.id
        )
...
def test_check_quota_reset_next_day():
    ...
    usage = AITokenUsage.objects.create(
        user=user, engine="gpt-3.5-turbo", total_tokens=100000, cost_estimate=1.0
    )
...
def test_check_quota_different_tiers():
    ...
    adapter.log_usage(
        engine="gpt-3.5-turbo", input_tokens=30000, output_tokens=30000, user_id=user.id
    )
