...
The adapter mixes two backends:
  ...
  * a pgvector vector store (``vector_manager``) for *semantic* (cosine) lookups.

Both are imported lazily *inside* the methods, so we patch them in their source
modules (``animetix.models`` / ``pipeline.vector_client``) via injected fakes —
no real Django DB and no real pgvector store are touched, keeping the test a
...
def fake_vectors(monkeypatch):
    """Inject a fake ``pipeline.vector_client`` module exposing ``vector_manager``."""
    manager = MagicMock(name="vector_manager")
    fake = types.ModuleType("pipeline.vector_client")
    fake.vector_manager = manager
    monkeypatch.setitem(sys.modules, "pipeline.vector_client", fake)
    return manager
...
def test_get_semantic_empty_embedding_short_circuits(fake_vectors):
    ...
    fake_vectors.query_collection.assert_not_called()


# --- get_semantic: hit / miss threshold branch --------------------------


def _vector_result(distance, meta):
    return {"distances": [[distance]], "metadatas": [[meta]]}


def test_get_semantic_hit_when_similarity_meets_threshold(fake_vectors):
    # distance 0.1 -> similarity 0.9 ; threshold 0.9 -> 0.9 >= 0.9 is a HIT.
    fake_vectors.query_collection.return_value = _vector_result(
    ...
    fake_vectors.query_collection.assert_called_once_with(
...
def test_get_semantic_miss_when_below_threshold(fake_vectors):
...
def test_get_semantic_boundary_exact_threshold_is_hit(fake_vectors):
...
def test_get_semantic_parses_json_string_metadata(fake_vectors):
    # Some backends return metadata as a JSON string; it must be decoded.
    meta = json.dumps({"response": "from json", "query": "q"})
    fake_vectors.query_collection.return_value = _vector_result(0.0, meta)
    assert (
        DjangoSemanticCacheAdapter().get_semantic([1.0], threshold=0.5) == "from json"
    )


def test_get_semantic_invalid_json_string_returns_none(fake_vectors):
    fake_vectors.query_collection.return_value = _vector_result(0.0, "{not json")
    assert DjangoSemanticCacheAdapter().get_semantic([1.0], threshold=0.5) is None


def test_get_semantic_missing_response_key_returns_none(fake_vectors):
...
def test_get_semantic_empty_results_returns_none(fake_vectors):
    fake_vectors.query_collection.return_value = {
        "distances": [[]],
        "metadatas": [[]],
    }
    assert DjangoSemanticCacheAdapter().get_semantic([1.0], threshold=0.5) is None
...
def test_get_semantic_swallows_store_exception(fake_vectors):
    fake_vectors.query_collection.side_effect = RuntimeError("vector store down")
...
def test_set_upserts_relational_and_adds_vector(fake_model, fake_vectors):
    ...
    fake_vectors.add_to_collection.assert_called_once_with(
        collection_name="col",
        ids=[expected_hash],
        embeddings=[[0.1, 0.2]],
...
def test_set_skips_vector_add_when_no_embedding(fake_model, fake_vectors):
    ...
    fake_vectors.add_to_collection.assert_not_called()


def test_set_swallows_vector_add_exception(fake_model, fake_vectors):
    fake_vectors.add_to_collection.side_effect = RuntimeError("boom")
