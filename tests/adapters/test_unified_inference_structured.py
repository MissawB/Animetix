...
def test_unified_generate_structured_success():
    adapter = UnifiedInferenceAdapter(
        api_base="http://localhost:11434/v1", model_name="qwen3.5"
    )
    ...
    with patch.object(
        adapter, "generate", return_value=InferenceResponse(text=mock_user_json)
    ) as mock_generate:
...
def test_unified_generate_structured_failure():
    ...
    with patch.object(
        adapter, "generate", return_value=InferenceResponse(text="Still not JSON")
    ):
        with pytest.raises(Exception) as exc_info:
            adapter.generate_structured(
                prompt="Extract user info", response_model=User, max_retries=2
            )
