...
def test_semantic_moderation_success():
    ...
    res = adapter.moderate_content(
        text="Quelque chose de inapproprié", categories=["nsfw", "violence"]
    )
    ...
    mock_engine.moderate_content.assert_called_once_with(
        "Quelque chose de inapproprié", ["nsfw", "violence"]
    )
...
def test_semantic_moderation_fallback_on_failure():
    ...
    res = adapter.moderate_content(
        text="This contains hentai and porn content", categories=["nsfw"]
    )
    ...
    res_safe = adapter.moderate_content(
        text="This is a very clean discussion about Naruto", categories=["nsfw"]
    )
