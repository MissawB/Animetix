...
def test_update_settings_without_donation(user_client):
    ...
    res = client.patch("/api/v1/profiles/update_settings/", {"custom_username_color": "#FF0000"}, format="json")
...
def test_update_settings_with_donation_success(user_client):
    ...
    res = client.patch("/api/v1/profiles/update_settings/", {"custom_username_color": "#FF66B2"}, format="json")
    assert res.status_code == 200
    ...
    # Reset color
    res = client.patch("/api/v1/profiles/update_settings/", {"custom_username_color": ""}, format="json")
    assert res.status_code == 200
...
def test_update_settings_invalid_color_format(user_client):
    ...
    res = client.patch("/api/v1/profiles/update_settings/", {"custom_username_color": "red"}, format="json")
    assert res.status_code == 400
    assert "Invalid color format" in res.data["error"]
    
    res = client.patch("/api/v1/profiles/update_settings/", {"custom_username_color": "#FFFF"}, format="json")
    assert res.status_code == 400
    
    res = client.patch("/api/v1/profiles/update_settings/", {"custom_username_color": "123456"}, format="json")
    assert res.status_code == 400
