...
def media_items(db):
    for i in range(1, 10):
        MediaItem.objects.create(
            external_id=str(i), media_type="Anime", title=f"Anime {i}", 
            popularity=100.0 - i, metadata={"genres": ["Action"]}
        )
...
def test_media_explore_view(api_client, media_items):
    ...
    action_cat = next((c for c in response.data['categories'] if c['name'] == 'Action'), None)
