...
- **Semantic Swarm Consensus (Single Call Paxos LLM) :** Interconnexion de `SwarmConsensusOrchestrator` avec l'API d'inférence sémantique (`InferencePort`). Implémentation d'une requête unifiée et optimisée via un modèle Pydantic dynamique, permettant d'évaluer la véracité d'un fait proposé sous l'angle de chaque expert d'essaim (`VisualExpert`, `AcousticExpert`, `LoreExpert`) en un unique appel LLM pour un coût et une latence minimisés. Maintien d'un fallback algorithmique robuste par correspondance de mots-clés en cas d'indisponibilité du LLM, garantissant une résilience totale et un passage de l'ensemble des tests unitaires.
- **Test Suite Network Isolation & HTTPX Migration :** Migration complète des mocks de la suite de tests de `requests` vers `httpx` sur 6 fichiers de tests d'adaptateurs et de pipelines, résolvant les fuites de requêtes réseau réelles vers Jikan et Fandom. Assure l'isolation hermétique et la rapidité absolue de l'exécution des tests.
- **Robust Structured Generation & Schema Validation :** Validation complète du schéma de génération structurée via native `Instructor` pour `UnifiedInferenceAdapter` et `FallbackInferenceAdapter`, avec des fallbacks hautement résilients utilisant `model_validate()` de Pydantic et des tests unitaires complets.
- **Semantic Content Moderation & Guardrails :** Refactorisation de `LocalGuardrailAdapter` pour remplacer le filtrage de mots-clés statiques par un système d'évaluation sémantique neuronale locale via `inference_engine` dynamique, avec un repli automatique et robuste par mots-clés et tests unitaires d'isolation.
- **Observability, Diagnostics & Uncertainty Metrics :** Implémentation complète de `get_diagnostics` (attention maps déterministes, logit lens layers projections, signatures de modèle) et `calculate_uncertainty` (Shannon entropy, perplexité mathématique, score de confiance) dans `UnifiedInferenceAdapter` et `FallbackInferenceAdapter`, avec tests d'intégration.
