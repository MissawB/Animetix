...
- [ ] **Tâches planifiées avec Cloud Run Jobs & Cloud Scheduler** (Priorité : BASSE) : Déporter les scripts de maintenance et d'ETL (comme la commande Django `sync_catalog`) vers des Jobs Cloud Run éphémères déclenchés par Cloud Scheduler.