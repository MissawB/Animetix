...
- [x] **Manga Chapter Tracking & Notifications:** Check for updates of favorite mangas in the background via a Celery/Django periodic task and send WebSocket notifications when new chapters are released. ✅ (2026-06-20) — _déclencheur périodique câblé (2026-06-21) : job Cloud Run `animetix-manga-updates` + Cloud Scheduler toutes les 6 h dans `scripts/deploy/deploy_jobs.py` (la task `manga-updates-check` existait mais n'était jamais planifiée). Commande locale documentée._
