...
class AkinetixGameAnswerView(APIView):
    ...
    def post(self, request):
        ...
        answer = answer.upper()
        if answer not in ['OUI', 'NON', 'PEUT-├èTRE', 'PEUT-ETRE']:
            return Response({"error": f"Invalid answer '{answer}'. Expected OUI, NON, or PEUT-├èTRE."}, status=status.HTTP_400_BAD_REQUEST)
...
class ParadoxGameStartView(APIView):
    ...
    def post(self, request):
        ...
        language = session.get('language', 'Fran├ºais')
...
class AkinetixRLStateView(APIView):
    """Récupère l'état actuel de la session RL."""
    permission_classes = [permissions.AllowAny]
    def get(self, request):
        session = GameSessionManager(request)
        state = session.get('akinetix_rl_state', {})
        return Response(state)

class AkinetixRLStartView(APIView):
    """Initialise une nouvelle partie avec l'IA Expert (RL)."""
    permission_classes = [permissions.AllowAny]
    def post(self, request):
        media_type = request.data.get('media_type', 'Anime')
        container = get_container()
        catalog = container.catalog_service.load_data(media_type)
        
        state = container.akinetix_expert_service.start_new_game(catalog['db'])
        
        session = GameSessionManager(request)
        session.set('akinetix_rl_state', state)
        session.set('media_type', media_type)
        
        return Response(state)

class AkinetixRLAnswerView(APIView):
    """Traite la réponse utilisateur via l'IA RL."""
    permission_classes = [permissions.AllowAny]
    def post(self, request):
        answer = request.data.get('answer')
        session = GameSessionManager(request)
        ...
        if not state: return Response({"error": "No RL session"}, status=400)
        
        container = get_container()
        catalog = container.catalog_service.load_data(media_type)
        
        new_state = container.akinetix_expert_service.process_answer(catalog['db'], state, answer)
        session.set('akinetix_rl_state', new_state)
...
class ArchetypistStartFusionView(APIView):
    ...
    def post(self, request):
        ...
        fusion = CreativeFusion.objects.create(
            ...
            scenario_text="G├®n├®ration en cours..."
        )
        ...
        task = chain(
            generate_fusion_scenario_task.s(
                ...
                request.session.get('language', 'Fran├ºais'), 
                chaos_level=chaos_level, universe_balance=universe_balance, art_style=art_style
            ), 
    ...
    def get(self, request):
        ...
        if task.ready():
            try:
                ...
                # Note: 'fusion.image' semble ├¬tre un URLField dans le mod├¿le, 
                # il faut v├®rifier s'il existe une m├®thode ou attribut correct.
                # En attendant, on utilise l'attribut image_url d├®fini dans le mod├¿le.
