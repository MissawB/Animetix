...
def sync_media_item_task(self, media_type, item_id, data):
    """Synchronise un item avec les bases Graphes et Vecteurs."""
    try:
        container = get_container()
...
def scheduled_dpo_optimization():
    """Tâche périodique pour optimiser les prompts."""
    ...
    try:
        ...
        stats = AIFeedback.objects.filter(is_positive=False).values('feedback_type').annotate(rejected_count=Count('id')).filter(rejected_count__gte=MIN_REJECTED_THRESHOLD)
        ...
        for stat in stats:
            ...
            if prompt_key in prompt_manager.prompts:
                try:
                    ...
                    if new_prompt: optimized_categories.append(prompt_key)
                except Exception as e: logger.error(f"Error optimizing '{prompt_key}': {e}")
        return f"Optimization cycle complete. Categories updated: {', '.join(optimized_categories)}"
    finally: cache.delete(lock_id)
...
def cleanup_duel_resources_task(room_code: str):
    ...
    try:
        ...
        if channel_layer:
            ...
            async_to_sync(channel_layer.group_send)(group_name, {"type": "duel.cleanup", "message": "Room closed by system."})
    except ImportError: pass
    return f"Cleanup triggered for {room_code}"

@shared_task
def log_dpo_preference_task(query, chosen, rejected, project_root):
    """Journalisation asynchrone des paires de préférence DPO."""
    import json
    import os
    try:
        ...
        pair_chosen = {"context": query, "output": chosen, "is_positive": True}
        pair_rejected = {"context": query, "output": rejected, "is_positive": False}
