...
class ReasoningAgentService:
    ...
    def execute_react_loop(self, query: str, media_type: str = "Anime", max_iterations: int = 3) -> str:
        ...
        for i in range(max_iterations):
            ...
            if "ACTION: SEARCH" in res:
                ...
                if self.search_service:
                    results = self.search_service.hybrid_search(search_query, media_type, limit=3)
                    for r in results:
                        title = r.get('title') or r.get('name')
                        if title not in sources:
                            context_data += f"- {title}: {r.get('description', '')[:300]}\n"
                ...
                if self.graph_manager and entity:
                    ...
                    context_data += f"\nINFOS GRAPHE ({entity}):\n{graph_res}\n"
                    history.append(f"OBSERVATION: Données de graphe récupérées pour {entity}.")
                else:
                    history.append("OBSERVATION: Graphe non disponible ou entité manquante.")
            
            else:
                # Si le format n'est pas respecté ou action inconnue
                history.append("OBSERVATION: Format non reconnu. Veuillez choisir SEARCH, GRAPH ou ANSWER.")

        # 3. Réponse finale (Final Answer)
        final_prompt, sys = self.prompt_manager.get_prompt(
            "reasoning_final",
            query=query,
            context=context_data if context_data else "Aucun contexte additionnel trouvé.",
            thought="\n".join(history)
        )
