...
class VoiceCloningService:
    ...
    def generate_character_voice(self, text: str, character_audio_sample: bytes, language: str = "fr") -> bytes:
        ...
        audio_bytes = self.inference_engine.clone_voice(
            text=text, 
            reference_audio=character_audio_sample, 
            language=language
        )
...
class NativeSpeechLLMService:
    ...
    def process_voice_interaction(self, user_audio_input: bytes, persona_prompt: str = "") -> Dict[str, Any]:
        """
        ...
        if not response_audio:
            return {"status": "error", "audio_data": b"", "message": "Voice processing failed."}
