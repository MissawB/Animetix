...
from ..exceptions import GameLogicError, AnimetixError
...
class GameService:
    ...
    def __init__(self, 
        ...
        self.undercover_service = undercover_service or UndercoverService(catalog_service, self.similarity_service)
    ...
    def calculate_raw_similarity(self, media_type: str, secret_title: str, guess_title: str, catalog: Dict) -> float:
        ...
        return self.similarity_service.calculate_raw_similarity(media_type, secret_title, guess_title, catalog)
    ...
    def select_secret(self, media_type: str, difficulty: str, rank_limits: Dict) -> str:
        ...
        valid_titles = [item.get('title') or item.get('name') for item in valid_lookup if (item.get('title') or item.get('name')) in catalog.get('title_to_full_data', {})]
    ...
    def select_secret_custom(self, media_type: str, config: Dict, catalog: Dict) -> str:
        ...
        g_white, g_black = config.get('genres_white', []), config.get('genres_black', [])
        t_white, t_black = config.get('tags_white', []), config.get('tags_black', [])
        
        pool = [catalog['title_to_full_data'][t] for t in whitelist if t in catalog['title_to_full_data']] if (mode == 'white' and whitelist) else \
               [item for item in catalog['db'] if (item.get('title') or item.get('name')) not in blacklist]
        
        filtered_pool = []
        for item in pool:
            item_genres, item_tags = set(item.get('genres', [])), set(item.get('tags', []))
            if any(g in item_genres for g in g_black) or any(t in item_tags for t in t_black): continue
            if (g_white or t_white) and not (any(g in item_genres for g in g_white) or any(t in item_tags for t in t_white)): continue
            filtered_pool.append(item)
        
        if not filtered_pool: filtered_pool = catalog['db'][:50]
        if not filtered_pool: return ""
        
        secret_obj = random.choice(filtered_pool)
        return secret_obj.get('title') or secret_obj.get('name')

    def start_undercover_game(self, media_type: str, difficulty: str, player_ids: List[str], rank_limits: Dict) -> Dict:
        """Délégation au UndercoverService."""
        return self.undercover_service.start_game(media_type, difficulty, player_ids, rank_limits)

    def calculate_similarity(self, media_type: str, item_a_id: str, item_b_id: str) -> float:
        """Délégation au SimilarityService."""
        return self.similarity_service.calculate_similarity(media_type, item_a_id, item_b_id)
