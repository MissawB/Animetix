...
class EvaluationResult(BaseModel):
    relevance_score: float = Field(description="Score de pertinence globale du contexte par rapport à la question, entre 0.0 et 1.0")
    is_sufficient: bool = Field(description="True si les documents contiennent suffisamment d'informations pour répondre précisément, False sinon")
    missing_aspects: str = Field(description="Description concise des aspects manquants ou incomplets dans le contexte actuel")
    corrective_query: str = Field(description="Requête de recherche corrective ciblée à exécuter sur le web si le contexte est insuffisant")
...
class RetrievalEvaluator:
    ...
    def evaluate(self, query: str, context: str) -> EvaluationResult:
        ...
        try:
            ...
            logger.info(f"🔍 Retrieval Evaluator (CRAG): Score={result.relevance_score:.2f}, Sufficient={result.is_sufficient} for query='{query}'")
            return result
        except Exception as e:
            logger.error(f"❌ Retrieval Evaluation failed: {e}. Defaulting to sufficient.")
