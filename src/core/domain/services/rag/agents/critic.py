...
class ResponseCritic:
    ...
    def evaluate(self, query: str, context: str, thinking_budget: int = 0) -> CritiqueResult:
        crit_prompt, crit_sys = self.prompt_manager.get_prompt("critic_evaluation", query=query, context=context)
        crit_raw = self.llm_service.generate(crit_prompt, crit_sys, use_slm=True, thinking_budget=thinking_budget)
