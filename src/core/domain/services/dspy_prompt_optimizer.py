...
class DSPyPromptOptimizer:
    ...
    def mutate_template(self, original_template: str, num_mutations: int = 3) -> List[str]:
        """
        Génère des mutations linguistiques d'un prompt d'origine via LLM.
        """
        logger.info(f"🧬 DSPy: Mutating original template (Options: {num_mutations})...")
    ...
    def evaluate_and_select_best(self, original_template: str, test_dataset: List[Dict[str, str]]) -> Tuple[str, float]:
        """
        ...
        for idx, template in enumerate(mutations):
            ...
            logger.info(f"📊 Variant #{idx+1} average evaluation score: {avg_score:.2f}")
    ...
    def _evaluate_accuracy(self, response: str, expected: str) -> float:
        ...
        try:
            score_text = self.inference_engine.generate(
                prompt=judge_prompt,
                system_prompt="Tu es le Juge d'évaluation."
            ).strip()
            import re
