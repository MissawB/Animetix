...
class EpisodicMemoryCompressor:
    def __init__(self, chroma_resource, inference_engine, neo4j_manager: Optional[GraphPersistencePort] = None):
    ...
    def compress_user_memories(self, user_id: str) -> str:
        ...
        """
        logger.info(f"🧠 Memory Compressor: Consolidating memories for user '{user_id}'...")
        ...
        try:
            ...
            logger.info(f"✅ Memory Compressor: Successfully compressed {len(documents)} memories for user {user_id}.")
    ...
    def _link_preferences_to_neo4j(self, user_id: str, profile_summary: str):
        ...
        if not self.neo4j_manager or not hasattr(self.neo4j_manager, "execute_query"):
            logger.warning("⚠️ Neo4j non disponible pour la liaison de profil. Liaison simulée.")
            return
        ...
        try:
            ...
            """
            self.neo4j_manager.execute_query(user_cypher, {"user_id": user_id, "profile": profile_summary})
            ...
            for genre in genres:
                if genre.lower() in profile_summary.lower():
                    ...
                    """
                    self.neo4j_manager.execute_query(link_cypher, {"user_id": user_id, "genre": genre})
            logger.info(f"🕸️ Neo4j user preference graph links established for user {user_id}.")
        except Exception as e:
