...
def run_vectorization(chroma_res=None):
    if not os.path.exists(CLEAN_DB):
        logger.error(f"❌ {CLEAN_DB} not found."); return False
    ...
    logger.info(f"🚀 Vectorizing {len(new_items)} unique new actors (Full Chroma mode)...")
    ...
    logger.info(f"✅ Actor Chroma Sync Complete!")
    return True
