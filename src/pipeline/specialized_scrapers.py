...
logger = logging.getLogger("animetix.pipeline.specialized_scrapers")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for char_entry in characters_data[:8]:
                    ...
                    japanese_va = next((va.get('person', {}).get('name') for va in voice_actors if va.get('language') == 'Japanese'), None)
                    french_va = next((va.get('person', {}).get('name') for va in voice_actors if va.get('language') == 'French'), None)
    ...
    def synthesize_french_reviews(title: str, media_type: str = 'Anime') -> dict:
        ...
        if not gemini_client:
            return {
                ...
                "reviews_sources": [
                    {"site": "Nautiljon", "note": "17/20", "critique": "Une œuvre d'art magnifique."},
                    {"site": "SensCritique", "note": "8/10", "critique": "Une tension narrative incroyable."}
                ]
            }
...
def run_tripartite_enrichment(limit: int = 5, dry_run: bool = False):
    ...
    logger.info(f"🚀 Lancement de la Suite Tripartite de Scraping (Limite: {limit}, Simulation: {dry_run})")
    ...
    for idx, item in enumerate(items):
        ...
        if not dry_run and item.external_id.isdigit():
            cast_data = ScraperA_Casting.scrape_casting(item.external_id, item.media_type)
            time.sleep(1.2) # Jikan limit friendly
        else:
            cast_data = [{"character": "Protagonist", "role": "Main", "seiyuu_vo": "Mock Seiyuu", "doubleur_vf": "Mock VF"}]
        ...
        reviews_data = {}
        if not dry_run:
            reviews_data = ScraperC_Reviews.synthesize_french_reviews(item.title, item.media_type)
        else:
            reviews_data = {
                "score_global_fr": "9.0/10",
                "consensus_fr": "Excellent consensus simulé.",
                "reviews_sources": [{"site": "SensCritique", "note": "9/10", "critique": "Chef d'oeuvre."}]
            }
        ...
        if not dry_run:
            ...
            update_json_metadata_field(json_file, item.external_id, "themes_musicaux", music_data)
            update_json_metadata_field(json_file, item.external_id, "critiques_fr", reviews_data)
            ...
            try:
                ...
                if neo4j_manager and neo4j_manager.driver:
                    ...
                    logger.info("   🕸️ Graphe Neo4j synchronisé pour les métadonnées spécialisées.")
            except Exception as e:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Exécute séquentiellement les 3 scrapers spécialisés sur la DB.")
